...
-- Matrículas novas do mês corrente (por linha, exclui banda/coral/bolsista)
matriculas_mes AS (
  SELECT
    a.unidade_id,
...
alunos_mes AS (
  SELECT
    ...
    -- Ativos: pessoas distintas no snapshot
...
BEGIN
  ...
  -- (inclui quem tem banda, inclui trancado)
  -- ---------------------------------------------------------
  ...
  -- ---------------------------------------------------------
  -- novas_matriculas: linhas novas no mês (exclui banda/coral/bolsista)
  -- ---------------------------------------------------------
