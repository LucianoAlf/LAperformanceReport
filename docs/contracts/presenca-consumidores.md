...
O cutover operacional de `fn_texto_relatorio_presenca` permanece em
`20260827031700_presenca_rollout_adapters.sql`. Para LA Teacher e Fábio, a
última mutação viva é
`20260828005722_presenca_roster_v2_publicacao_gatada.sql`: ela preserva os
corpos legados em aliases privados e só encaminha leitura/escrita às portas v2
no modo `canonico_v2`; `sombra` e `legado` não calculam o roster v2.
...
| LA Teacher | Leitura-base `app_minha_agenda_sessao` em `20260812172432_presenca_canonica_resolvedor_conflitos.sql`; writer em `20260815112104_reverte_precedencia_secretaria_prevalece.sql`; patch do slot em `20260824231632_la_teacher_presenca_pela_canonica_do_slot.sql`; envelope v2 em `20260827031100_la_teacher_presenca_canonica_v2.sql`; request id em `20260827030900_presenca_comando_overloads_compatibilidade.sql`; portas v2 em `20260828005715_presenca_portas_reservadas_duais.sql`; despacho final em `20260828005722_presenca_roster_v2_publicacao_gatada.sql` | Writer preserva decisão forte; agenda do professor recebe ocorrência v2, frescor, versão da regra e estado estrutural sem transformar ausência bruta do Emusys em falta. |
| Fábio por ação/registro | Ledger inicial em `20260827030800_presenca_comando_portas_fabio.sql`; portas reservadas em `20260828005715_presenca_portas_reservadas_duais.sql`; aliases legados e despacho governado em `20260828005722_presenca_roster_v2_publicacao_gatada.sql` | `service_role` apenas; retryable mantém ação/registro abertos e o mesmo request id, enquanto sucesso fecha a origem e publica o recibo. |
