...
A jornada operacional foi reconciliada com a grade recorrente atual do Emusys, sem substituir a autoridade do catalogo formal governado no LA Report. As tres pendencias apontadas eram cursos antigos gravados no payload de matricula, nao atribuicoes atuais dos professores.

Depois do backfill:

- 35 jornadas foram reconciliadas em lotes;
- 35 correcoes possuem trilha em `jornada_curso_resolucao_log`;
- zero excecoes atuais permanecem em `get_professor_curso_modalidade_excecoes_v2(null, 500, false)`;
...
3. A grade recorrente de `aulas_emusys`, resolvida pelo de-para oficial, confirma o curso atual quando o GET de matriculas ainda entrega uma disciplina antiga.
...
- `fn_resolver_jornada_curso_grade_atual_v1`: escolhe o curso atual por unidade e matricula-disciplina.
- `backfill_jornada_curso_grade_atual_v1`: executa reconciliacao idempotente e paginavel.
- trigger em `aluno_jornada_matricula_disciplina`: protege escrita futura contra regressao do payload antigo.
...
```sql
select count(*)
from public.get_professor_curso_modalidade_excecoes_v2(null, 500, false);
