...
- **Conciliação vigente:** os dois acessos raw do núcleo P24 filtram `snapshot_ativo=true`. Reagendamento compara `data + horário` e aceita qualquer estado posterior conhecido; a linha anterior só é ignorada quando não existe presença nem falta raw ativa. A fachada mantém a autorização P23, o cap comercial P21 e a correção de sobra pequena P22, acrescentando a fonte `snapshot_ativo_p24`.
