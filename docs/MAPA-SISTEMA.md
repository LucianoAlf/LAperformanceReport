...
- **Recortes:** mensal usa somente fatos da competência selecionada. Ciclo resolve os intervalos fixos Mar-Abr-Mai, Jun-Jul-Ago, Set-Out-Nov e Dez-Jan-Fev, agregando numeradores e denominadores brutos; não calcula taxa pela média de percentuais nem score pela média dos meses. Dez-Jan-Fev atravessa o ano civil.
...
A projeção mensal reutiliza fatos exclusivos da competência. A projeção de ciclo soma numeradores e denominadores brutos no intervalo fixo, depois executa o mesmo motor de nota e comparabilidade; nunca tira média dos scores mensais. Referências anteriores são apenas visuais, não pontuam e carregam a competência de origem. Falha na consulta auxiliar de cursos não derruba a lista principal de professores. Histórico e snapshots fechados continuam append-only e não são recalculados pela leitura.
