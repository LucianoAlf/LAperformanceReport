...
- **RPCs:** `get_kpis_comercial_canonicos_v2`, `get_conciliacao_experimentais_v2`, `get_experimentais_emusys_operacional_v1`, `pode_gerar_relatorio_comercial_v1`, `buscar_anamnese_pendente`. A aplicação do espelho Emusys usa a RPC privada `aplicar_snapshot_experimentais_emusys_v1`; `admitir_refresh_snapshot_experimentais_v1` e `finalizar_refresh_snapshot_experimentais_v1` governam o single-flight; a conciliação vigente fica no núcleo privado `get_conciliacao_experimentais_snapshot_v1`. As quatro RPCs de escrita/núcleo são executáveis somente por `service_role`, enquanto a fachada pública preserva a assinatura e a ACL de `get_conciliacao_experimentais_v2`.
