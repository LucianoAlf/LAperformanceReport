...
Na aba Performance de `/app/professores`, mês e ciclo consomem `get_health_score_professor_v3_performance`. O ciclo é a leitura principal. A RPC escolhe snapshot oficial quando existir; caso contrário, entrega uma projeção viva calculada no servidor. O modal individual consome o mesmo contrato por `get_health_score_professor_v3_snapshot_modal`.
