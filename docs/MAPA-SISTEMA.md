...
- **RPCs V3:** `get_health_score_professor_v3_snapshot_modal`, `get_health_score_professor_v3_performance`, `get_health_score_professor_v3_config_ui`, `criar_health_score_professor_v3_config_rascunho`, `salvar_health_score_professor_v3_config_rascunho`, `simular_health_score_professor_v3_config`, `ativar_health_score_professor_v3_config`. Todas as leituras preservam `null/sem_base`, fonte, amostra, cobertura e versão da regra.
...
- **Views/fontes V3:** identidade `vw_aluno_identidade_unidade_canonica`; jornada `aluno_jornada_matricula_disciplina`; períodos `vw_professor_periodos_efetivos_v3_sombra`; presença `vw_aluno_presenca_semantica_v1`; roster `aula_alunos_emusys`; experimentais `emusys_experimentais_raw`; saídas `movimentacoes_admin` + `motivos_saida`; snapshots e configurações `health_score_professor_v3_*`.
