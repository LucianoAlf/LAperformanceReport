...
- **Consumo na tela:** o gerador diário de `ComercialPage.tsx` exige uma unidade específica e chama somente `relatorio-admin-whatsapp` em `dry_run_comercial`, com `unidade` e `data_referencia`. A tela não chama o sync nem as três RPCs comerciais para montar esse documento; exibe exatamente o `texto` devolvido pela edge, limpa texto anterior durante a geração e invalida respostas atrasadas quando a unidade ou o período mudam. Copiar e enfileirar usam o mesmo `relatorioTexto` exibido. Os geradores semanal, mensal, de matrículas e comparativos permanecem locais e inalterados.
