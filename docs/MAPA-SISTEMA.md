...
- **RPCs:** `get_kpis_comercial_canonicos_v2`, `get_conciliacao_experimentais_v2`, `get_experimentais_emusys_operacional_v1`, `pode_gerar_relatorio_comercial_v1`, `buscar_anamnese_pendente`. A aplicação do espelho Emusys usa a RPC privada `aplicar_snapshot_experimentais_emusys_v1`; a conciliação vigente fica no núcleo privado `get_conciliacao_experimentais_snapshot_v1`. Ambos são executáveis somente por `service_role`, enquanto a fachada pública preserva a assinatura e a ACL de `get_conciliacao_experimentais_v2`.
- **Snapshot de experimentais:** `emusys_experimentais_raw` mantém o histórico recebido e marca somente uma linha vigente por `unidade + aula Emusys + participante`. `emusys_experimentais_snapshot_execucoes` registra intervalo, contagens e conclusão de cada lote completo. Leituras operacionais e relatórios usam apenas `snapshot_ativo=true`; linhas inativas permanecem exclusivamente como trilha de auditoria.
...
- **Atualização sob demanda:** `sync-presenca-emusys` aceita o modo leve `experimentais` com `unidade_id`, `data_inicio` e `data_fim` explícitos (UUID conhecido e no máximo 45 dias). Ele pagina o intervalo inteiro, aplica uma única fotografia pela RPC privada e somente depois reconcilia por IDs Emusys escopados pela unidade. Curso vem do de-para canônico da própria unidade e o horário é normalizado antes das chaves de negócio. A orquestração pura e injetável fica em `_shared/sync-experimentais-mode.ts`; a edge mantém apenas os adapters Emusys/Supabase. A resposta expõe apenas unidade, intervalo, execução e contagens; não inclui nomes, telefones, nascimento ou payload bruto.
...
- **Orquestração do relatório diário:** `relatorio-admin-whatsapp` atualiza primeiro o snapshot Emusys da unidade, do primeiro dia da competência até D+7, e falha fechado se HTTP, JSON, status, unidade, intervalo ou execução não forem confirmados. Só depois lê, em paralelo, KPIs, conciliação, operação Emusys, metas, agenda futura, lançamentos do dia e matrículas detalhadas nas fontes canônicas; joins por IDs externos permanecem escopados por `unidade_id`, inclusive sob `service_role`, e não há uso de `get_dados_comercial_ia`. A agenda raw seleciona exclusivamente o `snapshot_execucao_id` confirmado pelo preflight e repete a confirmação operacional após a leitura para detectar substituição concorrente. Prévia e cron chamam o mesmo gerador: a prévia exige JWT válido e `pode_gerar_relatorio_comercial_v1` para uma única unidade, enquanto o cron exige `service_role` e grava somente em `fila_relatorios_whatsapp`. A fila identifica `relatorio_admin` e `relatorio_comercial` em `tipo_relatorio`; sua unicidade por `tipo + unidade + JID + dia` permite os dois documentos no mesmo destino e deduplica cada tipo separadamente. A fila manual `fila_relatorios_sol_hermes` não participa desse fluxo. Os contratos dos modos `dry_run`, `dry_run_comercial`, manual e cron foram preservados; a postura de autenticação passiva dos fluxos legados ficou fora do escopo desta correção.
- **Consumo na tela:** o gerador diário de `ComercialPage.tsx` exige uma unidade específica e chama somente `relatorio-admin-whatsapp` em `dry_run_comercial`, com `unidade` e `data_referencia`. A edge exige uma data de calendário estrita em `YYYY-MM-DD`, converte-a no meio-dia BRT sem interpretação UTC da string e devolve `400` para ausência, timestamp, offset ou data impossível; essa referência governa o cabeçalho, o dia consultado e a janela do primeiro dia do mês até D+7. O cron não recebe esse campo e continua resolvendo o instante atual em `America/Sao_Paulo`. A tela não chama o sync nem as três RPCs comerciais para montar esse documento; exibe exatamente o `texto` devolvido pela edge e o associa a uma origem imutável com tipo, unidade, período, datas e competência. Copiar e enfileirar só ficam disponíveis enquanto essa origem coincide com o contexto atual, e o enqueue usa a unidade armazenada com o texto. IDs independentes de geração e envio invalidam respostas atrasadas, inclusive após regeneração na mesma origem; sucesso/erro de envio são limpos em nova geração, troca de contexto, retorno ou fechamento. Os geradores semanal, mensal, de matrículas e comparativos permanecem locais e inalterados.
