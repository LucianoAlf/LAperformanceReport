# Relatório Final v4 — Saneamento Ciclo de Vida + Simulação
## Campo Grande / Maio 2026

**Gerado em:** 31/05/2026  
**Status:** NENHUM UPDATE APLICADO — aguardando aprovação do Alf  
**Arquivos SQL (aprovados):**
- `SIMULACAO_SANEAMENTO_CG_MAIO2026_V4.sql` — Preview 100% read-only (SELECT, zero DDL)
- `UPDATE_SANEAMENTO_CG_MAIO2026_V4.sql` — Updates com guards + asserts corretos (comentado)

**Não aprovado:** `recalcular_dados_mensais` ainda não foi auditada como SQL/migration.
...
| **DEPOIS A** | **500** |
| Nova regra banda: remove 5 ativos banda (Barbara, Leticia, Pedro Lucas + Alan, Leamsi que não estavam no snapshot de qualquer forma; mas no cálculo real remove 5) | -5 |
| **DEPOIS B** | **495** |
...
Apenas os itens abaixo estão aprovados pelo Alf. Não prosseguir além sem nova aprovação.

1. **Executar** `SIMULACAO_SANEAMENTO_CG_MAIO2026_V4.sql` no SQL Editor (validar preview nominal + cenários agregados)
2. **Descomentar e executar** `UPDATE_SANEAMENTO_CG_MAIO2026_V4.sql`
3. **Validar** com query:
   ```sql
...
### ⛔ NÃO APROVADO — AGUARDANDO AUDITORIA

Os itens abaixo **NÃO estão aprovados**. A alteração da função `recalcular_dados_mensais` ainda não foi auditada como SQL/migration.

4. ~~Atualizar `recalcular_dados_mensais`~~ — depende de migration auditada
5. ~~Executar `recalcular_dados_mensais(2026, 5, Campo Grande)`~~ — depende do item 4
6. ~~Backfill Jan-Abr~~ — depende do item 5
7. ~~Replicar para Barra e Recreio~~ — depende do item 5

---

## 9. Diferença para Versão Anterior (v3 → v4)

| Problema v3 | Correção v4 |
|------------|-------------|
| Simulação: erro de sintaxe (parêntese faltante no `no_snapshot_virt`) | **Corrigido.** Parêntese fechado corretamente |
| Update: asserts com `GET DIAGNOSTICS ROW_COUNT` após múltiplos UPDATEs retornavam só o último | **Corrigido.** `WITH updated AS (UPDATE ... FROM (VALUES ...) RETURNING id)` + `SELECT COUNT(*) INTO v_count FROM updated` |
| Grupo A: assert falharia (1 em vez de 16) | **Corrigido.** UPDATE batch com VALUES + assert por SELECT COUNT |
| Grupo C/D/E: mesmo problema de assert | **Corrigido.** Mesma estrutura batch para todos |
