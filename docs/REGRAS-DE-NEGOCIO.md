...
- **Bolsista e matrícula de banda também não contam** (§3.6: `BOLSISTA_INT`, `BOLSISTA_PARC` e `BANDA` têm "Churn ✘"; §3.7). Não é regra nova — mas até 27/08/2026 nenhuma implementação a aplicava, porque todas filtravam só por **curso** e bolsista em curso regular passava batido. O churn ficava aritmeticamente incoerente: `evasoes / alunos_pagantes`, com bolsista no numerador e fora do denominador por definição (`conta_como_pagante = false`).
  ...
  - ⚠️ Cobertura fechada em 27/08 nas **três** camadas: o predicado, o helper `is_movimentacao_admin_retencao_valida` (que delega, alcançando 17 funções) e as **2 funções que chamavam `is_atividade_extra_curso` direto** e por isso não herdavam nada — `get_programa_fideliza_dados` (o desempate do programa é *menor churn*, então uma evasão muda o ranking) e `aplicar_retificacao_relatorio_gerencial_retencao_v1`.
...
⚠️ **`alunos.tempo_permanencia_meses` é POR CONTRATO/matrícula — zera a cada renovação** (renovação abre contrato novo). Para "tempo de escola" da pessoa (aluno VIVO), a fonte canônica é `MIN(data_matricula)` de todas as matrículas da pessoa. No módulo **Bandas**, `banda_integrantes` e `bandas_kpis` usam a helper `banda_permanencia_meses(aluno_id)` (2026-08-24), que calcula exatamente isso — usar `tempo_permanencia_meses` cru ali mostrava ~6 meses quando a real é 30+. E a **permanência média é por PESSOA distinta**: soma dos meses das pessoas em banda ÷ quantidade de pessoas — aluno matriculado em 2 bandas conta uma vez só (antes contava em dobro: CG marcava 41,2 quando a média certa era 38,1).

🔴 **Esta regra JÁ REGREDIU uma vez (28/08/2026) e o motivo importa.** Ao ganharem suporte a integrante manual, `bandas_kpis` e `banda_integrantes` foram reescritas a partir de uma cópia anterior aos três fixes (1ª matrícula, por pessoa distinta, sem teto de 99) — e os três voltaram juntos. O Dashboard passou a mostrar **8,5 meses** de permanência média na rede; Vinícius Lopa (1ª matrícula 14/05/2018) aparecia com **3 meses** e Miguel Bittencourt (12/05/2018) com **5**. Restaurado pela migration `20260828230000_bandas_restaura_permanencia_canonica.sql`: Barra 30,9 · CG 39,7 · Recreio 37,2. ⚠️ **A lição não é sobre Bandas:** função grande reescrita à mão perde correção anterior em silêncio, porque o número novo continua plausível. Alterar corpo de função vivo por `replace()` sobre `pg_get_functiondef()` com guarda do nº de ocorrências, nunca por transcrição. ⚠️ O teto de 99 é armadilha recorrente: com a permanência canônica ele corta exatamente o veterano, já que o piso histórico do Emusys (2018) dá ~99 meses hoje.

🔴 **`emusys_student_id` NÃO é único na rede — é numerado por unidade (29/08/2026).** 91 ids são compartilhados por pessoas diferentes e **zero** deles tem o mesmo nome: é colisão, não transferência (a base toda tem 3 transferências). `banda_permanencia_meses` agrupava só por esse id e fundia estranhos — **Helena de Souza Lima da Silva** aparecia com 62 meses de escola quando são **5** (herdados de Miguel Silva Roca, de outra unidade). Corrigido para a chave **(unidade_id, emusys_student_id)** na migration `20260829120000`; a média de banda da rede caiu de 37,2 para **35,0**, e o número certo é o mais baixo. ⚠️ **O escopo foi verificado antes de aplicar, e ficou contido:** era a única função do banco com a chave sem unidade; a carteira do professor foi testada nos 2 casos reais que a alcançam (Israel/id 1489, Larissa/id 915) e **preserva os dois alunos**; e o pilar "permanência" do Health Score v3 do professor **não usa tempo de casa** (mede retenção) — nenhuma das ~100 funções `health_score_professor_v3` lê `tempo_permanencia_meses`. **Regra geral: chave do Emusys só vale com `unidade_id` junto.**

⚠️ **Sem teto de 99 meses na média de Bandas (2026-08-24, rev. 2).** A primeira versão da `bandas_kpis` filtrava `perm < 99` — herança da sentinela do campo por-contrato. Com a permanência canônica (`MIN(data_matricula)`), isso excluía veteranos REAIS: Miguel Bittencourt e Vinícius Lopa (1ª matrícula 12–14/05/2018, piso histórico — o Emusys foi contratado em 2018, não há data digital anterior; na prática ~12 anos de escola) tinham ~99 meses e ficavam de fora, sub-reportando CG (38,1 em vez de 42,0). Agora a média aceita `perm >= 0` (calouros contam) e só descarta `null` (sem `data_matricula`) e negativos (data futura = erro de dado). O teto de 99 também foi removido do detalhe do integrante em `BandaDetalheDialog`.
...
- **Duas turmas do MESMO professor no MESMO horário quase nunca é conflito de grade ✅** (Anne, ADM Barra, 04/09/2026; **cada item conferido na API do Emusys**). Medidas **134 ocorrências em 38 dias**; depois dos filtros abaixo, **zero** eram colisão real. Quatro causas:
  1. **Aluno em aviso prévio ou trancado continua na grade e não frequenta.** O contrato segue pago, a turma segue marcada, o aluno parou de vir — o professor tem dois horários e atende um. Conferido na API: `MPpi_Qui_18` (Barra) devolve **Gabriela de Lima Sodré em 03/09 e 17/09**, embora ela esteja `trancado` e sem presença real desde 16/07; ao lado, `B_Qui_18` com aluna que frequenta.
  2. **Reposição é lançada POR CIMA do horário do professor, fora da turma, de propósito** — *"reposição a gente não coloca na turma, joga em cima do horário daquele professor, vai separado"*. Vale mesmo quando o aluno já é da turma.
  3. 🔴 **`aulas_emusys.sala_nome` NÃO é atualizada quando o aluno é movido de horário** — *"quando move, só move; não necessariamente troca a sala"*. A sala registrada é a de **origem**; na prática o aluno vai para a sala de quem já estava no horário. **Não usar `sala_nome` como sinal de conflito nem de ocupação real.** Prova na API: `C_Qui_16` (Recreio) movida de qui 03/09 16h → sáb 05/09 09h **mantendo a sala `Cazuza`**, enquanto a `C_Sá_09` que já ocupava o horário está na `Elis Regina`. Foi isso que produziu a leitura falsa de "professor em duas salas ao mesmo tempo" (12 casos, todos explicados).
  4. **Troca de turma sobrepõe as duas por 1-2 semanas** até a antiga ser encerrada. Caso: Pedro de Figueiredo Vieira (Barra) trocou Guitarra→Bateria, com `G_Qui_17` e `B_Qui_17` coexistindo de 20 a 27/08 — o **mesmo aluno** com presença nas duas.
  ⚠️ **Coexistência genuína existe e é rara:** Larissa (Recreio), sexta 09h, `P_Sex_09` [Elton John] + `T_Sex_09` [Mozart], uma aluna em cada — confirmado na API em 11/09. Piano e Teclado são instrumentos irmãos.
- ⚠️ **A falta é maior nesses horários (24,7% × 15,2% do resto da grade), mas a causalidade é INVERSA.** Não é o horário duplicado que gera falta — é o aluno de saída que gera o horário duplicado. Ler ao contrário produz alerta que acusa erro onde não há; foi o que quase virou aviso à operação em 04/09/2026.
- ⚠️ **Contar aluno somando linhas de `aulas_emusys` DOBRA o número.** A API do Emusys devolve cada aula 2× (`turma` + `individual`) — confirmado item a item — com um par por aluno em turma: contar assim deu 9 onde eram 5 e 4 onde eram 2. Usar `vw_presenca_slot_canonica_v1`. E **roster ≠ presença**: `aula_alunos_emusys` diz quem está matriculado no horário, não quem apareceu.
- 🔴 **O Emusys REESCREVE o passado — não auditar data passada pela API.** `GET /aulas` devolve o estado **atual** daquele horário, não a foto do dia. Medido em 04/09/2026: a `P_Qua_16` do Fabricio em 05/08 não existe mais; aquele horário hoje pertence ao Kaio com `T_Qua_16`/`T_Qua_14`. Nosso espelho guarda o que valia na data. Comparar os dois no passado produz "divergência de sync" que não existe.
