...
- **Grade ausente na fonte:** a fotografia completa de `/aulas` só reconcilia aulas normais de hoje/futuro. Aulas ausentes ficam `cancelada=true` com origem `sync_ausente_emusys`; vínculo ausente só sai do roster se a identidade local+Emusys estiver resolvida e `fn_presenca_fecha_chamada` não encontrar presença, falta ou justificativa terminal (inclusive no campo legado `status`). Erro de fonte, upsert ou roster invalida o ciclo automático da unidade. Isso não remove fatos nem altera o numerador/denominador histórico de presença.
...
- **Pesquisa de evasão:** `pesquisa_evasao.evasao_id` referencia a saída canônica em `movimentacoes_admin.id`; envio e resposta são estados da pesquisa, não uma nova contagem de evasão. No motor multipartes, rodadas e versões de análise não multiplicam a taxa de resposta: o denominador e o respondente continuam sendo a pesquisa. Nova rodada após revisão apenas devolve o caso à fila com conteúdo novo, mantendo a versão revisada anterior imutável.
