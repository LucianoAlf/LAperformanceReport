...
- **Export:** `export-contas-receber` modo `snapshot` agora faz **merge** (OPT-IN): snapshot por vencimento + `faturas_pagas_mes` por pagamento, **dedup por `(unidade_id, emusys_fatura_id)`** (a do snapshot vence primeiro; a de pagamento é extra). O manifesto traz `faturas_pagas_mes_extras` = quantas faturas extras por pagamento foram incluídas. **Desligado por padrão** — só liga com `"incluir_pagas_no_mes": true` no body. Sem o parâmetro, o export é exatamente o de antes (mesmos itens, mesmo manifesto, mesmo hash). As faturas extras vêm com `competencia` = competência do vencimento, não a do pagamento; o Super Folha deve consumi-las só no DRE caixa (por data de pagamento), não na gravação por competência.
