...
- **Aplicação atômica e falha fechada:** `aplicar_snapshot_experimentais_emusys_v1` valida execução, unidade, intervalo de até 45 dias, identidades e duplicidades antes de escrever. Na mesma transação, faz upsert da business key vigente, inativa apenas ausentes da unidade/intervalo e registra `emusys_experimentais_snapshot_execucoes` por último. Payload inválido, paginação incompleta ou erro de banco revertem o lote inteiro; a execução não ganha status `completo` e o snapshot vigente anterior permanece publicável.
- **Histórico e frescor:** a ausência em um lote completo não apaga a linha; marca `snapshot_ativo=false` e `snapshot_inativado_em`. Reentrada cria uma nova linha vigente. A leitura operacional conta somente vigentes e expõe ID, conclusão, status e quantidade inativada da última execução completa que cobre do início do mês até D+7; essa janela atravessa a virada da competência quando necessário.
