...
`refresh-contas-receber` é o orquestrador da fila única `financeiro_sync_queue`. Pedido de uma competência primeiro procura um run live completo ainda dentro de `stale_after`: se encontrar, devolve HTTP 200 com o mesmo `sync_run_id`, sem enfileirar. Se já houver job `pending`, `running` ou `retry_wait`, devolve HTTP 202 com o UUID existente. Pedido de um mês fechado específico nunca inclui backlog; o backlog continua reservado aos produtores gerais e inclui mês atual, anterior, seguinte e toda competência cujo último snapshot ainda tenha fatura `aberta` ou `source_missing`. O worker faz claim com lease e processa uma competência por vez; nenhum caminho paralelo deve disputar o mesmo rate limit do Emusys.
