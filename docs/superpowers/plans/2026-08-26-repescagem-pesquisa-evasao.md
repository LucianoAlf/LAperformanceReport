...
**Tech Stack:** PostgreSQL (Supabase, projeto `ouqwbbermlzqqvtqwlul`), Deno edge functions, React 19 + TypeScript, `node --test` para testes de contrato/frontend. **Sem Docker** (decisão do Hugo, 26/08): o comportamento SQL concorrente é verificado à mão no Supabase, em transação com rollback, nos steps de verificação manual das Tasks 3 e 4.
...
| `tests/repescagemEvasaoFilaPostgres.test.mjs` | Contrato das RPCs de fila (texto da migration) |
...
> **Decisão do Hugo (26/08):** este projeto **não usa Docker**. Os testes de
> comportamento em Postgres real ficam fora; a verificação equivalente é feita à
> mão no Supabase, depois de aplicar a migration, e está listada em "Verificação
> manual obrigatória" ao final desta task. O teste automatizado aqui é de
> **contrato**: lê o texto da migration e prova que as regras que importam estão
> escritas do jeito certo. É mais fraco que exercício real — está declarado.

```javascript
// tests/repescagemEvasaoFilaPostgres.test.mjs
import assert from 'node:assert/strict';
import { existsSync, readFileSync } from 'node:fs';
import test from 'node:test';
...
**Verificação manual obrigatória** (substitui os testes de comportamento que
exigiriam Docker — sem ela, a regra do sorteio fica sem exercício real):

```sql
-- 1. Janela util: cada base deve cair em dia util, entre 09:00 e 19:00 BRT.
select base,
       public.proximo_horario_envio_repescagem(base) at time zone 'America/Sao_Paulo' as resultado
from (values
  (timestamptz '2026-08-29 10:00-03'),  -- sabado  -> segunda >= 09:00
  (timestamptz '2026-08-25 19:30-03'),  -- apos as 19h -> dia seguinte >= 09:00
  (timestamptz '2026-08-25 08:00-03')   -- antes das 9h -> mesmo dia 09:00
) as t(base);

-- 2. Sorteio e teto, sem escrever nada: enfileirar um lote de teste dentro de
-- uma transacao e conferir intervalos e contagem por dia, depois abortar.
begin;
  select public.enfileirar_repescagem_evasao(
    array(select id from public.pesquisa_evasao
           where modo_teste = false and resposta_status = 'sem_resposta' limit 35)
  );
  select (agendada_para at time zone 'America/Sao_Paulo')::date as dia, count(*)
    from public.pesquisa_evasao_envios_fila group by 1 order by 1;   -- max 30/dia
  select extract(epoch from (agendada_para - lag(agendada_para)
           over (order by agendada_para)))::int as gap
    from public.pesquisa_evasao_envios_fila order by agendada_para;  -- 90..240
rollback;
```

O `rollback` é o que torna essa checagem segura em produção: nada é gravado.
...
> Mesma decisão da Task 3: **sem Docker**. Teste de contrato aqui, exercício real
> na verificação manual do Step 5.
...
**Verificação manual obrigatória** (o exercício real do claim, que sem Docker não
tem teste automatizado). Numa transação abortada, com uma linha de teste:

```sql
begin;
  -- linha artificial pronta para envio
  insert into public.pesquisa_evasao_envios_fila
    (pesquisa_id, unidade_id, toque, template_id, template_versao, status, agendada_para)
  select p.id, p.unidade_id, 2, t.id, t.versao, 'pendente', now() - interval '1 minute'
    from public.pesquisa_evasao p
    join public.pesquisa_evasao_templates t
      on t.chave = 'evasao_repescagem' and t.publico = 'responsavel'
   where p.modo_teste = false limit 1;

  -- 1. dois claims seguidos: o segundo tem de vir NULL
  select public.claim_repescagem_evasao_job('11111111-1111-1111-1111-111111111111') as primeiro;
  select public.claim_repescagem_evasao_job('22222222-2222-2222-2222-222222222222') as segundo;

  -- 2. lease vencido vira 'falhou', nunca 'pendente'
  update public.pesquisa_evasao_envios_fila
     set lease_expires_at = now() - interval '1 minute' where status = 'enviando';
  select public.claim_repescagem_evasao_job('33333333-3333-3333-3333-333333333333');
  select status, ultimo_erro from public.pesquisa_evasao_envios_fila;
rollback;
```

Esperado: `segundo` = NULL; ao final, `status = 'falhou'` com `ultimo_erro`
começando em `LEASE_EXPIRADO`. Se a segunda chamada devolver a mesma linha, o
claim não é atômico e a implementação está errada.
