...
| `calcular_reajuste_renovacao()` | ATIVA | INVOKER | trigger:renovacoes_legado.trigger_calcular_reajuste |
...
| `fn_agendar_processamento_pesquisa_evasao()` | ATIVA | DEFINER | trigger:pesquisa_evasao_mensagens.trg_agendar_processamento_pesquisa_evasao |
| `fn_aluno_ativo_sem_data_saida()` | ATIVA | INVOKER | trigger:alunos.trg_aluno_ativo_sem_data_saida |
...
| `fn_anamnese_define_pessoa_chave()` | ATIVA | DEFINER | trigger:anamneses.trg_anamnese_pessoa_chave |
| `fn_anamnese_do_aluno(p_aluno_id integer)` | SO-INTERNA | DEFINER | funcao:app_aluno_ficha |
...
| `fn_atualizar_aluno_anamnese()` | ATIVA | DEFINER | trigger:anamneses.trg_anamnese_atualiza_aluno |
| `fn_bloqueia_delete_movimentacao_admin()` | ATIVA | INVOKER | trigger:movimentacoes_admin.trg_bloqueia_delete_movimentacao_admin |
...
| `get_checklists_farmer(p_colaborador_id integer, p_unidade_id uuid, p_status text)` | ATIVA | INVOKER | front:src/components/App/Administrativo/PainelFarmer/hooks/useChecklists.ts, front:src/components/App/Administrativo/PainelFarmer/hooks/useDashboardStats.ts |
...
| `get_historico_ltv(p_unidade_id uuid)` | ATIVA | DEFINER | front:src/components/GestaoMensal/ModalPermanenciaDetalhe.tsx, front:src/hooks/useHistoricoLTV.ts, funcao:get_tempo_permanencia_sem_cache_20260924 |
...
| `get_kpis_alunos_admin_operacional_cache_v1(p_unidade_id uuid, p_ano integer, p_mes integer)` | ATIVA | DEFINER | front:src/components/App/Alunos/AlunosPage.tsx |
...
| `get_kpis_alunos_canonicos(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | DEFINER | funcao:dashboard_aquecer_caches_v1, funcao:get_dados_relatorio_gerencial_legacy_p02r_20260620, funcao:get_dados_retencao_ia, funcao:get_kpis_alunos_canonicos_cache_v1, funcao:gravar_snapshot_fechamento_mensal, funcao:preview_fechamento_mensal |
...
| `get_kpis_alunos_canonicos_cache_v1(p_unidade_id uuid, p_ano integer, p_mes integer)` | ATIVA | DEFINER | front:src/lib/kpisAlunosVivosCanonicos.ts |
...
| `get_radar_renovacoes(p_unidade_id uuid)` | ORFA | INVOKER | sem consumidor conhecido |
| `get_respostas_evasao(p_unidade_id uuid, p_ano integer, p_mes integer)` | ORFA | INVOKER | sem consumidor conhecido |
| `get_respostas_pesquisa(p_unidade_id uuid, p_data_inicio date, p_data_fim date)` | ATIVA | INVOKER | front:src/components/App/SucessoCliente/hooks/useAnalisePesquisas.ts |
...
| `get_situacao_alunos_resumo_v1(p_unidade_id uuid, p_referencia date)` | SO-INTERNA | DEFINER | funcao:sol_porta_situacao_alunos_v1 |
...
| `get_tempo_permanencia(p_unidade_id uuid, p_ano integer, p_mes integer)` | ATIVA | DEFINER | front:src/components/App/Alunos/AlunosPage.tsx, funcao:dashboard_aquecer_caches_v1, funcao:get_kpis_alunos_canonicos_base_v131 |
| `get_tempo_permanencia_sem_cache_20260924(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | DEFINER | funcao:get_tempo_permanencia |
...
| `radar_detectar_aviso_previo_v1(p_data date)` | ATIVA | DEFINER | cron:radar-detectar-sinais-diario (inativo), funcao:radar_rodada_diaria_v1, funcao:radar_sincronizar_detectores_v1 |
| `radar_detectar_calor_atendimento_v1(p_horas_minimas integer)` | ATIVA | DEFINER | edge:supabase/functions/ingerir-calor-atendimento/index.ts, funcao:radar_rodada_diaria_v1, funcao:radar_sincronizar_detectores_v1 |
| `radar_detectar_matricula_sem_anamnese_v1()` | SO-INTERNA | DEFINER | funcao:radar_rodada_diaria_v1, funcao:radar_sincronizar_detectores_v1 |
| `radar_detectar_sinais_comercial_v1()` | ATIVA | DEFINER | cron:radar-detectar-sinais-comercial-diario (inativo), funcao:radar_rodada_diaria_v1, funcao:radar_sincronizar_detectores_v1 |
| `radar_detectar_sinais_sql_v1(p_data date)` | ATIVA | DEFINER | cron:radar-detectar-sinais-diario (inativo), funcao:radar_rodada_diaria_v1, funcao:radar_sincronizar_detectores_v1 |
...
| `radar_enfileirar_pauta_v1(p_dry_run boolean, p_data date)` | ATIVA | DEFINER | cron:radar-pauta-grupo |
| `radar_ficha_v1(p_unidade_id uuid, p_severidade_min text, p_limite integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `radar_guarda_elegibilidade()` | ATIVA | INVOKER | edge:supabase/functions/extrair-sinais-conversa/index.ts, trigger:radar_sinais.trg_radar_guarda_elegibilidade |
| `radar_marcar_foto_conversas_v1(p_conversa_ids integer[], p_truncado boolean, p_janela_dias integer)` | ATIVA | DEFINER | edge:supabase/functions/extrair-sinais-conversa/index.ts |
...
| `radar_sincronizar_detectores_v1()` | SO-INTERNA | DEFINER | funcao:radar_rodada_diaria_v1 |
| `radar_texto_operacional_v1(p_itens jsonb, p_na_fila integer)` | SO-INTERNA | DEFINER | funcao:radar_pauta_v1 |
| `radar_trafego_canal_v1(p_dias integer, p_maturidade_dias integer, p_de date, p_ate date)` | ATIVA | DEFINER | edge:supabase/functions/capturar-google-ads-diario/index.ts |
...
| `resolver_aluno_caixa_por_telefone_v1(p_telefone text, p_unidade_id uuid)` | ATIVA | INVOKER | edge:supabase/functions/_shared/caixa-dono-do-numero.ts, edge:supabase/functions/webhook-whatsapp-inbox/index.ts |
...
| `atualizar_conversa_on_mensagem()` | ATIVA | INVOKER | trigger:crm_mensagens.tr_atualizar_conversa_on_mensagem |
...
| `congelar_professor_experimental_relatorio_comercial_v1(p_payload jsonb)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_comercial_mensal_payload_v1 |
| `consolidar_origem_leads_mes(p_ano integer, p_mes integer)` | ORFA | INVOKER | sem consumidor conhecido |
| `convertidos_do_periodo_v1(p_meses integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `creditar_lalita_matricula()` | ORFA | INVOKER | sem consumidor conhecido |
...
| `fn_texto_emusys_experimental(p_registro_id uuid)` | SO-INTERNA | INVOKER | funcao:fn_emusys_escrita_pendentes |
...
| `fn_variantes_telefone_br(p_numero text)` | SO-INTERNA | INVOKER | funcao:fn_pedir_codigo_de_acesso |
...
| `mila_base_comercial_v1(p_solicitante_telefone text, p_situacao text, p_limite integer, p_origem text)` | ORFA | DEFINER | sem consumidor conhecido |
| `mila_briefing_lideranca_v1(p_solicitante_telefone text, p_data date, p_tipo text)` | ORFA | DEFINER | sem consumidor conhecido |
| `mila_briefing_manha_v1(p_solicitante_telefone text, p_data date, p_unidade_id uuid)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `mila_conversa_do_lead_v1(p_solicitante_telefone text, p_lead_id bigint)` | ORFA | DEFINER | sem consumidor conhecido |
| `mila_cutucada_v1(p_solicitante_telefone text, p_limite integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `mila_desfecho_retomada_v1(p_solicitante_telefone text, p_retomada_id uuid, p_desfecho text, p_nota text)` | SO-INTERNA | DEFINER | funcao:mila_retomadas_do_dia_v1 |
| `mila_envio_proativo_pendente_v1(p_telefone text, p_horas integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `mila_registrar_eficacia_v1(p_solicitante_telefone text, p_codigo_estrategia text, p_resultado text, p_rotulo text, p_comparador text)` | ORFA | DEFINER | sem consumidor conhecido |
| `mila_registrar_entrega_dm_v1(p_solicitante_telefone text, p_origem text, p_itens jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
| `mila_registrar_envio_proativo_v1(p_telefone text, p_texto text, p_origem text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `mila_responder_cutucada_v1(p_solicitante_telefone text, p_opcao integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `normalize_phone(tel text)` | ORFA | INVOKER | sem consumidor conhecido |
| `normalize_telefone(tel text)` | SO-INTERNA | INVOKER | funcao:get_candidatos_pesquisa_primeira_aula, funcao:trigger_normalize_telefone |
...
| `retificar_experimentais_status_operacional_v1(p_ano integer, p_mes integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `update_mila_config_updated_at()` | ATIVA | INVOKER | trigger:mila_config.trigger_mila_config_updated_at |
...
| `complete_sync_faturas_pagas_mes_job(p_job_id uuid, p_worker_id uuid)` | ORFA | DEFINER | sem consumidor conhecido |
| `complete_sync_financeiro_emusys_job(p_job_id uuid, p_worker_id uuid)` | ATIVA | DEFINER | edge:supabase/functions/sync-financeiro-emusys/index.ts |
...
| `fail_sync_financeiro_emusys_job(p_job_id uuid, p_worker_id uuid, p_error_code text, p_error_detail text, p_http_status integer)` | ATIVA | DEFINER | edge:supabase/functions/sync-financeiro-emusys/index.ts |
...
| `financeiro_enriquecer_fatura_item(p_item jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
| `financeiro_enriquecer_faturas_itens_v1(p_items jsonb)` | SO-INTERNA | DEFINER | funcao:get_faturas_alunos_financeiro_v1_contrato_tipo_20260817 |
...
| `financeiro_fatura_reconciliacao_decisao_immutavel()` | ATIVA | INVOKER | trigger:financeiro_fatura_reconciliacao_decisoes.financeiro_fatura_reconciliacao_decisao_immutavel |
...
| `fn_financeiro_snapshot_append_only()` | ATIVA | INVOKER | trigger:emusys_fatura_source_events.trg_emusys_fatura_source_events_append_only, trigger:sync_run_items.trg_sync_run_items_append_only, trigger:sync_run_overrides.trg_sync_run_overrides_append_only |
| `fn_financeiro_sync_run_guard()` | ATIVA | INVOKER | trigger:sync_runs.trg_sync_runs_guard |
...
| `get_faturas_alunos_financeiro_v1(p_unidade_id uuid, p_ano integer, p_mes integer, p_modo_periodo text, p_status text, p_as_of_date date)` | ATIVA | DEFINER | front:src/hooks/useFaturasParaCaixa.ts, front:src/lib/faturasAlunosFinanceiras.ts, funcao:dashboard_aquecer_caches_v1, funcao:sol_faturas_alunos_v1 |
...
| `get_financeiro_espelho_status()` | ATIVA | DEFINER | front:src/components/App/FaturasAlunos/EspelhoFinanceiroSaude.tsx |
| `get_financeiro_faturas_emusys(p_unidade_id uuid, p_ano integer, p_mes integer)` | ATIVA | DEFINER | front:src/components/App/Administrativo/ModalRelatorio.tsx, front:src/lib/financeiroFaturasEmusys.ts, funcao:aplicar_retificacao_relatorio_gerencial_financeiro_v1, funcao:dashboard_aquecer_caches_v1, funcao:fechar_competencia_mensal_canonica_v2, funcao:fechar_competencia_mensal_dia1_v1, +2 outros |
| `get_financeiro_faturas_emusys_base_ticket_contratual_v1(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | DEFINER | funcao:get_financeiro_faturas_emusys_sem_cache_20260924 |
| `get_financeiro_faturas_emusys_sem_cache_20260924(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | DEFINER | funcao:get_financeiro_faturas_emusys |
| `get_inadimplencia_canonica(p_unidade_id uuid, p_as_of_date date)` | ATIVA | DEFINER | front:src/components/App/Administrativo/PainelFarmer/hooks/useAlertas.ts, front:src/components/App/Alunos/AlunosPage.tsx, front:src/lib/faturasAlunosCanonicas.ts, edge:supabase/functions/atualizar-inadimplencia-emusys/index.ts, edge:supabase/functions/export-contas-receber/index.ts, funcao:dashboard_aquecer_caches_v1, +6 outros |
| `get_inadimplencia_canonica_sem_cache_20260924(p_unidade_id uuid, p_as_of_date date)` | SO-INTERNA | DEFINER | funcao:get_inadimplencia_canonica |
...
| `get_inadimplencia_canonica_v4_base(p_unidade_id uuid, p_as_of_date date)` | SO-INTERNA | DEFINER | funcao:get_inadimplencia_canonica_sem_cache_20260924 |
...
| `get_pix_migracao_v1(p_unidade_id uuid, p_fatia text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `retry_sync_financeiro_emusys_job(p_job_id uuid, p_worker_id uuid, p_error_code text, p_error_detail text, p_http_status integer, p_retry_after_seconds integer)` | ATIVA | DEFINER | edge:supabase/functions/sync-financeiro-emusys/index.ts |
...
| `sol_caixa_buscar_movimentos_v1(p_payload jsonb)` | SO-INTERNA | DEFINER | funcao:sol_porta_caixa_localizar_lancamento_v1 |
| `sol_caixa_casar_parcela(p_unidade_id uuid, p_aluno text, p_valor numeric, p_competencia text)` | SO-INTERNA | DEFINER | funcao:sol_caixa_resolver_pagamento_v1 |
...
| `sol_caixa_executar_abertura_fechamento_v3(p_payload jsonb, p_operacao text)` | SO-INTERNA | DEFINER | funcao:sol_caixa_abrir_v3, funcao:sol_caixa_fechar_v3 |
...
| `sol_caixa_normalizar_competencia_v1(p_competencia text)` | SO-INTERNA | INVOKER | funcao:sol_caixa_resolver_composto_aluno_env_v1, funcao:sol_caixa_resolver_multi_aluno_v1 |
...
| `sol_caixa_parcela_canonica_env_v1(p_env_in jsonb, p_unidade_id uuid, p_aluno text, p_valor numeric, p_as_of date)` | SO-INTERNA | DEFINER | funcao:sol_caixa_parcela_canonica, funcao:sol_caixa_resolver_pagamento_v1 |
...
| `sol_caixa_recalcular_cofre(p_caixa_diario_id uuid)` | SO-INTERNA | DEFINER | funcao:sol_caixa_corrigir_movimento_v1, funcao:sol_caixa_estornar_movimento_v1, funcao:sol_caixa_lancar_saida, funcao:trg_caixa_movimentacao_recalcula_saldo |
| `sol_caixa_resolver_abertura_v3(p_payload jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_caixa_resolver_composto_aluno_env_v1(p_env_in jsonb, p_payload jsonb)` | SO-INTERNA | DEFINER | funcao:sol_caixa_resolver_composto_aluno_v1, funcao:sol_caixa_resolver_pagamento_v1 |
| `sol_caixa_resolver_composto_aluno_v1(p_payload jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_caixa_resolver_envelope_v1(p_unidade_id uuid, p_envelope jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `sol_caixa_resolver_pagamento_itens_v1(p_unidade_id uuid, p_itens jsonb, p_valor_total numeric, p_competencia date)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_caixa_resolver_pagamento_v1(p_unidade_id uuid, p_itens jsonb, p_valor_total numeric, p_competencia date)` | SO-INTERNA | DEFINER | funcao:sol_caixa_resolver_pagamento_itens_v1 |
| `sol_caixa_responsavel_aluno(p_unidade_id uuid, p_aluno text)` | SO-INTERNA | DEFINER | funcao:sol_caixa_resolver_composto_aluno_env_v1, funcao:sol_caixa_resolver_envelope_v1, funcao:sol_caixa_resolver_pagamento_itens_v1, funcao:sol_caixa_resolver_pagamento_v1 |
| `sol_caixa_resumo_do_dia(p_unidade_id uuid, p_data date)` | SO-INTERNA | DEFINER | funcao:sol_porta_caixa_do_dia_assinado_v1, funcao:sol_porta_caixa_do_dia_v1 |
| `sol_caixa_saldo_fisico_v1(p_caixa_diario_id uuid)` | SO-INTERNA | DEFINER | funcao:sol_caixa_abrir, funcao:sol_caixa_dados_fechamento, funcao:sol_caixa_fechar, funcao:sol_caixa_recalcular_cofre, funcao:sol_caixa_snapshot_abertura_fechamento_v3 |
...
| `sol_porta_caixa_contexto_v1(p_cracha text, p_chat_id text)` | SO-INTERNA | DEFINER | funcao:sol_porta_caixa_do_dia_assinado_v1, funcao:sol_porta_caixa_localizar_lancamento_v1 |
| `sol_porta_caixa_do_dia_assinado_v1(p_cracha text, p_chat_id text, p_data date)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_porta_caixa_do_dia_v1(p_solicitante_telefone text, p_unidade text, p_data date)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_porta_caixa_localizar_lancamento_v1(p_cracha text, p_chat_id text, p_data_inicio date, p_data_fim date, p_valor numeric, p_categoria text, p_forma text, p_texto text)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_porta_contratos_vencendo_v1(p_solicitante_telefone text, p_unidade text, p_dias integer)` | SO-INTERNA | DEFINER | funcao:sol_governanca_snapshot_operacional_v1 |
| `sol_porta_faturas_do_mes_v1(p_solicitante_telefone text, p_unidade text, p_ano integer, p_mes integer, p_status text)` | SO-INTERNA | DEFINER | funcao:sol_governanca_snapshot_operacional_v1 |
| `sol_porta_inadimplencia_v1(p_solicitante_telefone text, p_unidade text)` | SO-INTERNA | DEFINER | funcao:sol_governanca_snapshot_operacional_v1 |
...
| `trg_caixa_movimentacao_recalcula_saldo()` | ATIVA | DEFINER | trigger:caixa_movimentacoes.trg_caixa_movimentacao_recalcula_saldo |
| `trg_marcar_contratos_para_recalculo()` | ATIVA | DEFINER | trigger:calendario_escolar.trg_marcar_contratos_para_recalculo |
...
| `get_analise_pesquisas(p_unidade_id uuid, p_data_inicio date, p_data_fim date)` | ATIVA | INVOKER | front:src/components/App/SucessoCliente/hooks/useAnalisePesquisas.ts |
| `get_comparativo_anos(p_ano_atual integer, p_ano_anterior integer)` | ATIVA | INVOKER | front:src/hooks/useSupabase.ts |
...
| `get_relatorio_coordenacao_canonico_v3(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | ORFA | DEFINER | sem consumidor conhecido |
| `get_relatorio_coordenacao_documento_v4(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | ATIVA | DEFINER | front:src/components/App/Professores/ModalRelatorioCoordenacao.tsx, edge:supabase/functions/gemini-relatorio-coordenacao/index.ts, funcao:get_relatorio_coordenacao_canonico_v3 |
| `get_relatorio_coordenacao_documento_v4_por_id(p_documento_id uuid)` | ATIVA | DEFINER | edge:supabase/functions/gemini-relatorio-coordenacao/index.ts |
...
| `montar_relatorio_comercial_mensal_payload_sem_pagantes_v1(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | DEFINER | funcao:montar_rel_comercial_payload_before_professor_imutavel_20260909 |
...
| `prever_projecao_contrato(p_unidade_id uuid, p_dia_semana text, p_data_inicio date, p_qtd_aulas integer)` | ATIVA | DEFINER | front:src/components/App/Alunos/ModalNovoAluno.tsx |
| `proteger_relatorio_coordenacao_final_v4()` | ATIVA | DEFINER | trigger:fechamento_mensal_snapshots.trg_relatorio_coordenacao_final_v4 |
...
| `relatorio_coordenacao_matriculas_base_v4(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text, p_data_corte date)` | SO-INTERNA | DEFINER | funcao:relatorio_coordenacao_matriculas_v4 |
| `relatorio_coordenacao_matriculas_v4(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text, p_data_corte date)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_conteudo_base_v4 |
| `relatorio_coordenacao_periodos_v4(p_ano integer, p_mes integer, p_periodicidade text, p_data_corte date)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_conteudo_base_v4, funcao:relatorio_coordenacao_carteira_v4, funcao:relatorio_coordenacao_matriculas_base_v4, funcao:relatorio_coordenacao_presenca_v4 |
| `relatorio_coordenacao_presenca_v4(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text, p_data_corte date)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_conteudo_base_v4 |
| `relatorio_coordenacao_saidas_v4(p_unidade_id uuid, p_inicio date, p_fim date)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_conteudo_base_v4 |
| `relatorio_coordenacao_unidades_v4(p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:relatorio_coordenacao_carteira_v4, funcao:relatorio_coordenacao_matriculas_base_v4 |
| `relatorio_matriculas_texto_v1(p_unidade_id uuid, p_ano integer, p_mes integer, p_gerado_por text, p_solicitante_telefone text)` | ATIVA | DEFINER | front:src/components/App/Comercial/ComercialPage.tsx |
| `relatorio_variacao_texto_v1(p_atual numeric, p_anterior numeric)` | SO-INTERNA | INVOKER | funcao:relatorio_comparativo_texto_v1 |
...
| `hermes_patch_status_reportar(p_agente text, p_patch text, p_patched boolean, p_detalhe text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `normalizar_whatsapp_message_id()` | ATIVA | INVOKER | trigger:admin_mensagens.trg_normalizar_wa_msg_id, trigger:crm_mensagens.trg_normalizar_wa_msg_id |
...
| `sync_experimentais_unidade()` | ATIVA | INVOKER | trigger:leads.tr_sync_experimentais_unidade |
| `sync_lead_etapa_status_on_insert()` | ATIVA | INVOKER | trigger:leads.tr_sync_etapa_status_on_insert |
...
| `calcular_comissao_venda()` | ATIVA | INVOKER | trigger:loja_vendas.trigger_calcular_comissao_venda |
...
| `simular_emenda(p_unidade_id uuid, p_data date, p_ano integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `transferir_estoque(p_produto_id integer, p_variacao_id integer, p_unidade_origem uuid, p_unidade_destino uuid, p_quantidade integer, p_motivo text, p_via_audit text)` | ORFA | INVOKER | sem consumidor conhecido |
| `transferir_estoque(p_produto_id uuid, p_variacao_id uuid, p_unidade_origem uuid, p_unidade_destino uuid, p_quantidade integer, p_motivo text, p_via_audit text)` | ORFA | INVOKER | sem consumidor conhecido |
| `update_inventario_updated_at()` | ATIVA | INVOKER | trigger:inventario.trigger_inventario_updated_at |
| `update_planos_acao_updated_at()` | ATIVA | INVOKER | trigger:planos_acao.trigger_planos_acao_updated_at |
| `update_projeto_fases_updated_at()` | ATIVA | INVOKER | trigger:projeto_fases.trigger_projeto_fases_updated_at |
| `update_projeto_tarefas_updated_at()` | ATIVA | INVOKER | trigger:projeto_tarefas.trigger_projeto_tarefas_updated_at |
...
| `admin_conversa_usar_numero_do_cadastro_v1(p_conversa_id uuid)` | ATIVA | DEFINER | front:src/components/App/Administrativo/CaixaEntrada/AdminChatPanel.tsx, front:src/lib/numeroDaConversa.ts |
...
| `app_minha_agenda_sessao_canonica_v2(p_data date)` | SO-INTERNA | DEFINER | funcao:app_minha_agenda_semana_v1, funcao:app_minha_agenda_sessao, funcao:app_minha_agenda_sessao_publicacao_legado_v1, funcao:fn_aulas_da_sessao, funcao:fn_onde_parou_enfileirar |
...
| `app_minha_divida()` | SO-INTERNA | DEFINER | funcao:app_minha_home |
...
| `bloquear_mutacao_retificacao_mensal_v1()` | ATIVA | INVOKER | trigger:fechamento_mensal_retificacoes.fechamento_mensal_retificacoes_append_only |
| `calc_classificacao(idade integer)` | ORFA | INVOKER | sem consumidor conhecido |
| `calc_idade(data_nasc date)` | ORFA | INVOKER | sem consumidor conhecido |
| `calcular_campos_aluno()` | ATIVA | INVOKER | front:src/components/App/Comercial/ComercialPage.tsx, trigger:alunos.trg_alunos_calcular_campos |
...
| `exigir_acesso_kpis_admin_v1(p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:kpis_alunos_admin_operacional_sem_cache_20260924 |
| `fmt_brl_v1(p_valor numeric)` | SO-INTERNA | INVOKER | funcao:relatorio_comparativo_texto_v1, funcao:relatorio_matriculas_texto_v1 |
| `fn_aplicar_edicao_campos(p_campos jsonb, p_mudancas jsonb)` | SO-INTERNA | INVOKER | funcao:fn_editar_registro_confirmado_core |
...
| `fn_audit_log()` | ATIVA | DEFINER | trigger:alunos.trg_audit, trigger:caixa_movimentacoes.trg_audit_caixa_movimentacoes, trigger:caixa_reaberturas_log.trg_audit_caixa_reaberturas_log, trigger:caixas_diarios.trg_audit_caixas_diarios, trigger:config_health_score_professor.trg_audit, trigger:crm_pipeline_etapas.trg_audit, +19 outros |
...
| `fn_costura_vincular_conversa_numero()` | ATIVA | INVOKER | trigger:alunos.trg_costura_vincular_conversa |
| `fn_data_corte_cobranca()` | ATIVA | INVOKER | view:vw_registro_pendencia, funcao:fn_data_corte_cobranca_do_professor |
| `fn_data_corte_cobranca_do_professor(p_professor_id integer)` | ATIVA | INVOKER | view:vw_aderencia_registro_professor, view:vw_registro_pendencia, funcao:fabio_contexto_professor |
| `fn_data_corte_experimental()` | ATIVA | INVOKER | view:vw_experimental_pendencia |
| `fn_data_inicio_medicao()` | ATIVA | INVOKER | view:vw_aderencia_registro_professor |
| `fn_diag_audio_cortado(p_horas integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_diag_audio_preso_no_aparelho(p_horas integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_diag_aulas_encerradas_do_teste(p_horas integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_diag_fora_do_prazo_antes_do_acesso(p_limite_dias integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_usuarios_trava_privilegio()` | ATIVA | DEFINER | trigger:usuarios.trg_usuarios_trava_privilegio |
| `get_cron_health()` | ATIVA | DEFINER | front:src/hooks/useSaudeCrons.ts |
...
| `la_os_erro_triar(p_grupo_id uuid, p_triagem text, p_nota text, p_quem text)` | ORFA | DEFINER | sem consumidor conhecido |
| `la_os_erros_grupos(p_agente text, p_origem text, p_triagem text)` | ORFA | DEFINER | sem consumidor conhecido |
| `la_os_erros_ocorrencias(p_grupo_id uuid, p_limite integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `la_os_erros_saude()` | ORFA | DEFINER | sem consumidor conhecido |
...
| `set_updated_at()` | ATIVA | INVOKER | trigger:agente_conversas.set_updated_at_agente_conversas, trigger:agentes.set_updated_at_agentes, trigger:alunos_emusys_atributos_divergencias.set_updated_at_alunos_emusys_atributos_divergencias, trigger:bi_ai_query_playbooks.trg_bi_ai_query_playbooks_updated_at, trigger:bi_conversations_lamusic.trg_bi_conversations_updated_at, trigger:bi_messages_lamusic.trg_bi_messages_updated_at, +9 outros |
...
| `sol_cracha_emitir_v1(p_telefone text, p_chat text, p_em timestamp with time zone)` | ORFA | DEFINER | sem consumidor conhecido |
| `sol_cracha_verificar_v1(p_cracha text, p_chat text)` | SO-INTERNA | DEFINER | funcao:sol_porta_caixa_contexto_v1, funcao:sol_resolver_escopo_v1 |
| `sol_governanca_append_evento_interno_v1(p_entity_type text, p_entity_id text, p_event_type text, p_actor_role text, p_retention_class text, p_payload jsonb, p_ingestion_key text)` | SO-INTERNA | DEFINER | funcao:sol_governanca_registrar_rodada_v1 |
| `sol_governanca_bloquear_mutacao_v1()` | ATIVA | INVOKER | trigger:sol_governanca_eventos.sol_governanca_eventos_no_truncate, trigger:sol_governanca_eventos.sol_governanca_eventos_no_update_delete |
| `sol_governanca_payload_sanitizado_v1(p_value jsonb)` | SO-INTERNA | INVOKER | funcao:sol_governanca_append_evento_interno_v1, funcao:sol_governanca_registrar_rodada_v1 |
| `sol_governanca_readback_v1()` | ORFA | DEFINER | sem consumidor conhecido |
...
| `update_onboarding_updated_at()` | ATIVA | INVOKER | trigger:usuario_onboarding.update_usuario_onboarding_updated_at |
| `update_updated_at()` | ATIVA | INVOKER | trigger:anotacoes.tr_anotacoes_updated_at, trigger:dados_mensais.tr_dados_mensais_updated_at, trigger:dashboard_config.tr_dashboard_config_updated_at, trigger:metas_legado.tr_metas_updated_at, trigger:unidades.tr_unidades_updated_at |
...
| `app_coordenacao_feedback_mes(p_competencia date, p_unidade_id uuid, p_limite integer, p_coracao text, p_professor_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `app_coordenacao_radar(p_unidade_id uuid, p_professor_id integer, p_status text, p_limite integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `app_corrigir_presenca_do_aluno(p_aula_emusys_id integer, p_aluno_id integer, p_status_presenca text, p_confirmo boolean)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `app_editar_registro_confirmado(p_registro_id uuid, p_versao integer, p_tronco jsonb, p_fatias jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `app_fabio_chat_enviar(p_texto text)` | ORFA | DEFINER | sem consumidor conhecido |
| `app_fabio_chat_listar(p_desde timestamp with time zone)` | ORFA | DEFINER | sem consumidor conhecido |
| `app_fabio_do_dia(p_dia date)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `calcular_pontos_perdidos_com_tolerancia(p_professor_id integer, p_unidade_id uuid, p_criterio_id integer, p_competencia character varying)` | ORFA | INVOKER | sem consumidor conhecido |
...
| `configurar_health_score_professor_v3_cron_ciclo_escopos()` | SO-INTERNA | DEFINER | funcao:executar_health_score_professor_v3_job_ciclo_escopo |
...
| `configurar_relatorio_coordenacao_documento_v4_cron()` | ORFA | DEFINER | sem consumidor conhecido |
...
| `escola_agenda_v1(p_de date, p_ate date, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:radar_enfileirar_pauta_v1 |
| `executar_health_score_professor_v3_cron_diario()` | ORFA | DEFINER | sem consumidor conhecido |
| `executar_health_score_professor_v3_escopo_diario(p_competencia date, p_periodicidade text, p_escopo text, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:executar_health_score_professor_v3_cron_diario, funcao:executar_health_score_professor_v3_job_ciclo_escopo, funcao:executar_health_score_professor_v3_job_escopo, funcao:executar_relatorio_coordenacao_batch_diario_v4 |
| `executar_health_score_professor_v3_job_ciclo_escopo(text, uuid)` | ATIVA | DEFINER | cron:materializar-health-score-professor-v3-ciclo-consolidado, cron:materializar-health-score-professor-v3-ciclo-unidade-2ec861f6-023f-4d7b-9927-3960ad8c2a92, cron:materializar-health-score-professor-v3-ciclo-unidade-368d47f5-2d88-4475-bc14-ba084a9a348e, cron:materializar-health-score-professor-v3-ciclo-unidade-95553e96-971b-4590-a6eb-0201d013c14d, funcao:configurar_health_score_professor_v3_cron_ciclo_escopos |
...
| `executar_relatorio_coordenacao_batch_diario_v4()` | ATIVA | DEFINER | cron:relatorio-coordenacao-v4-batch-diario, funcao:configurar_relatorio_coordenacao_documento_v4_cron |
| `executar_relatorio_coordenacao_documento_v4_diario(p_escopo text, p_unidade_id uuid, p_periodicidade text)` | ATIVA | DEFINER | cron:relatorio-coordenacao-v4-consolidado-ciclo (inativo), cron:relatorio-coordenacao-v4-consolidado-mensal (inativo), cron:relatorio-coordenacao-v4-unidade-2ec861f6-023f-4d7b-9927-3960ad8c2a92-ciclo (inativo), cron:relatorio-coordenacao-v4-unidade-2ec861f6-023f-4d7b-9927-3960ad8c2a92-mensal (inativo), cron:relatorio-coordenacao-v4-unidade-368d47f5-2d88-4475-bc14-ba084a9a348e-ciclo (inativo), cron:relatorio-coordenacao-v4-unidade-368d47f5-2d88-4475-bc14-ba084a9a348e-mensal (inativo), +2 outros |
...
| `fabio_agenda_periodo(p_professor_id integer, p_inicio date, p_fim date)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_alunos_visiveis(p_professor_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_anamnese_do_aluno(p_aluno_id integer, p_professor_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_arquivar_limpeza_parqueado_bloqueada(p_id uuid, p_lease_token uuid)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_aulas_ja_registradas(p_professor_id integer, p_dias integer, p_referencia timestamp with time zone)` | SO-INTERNA | DEFINER | funcao:fn_fabio_det_audio_recusado |
...
| `fabio_briefing_matinal(p_professor_id integer, p_data date, p_com_desde_ontem boolean)` | SO-INTERNA | DEFINER | funcao:fn_onde_parou_candidatas |
...
| `fabio_claim_parqueados_limpeza(p_limite integer, p_lease_segundos integer, p_retencao_dias integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_claim_registro_recibo(p_limite integer, p_professor_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_complementar_registro_aula(p_audio_id uuid, p_payload jsonb)` | SO-INTERNA | DEFINER | funcao:fabio_criar_registro |
| `fabio_concluir_limpeza(p_acao_id uuid, p_lease_token uuid, p_resultado jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_concluir_limpeza_parqueado(p_id uuid, p_lease_token uuid)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_corrigir_registro_confirmado(p_professor_id integer, p_registro_id uuid, p_campos jsonb, p_motivo text, p_canal text, p_acao_id text, p_autor_usuario_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_criar_registro(p_payload jsonb)` | SO-INTERNA | DEFINER | funcao:fabio_complementar_registro_aula |
...
| `fabio_lancar_presenca_professor(p_professor_id integer, p_aula_emusys_id integer, p_aluno_id integer, p_status_presenca text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_memoria_do_professor(p_professor_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_memoria_gravar(p_professor_id integer, p_tipo text, p_chave text, p_valor text, p_evidencia text, p_origem text, p_confianca text, p_horizonte text, p_janela_dia date)` | SO-INTERNA | DEFINER | funcao:fabio_memoria_propor |
| `fabio_memoria_janela_marcar(p_origem text, p_professor_id integer, p_janela_dia date, p_mensagens bigint, p_propostas integer, p_gravadas integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_memoria_propor(p_professor_id integer, p_tipo text, p_chave text, p_valor text, p_evidencia text, p_texto_janela text, p_janela_dia date, p_proposto_por text, p_dryrun boolean)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_memoria_revogar(p_professor_id integer, p_tipo text, p_chave text, p_motivo text)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_novidades_do_professor(p_professor_id integer, p_dias integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_novidades_montar(p_novidades jsonb, p_professor_id integer, p_desde timestamp with time zone)` | SO-INTERNA | INVOKER | funcao:fabio_novidades_do_professor |
| `fabio_onde_parou_claim(p_lote integer)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_onde_parou_devolver(p_id bigint, p_lease uuid)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_onde_parou_do_aluno(p_professor_id integer, p_aluno_id integer)` | SO-INTERNA | DEFINER | funcao:app_onde_parou |
| `fabio_onde_parou_falhou(p_id bigint, p_lease uuid, p_erro text, p_espera interval)` | ORFA | DEFINER | sem consumidor conhecido |
| `fabio_onde_parou_gerado(p_id bigint, p_lease uuid, p_cartao jsonb)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_professor_resolver(p_busca text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_agenda_dia_legado_envelope_v1(p_data date, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:get_agenda_dia_v2_sem_cache_20260924 |
...
| `fn_complementar_gaveta(p_velho text, p_novo text, p_campo text)` | SO-INTERNA | INVOKER | funcao:fabio_complementar_registro_aula |
...
| `fn_fabio_correcao_direcao(p_antes jsonb, p_depois jsonb, p_transcricao text, p_audio_id uuid)` | SO-INTERNA | DEFINER | funcao:fn_fabio_diario_coletar, funcao:fn_fabio_identidade_apurar |
| `fn_fabio_correcao_ligar(p_codigo text, p_ocorrencias uuid[], p_por text, p_motivo text)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_correcao_registrar(p_codigo text, p_titulo text, p_mecanismo text, p_prova text, p_referencia text, p_corrigido_em timestamp with time zone, p_sinal_tipo text, p_sinal text, p_sinal_classes text[], p_quem_sofreu text, p_quando text, p_citacao text)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_correcoes_para_triagem()` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_fabio_e_flexao(p_a text, p_b text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_grafia_divergente |
| `fn_fabio_expirar_acoes()` | ATIVA | DEFINER | cron:fabio-expirar-acoes-pendentes |
...
| `fn_fabio_grafia_divergente(p_termo text, p_transcricao text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_vocabulario_grafia_suspeita |
| `fn_fabio_herdar_triagem_da_janela(p_por text, p_janela interval)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_fabio_incidente_de(p_tipo text, p_referencia text, p_bruto jsonb, p_dia date, p_criado_em timestamp with time zone, p_professor_id integer)` | SO-INTERNA | DEFINER | funcao:fn_fabio_ocorrencia_incidente_trg |
| `fn_fabio_janelas_por_extrair_memoria(p_limite integer, p_ignorar_professores integer[])` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_janelas_por_minerar(p_limite integer)` | SO-INTERNA | DEFINER | funcao:fn_fabio_laudo, funcao:fn_fabio_laudo_gravar |
| `fn_fabio_known_issue_ativo(p_assinatura text)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_laudo(p_dia date, p_auditoria jsonb)` | SO-INTERNA | DEFINER | funcao:fn_fabio_laudo_gravar |
| `fn_fabio_laudo_gravar(p_dia date)` | ATIVA | DEFINER | cron:fabio-laudo |
| `fn_fabio_leitura_superficie()` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_licao_gravar(p_nome text, p_texto text, p_por text, p_porque text)` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_linhas_do_professor(p_texto text)` | SO-INTERNA | INVOKER | funcao:fabio_memoria_propor |
| `fn_fabio_memoria_vocabulario()` | SO-INTERNA | INVOKER | funcao:fabio_memoria_propor |
| `fn_fabio_mensagem_e_de_teste(p_professor_id integer, p_role text, p_wa_message_id text, p_criado_em timestamp with time zone)` | SO-INTERNA | DEFINER | funcao:fn_fabio_janelas_por_minerar, funcao:fn_fabio_texto_da_janela |
| `fn_fabio_normalizar_termo(p_bruto text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_identidade_apurar, funcao:fn_fabio_vocabulario_stt |
| `fn_fabio_ocorrencia_classe(p_id uuid)` | SO-INTERNA | DEFINER | funcao:fn_fabio_laudo, funcao:fn_fabio_triar_por_classe |
| `fn_fabio_ocorrencia_detalhe(p_id uuid)` | SO-INTERNA | DEFINER | funcao:fn_fabio_laudo |
| `fn_fabio_ocorrencia_incidente_trg()` | ATIVA | DEFINER | trigger:fabio_diario_ocorrencia.trg_fabio_ocorrencia_incidente |
| `fn_fabio_palavras_na_transcricao(p_palavras text[], p_transcricao text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_correcao_direcao |
| `fn_fabio_palavras_tiradas(p_antes jsonb, p_depois jsonb)` | SO-INTERNA | INVOKER | funcao:fn_fabio_correcao_direcao |
...
| `fn_fabio_relatar_confirmacao_falsa(p_registro_id uuid, p_veredicto text)` | SO-INTERNA | DEFINER | funcao:fn_fabio_superficie_aberta |
| `fn_fabio_relato_autor(p_proposta_id uuid)` | SO-INTERNA | DEFINER | funcao:fn_fabio_laudo, funcao:fn_fabio_ocorrencia_detalhe, funcao:fn_fabio_relatos_para_triagem |
| `fn_fabio_relato_promover()` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_relatos_para_triagem(p_limite integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_fabio_retry_fila()` | ATIVA | DEFINER | front:src/hooks/useSaudeCrons.ts, cron:fabio-retry-fila, funcao:fn_enfileirar_audio_core, funcao:get_saude_syncs_emusys |
...
| `fn_fabio_sonda_julgar(p_resposta text, p_controle jsonb)` | SO-INTERNA | INVOKER | funcao:fn_fabio_sonda_registrar |
...
| `fn_fabio_superficie_aberta()` | ORFA | DEFINER | sem consumidor conhecido |
| `fn_fabio_texto_afirma_ausencia(p_texto text)` | SO-INTERNA | INVOKER | funcao:fabio_marcar_audio_erro_terminal, funcao:fn_fabio_audio_recusa_causa |
...
| `fn_fabio_tipo_em_portugues(p_tipo text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_laudo |
| `fn_fabio_tipo_legado(p_tipo text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_laudo |
...
| `fn_fabio_trecho_na_transcricao(p_trecho text, p_transcricao text)` | SO-INTERNA | INVOKER | funcao:fn_fabio_correcao_direcao |
| `fn_fabio_triagem_decidir(p_incidente_em timestamp with time zone, p_confianca text, p_corrigido_em timestamp with time zone, p_status text, p_margem interval)` | SO-INTERNA | INVOKER | funcao:fn_fabio_triar |
| `fn_fabio_triar(p_ocorrencia uuid, p_codigo text, p_confianca numeric, p_motivo text, p_por text)` | SO-INTERNA | DEFINER | funcao:fn_fabio_correcao_ligar, funcao:fn_fabio_herdar_triagem_da_janela, funcao:fn_fabio_triar_por_classe |
| `fn_fabio_triar_por_classe()` | SO-INTERNA | DEFINER | funcao:fn_fabio_diario_fechar |
...
| `fn_gaveta_da_turma_que_falta(p_do_aluno text, p_da_turma text)` | SO-INTERNA | INVOKER | funcao:fn_compor_texto_prontuario |
| `fn_gavetas_da_aula(p_registro_id uuid)` | SO-INTERNA | INVOKER | funcao:app_historico_turma, funcao:fn_texto_emusys_como_no_app |
| `fn_gavetas_da_turma(p_tronco jsonb)` | SO-INTERNA | INVOKER | funcao:fn_gavetas_da_aula |
| `fn_gravar_fatias_que_a_presenca_liberou(p_registro_id uuid, p_modo text)` | SO-INTERNA | DEFINER | funcao:fn_confirmar_registro_core |
...
| `fn_health_score_professor_v3_competencia_ciclo_vivo(p_competencia date, p_data_corte date)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_performance, funcao:get_health_score_professor_v3_projecao_viva, funcao:montar_rel_coord_before_confiabilidade_20260909, funcao:montar_rel_coord_conteudo_before_carteira_painel_20260909, funcao:montar_rel_coord_conteudo_before_espelho_integral_20260909, funcao:montar_rel_coord_conteudo_before_release_20260909 |
...
| `fn_health_score_professor_v3_config_fingerprint_comparabilidade(p_config_id uuid)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_performance_snapshot_v1, funcao:get_hs_prof_v3_performance_before_ciclo_parity_20260909, funcao:get_hs_prof_v3_performance_before_scope_fix_20260804, funcao:get_hs_prof_v3_performance_payload_base_20260803 |
...
| `fn_health_score_v3_periodo(p_competencia date, p_periodicidade text)` | SO-INTERNA | INVOKER | funcao:fn_health_score_professor_v3_competencia_ciclo_vivo, funcao:fn_relatorio_coordenacao_recortes_diarios_v4, funcao:get_health_score_prof_v3_metricas_base_20260728_c95, funcao:get_health_score_professor_v3_carteira_periodo, funcao:get_health_score_professor_v3_conversao_base_v2, funcao:get_health_score_professor_v3_conversao_periodo_canonico, +18 outros |
...
| `fn_participacao_supersede_coerente()` | ATIVA | INVOKER | trigger:fabio_participacao_ocorrencias.trg_participacao_supersede_coerente |
...
| `fn_presenca_candidatos_periodo_v1(p_unidade_id uuid, p_data_inicio date, p_data_fim date)` | SO-INTERNA | DEFINER | funcao:fn_presenca_resumo_mensal_v1 |
...
| `fn_presenca_slot_key_v2(p_aluno_id integer, p_unidade_id uuid, p_professor_id integer, p_data_hora_inicio timestamp with time zone, p_data_hora_fim timestamp with time zone, p_curso_nome text)` | ATIVA | INVOKER | view:vw_presenca_ocorrencia_canonica_v2, funcao:fn_presenca_candidatos_periodo_v1, funcao:fn_presenca_estado_publicacao_periodo_v2, funcao:fn_presenca_ocorrencia_canonica_escopada_v2, funcao:fn_presenca_pendencias_do_dia_v2, funcao:get_presenca_metricas_canonicas_v2, +1 outros |
...
| `fn_presenca_status_efetivo(p_status_presenca text, p_status text)` | ORFA | INVOKER | sem consumidor conhecido |
...
| `get_agenda_dia(p_data date, p_unidade_id uuid)` | ATIVA | INVOKER | front:src/components/App/Agenda/Chamada/ChamadaDia.tsx, funcao:fn_agenda_dia_legado_envelope_v1, funcao:get_agenda_dia_canonica_v2, funcao:sol_porta_agenda_do_dia_v1 |
| `get_agenda_dia_canonica_v2(p_data date, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:get_agenda_dia_v2_sem_cache_20260924 |
| `get_agenda_dia_v2(p_data date, p_unidade_id uuid)` | ATIVA | DEFINER | front:src/hooks/useAgendaDia.ts, funcao:dashboard_aquecer_caches_v1, funcao:get_agenda_semana_v2 |
...
| `get_dados_relatorio_coordenacao_pre_totais_20260711(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | INVOKER | funcao:get_dados_relatorio_coordenacao |
...
| `get_health_score_prof_v3_metricas_base_20260728(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_hs_prof_v3_metricas_periodo_base_20260803 |
...
| `get_health_score_professor_v3_capacidade_diagnostico(p_competencia date, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_sinais, funcao:get_health_score_professor_v3_sinais_snapshot_v1 |
...
| `get_health_score_professor_v3_conversao_base_v2(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `get_health_score_professor_v3_conversao_periodo_canonico(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_conversao_ciclo, funcao:get_health_score_professor_v3_conversao_mensal |
...
| `get_health_score_professor_v3_performance(p_competencia date, p_unidade_id uuid)` | ATIVA | DEFINER | front:src/hooks/useDashboardDados.ts, funcao:executar_health_score_professor_v3_escopo_diario, funcao:fingerprint_health_score_professor_v3_escopo, funcao:get_dados_relatorio_gerencial, funcao:get_health_score_professor_v3_sinais, funcao:get_hs_prof_v3_snapshot_modal_base_comparabilidade, +8 outros |
| `get_health_score_professor_v3_performance(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | ATIVA | DEFINER | front:src/hooks/useDashboardDados.ts, funcao:executar_health_score_professor_v3_escopo_diario, funcao:fingerprint_health_score_professor_v3_escopo, funcao:get_dados_relatorio_gerencial, funcao:get_health_score_professor_v3_sinais, funcao:get_hs_prof_v3_snapshot_modal_base_comparabilidade, +8 outros |
...
| `get_health_score_professor_v3_sinais_snapshot_v1(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_contexto_v3_v1 |
...
| `get_heatmap_data(p_ano integer, p_metrica character varying)` | ATIVA | INVOKER | front:src/hooks/useSupabase.ts |
| `get_heatmap_totais(p_ano integer, p_metrica character varying)` | ATIVA | INVOKER | front:src/hooks/useSupabase.ts |
...
| `get_hs_prof_v3_conversao_ciclo_base_20260803(p_competencia date, p_unidade_id uuid)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `get_hs_prof_v3_performance_before_ciclo_parity_20260909(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_performance |
...
| `get_hs_prof_v3_performance_before_scope_fix_20260804(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_hs_prof_v3_performance_before_ciclo_parity_20260909 |
...
| `get_hs_prof_v3_projecao_viva_before_ciclo_parity_20260909(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_projecao_viva |
| `get_hs_prof_v3_projecao_viva_before_presence_cycle_fix_20260803(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_hs_prof_v3_projecao_viva_before_ciclo_parity_20260909 |
...
| `get_hs_prof_v3_snapshot_before_evid_corrente_20260909(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:hs_prof_v3_reader_sem_cache_20260924 |
...
| `get_kpis_professor_periodo_canonico_base_20260711(p_ano integer, p_mes integer, p_unidade_id uuid, p_data_inicio date, p_data_fim date)` | SO-INTERNA | INVOKER | funcao:get_kpis_professor_periodo_canonico, funcao:montar_relatorio_coordenacao_payload_v3 |
...
| `get_professores_briefing_anamnese(p_anamnese_id integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `get_programa_matriculador_dados(p_ano integer, p_unidade_id uuid)` | ATIVA | INVOKER | front:src/hooks/useMatriculadorPrograma.ts, funcao:gravar_snapshot_fechamento_mensal, funcao:preview_fechamento_mensal |
...
| `grade_norm_curso(p text)` | SO-INTERNA | INVOKER | funcao:sincronizar_grade_horaria_alunos |
| `grade_norm_nome(p text)` | SO-INTERNA | INVOKER | funcao:sincronizar_grade_horaria_alunos |
| `hs_prof_v3_reader_sem_cache_20260924(p_competencia date, p_unidade_id uuid, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_performance_snapshot_v3 |
...
| `materializar_health_score_professor_v3_escopo_diario(p_competencia date, p_periodicidade text, p_escopo text, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:executar_health_score_professor_v3_escopo_diario, funcao:executar_relatorio_coordenacao_batch_diario_v4 |
| `materializar_health_score_professor_v3_periodo(p_competencia date, p_periodicidade text, p_unidade_id uuid, p_professor_id integer)` | SO-INTERNA | DEFINER | funcao:materializar_health_score_professor_v3_rede, funcao:materializar_hs_prof_v3_escopo_before_lock_order_20260813, funcao:reprocessar_health_score_professor_v3_competencia_aberta, funcao:retificar_coordenacao_jun_ago_2026 |
...
| `montar_rel_coord_conteudo_before_carteira_painel_20260909(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_rel_coord_conteudo_before_espelho_integral_20260909 |
| `montar_rel_coord_conteudo_before_ciclo_vivo_20260909(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_rel_coord_conteudo_before_carteira_painel_20260909 |
| `montar_rel_coord_conteudo_before_competencia_painel_20260909(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_rel_coord_conteudo_before_release_20260909 |
| `montar_rel_coord_conteudo_before_espelho_integral_20260909(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_rel_coord_conteudo_before_competencia_painel_20260909 |
| `montar_rel_coord_conteudo_before_release_20260909(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_rel_coord_conteudo_before_amostra_exata_20260909 |
| `montar_relatorio_coordenacao_conteudo_base_v4(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_rel_coord_conteudo_before_ciclo_vivo_20260909 |
| `montar_relatorio_coordenacao_conteudo_v4(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:materializar_rel_coord_doc_before_finalidade_20260909 |
| `montar_relatorio_coordenacao_contexto_v3_v1(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_payload_v3 |
...
| `montar_relatorio_coordenacao_payload_v3(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text)` | SO-INTERNA | DEFINER | funcao:montar_relatorio_coordenacao_conteudo_base_v4, funcao:relatorio_coordenacao_carteira_v4 |
...
| `reconciliar_health_score_professor_v3_alertas()` | ATIVA | DEFINER | cron:reconciliar-health-score-professor-v3-alertas, funcao:configurar_health_score_professor_v3_cron_escopos, funcao:executar_health_score_professor_v3_job_ciclo_escopo, funcao:executar_health_score_professor_v3_job_escopo |
...
| `registrar_aula_fabio(p_aula_id integer, p_texto text, p_origem text, p_professor_id integer, p_modo text)` | SO-INTERNA | DEFINER | funcao:fabio_corrigir_registro_confirmado, funcao:fn_confirmar_registro_core, funcao:fn_gravar_fatias_que_a_presenca_liberou |
...
| `regularizar_coordenacao_jun_ago_2026_d30(p_justificativa text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `responder_passagem_bastao(p_id uuid, p_texto text, p_audio_url text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `retificar_coordenacao_jun_ago_2026()` | ORFA | DEFINER | sem consumidor conhecido |
...
| `retirar_do_roster_health_score_v3_ciclo(p_ciclo_codigo text, p_professor_id integer, p_unidade_id uuid, p_motivo text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `sol_porta_agenda_do_dia_v1(p_solicitante_telefone text, p_unidade text, p_data date)` | SO-INTERNA | DEFINER | funcao:sol_governanca_snapshot_operacional_v1 |
| `sol_porta_pendencias_cadastro_v1(p_solicitante_telefone text, p_amostra integer)` | SO-INTERNA | DEFINER | funcao:sol_governanca_snapshot_operacional_v1 |
| `sol_porta_presenca_pendente_v1(p_solicitante_telefone text, p_unidade text, p_data date)` | SO-INTERNA | DEFINER | funcao:sol_governanca_snapshot_operacional_v1 |
...
| `trg_professor_presente_quando_aluno_presente()` | ATIVA | INVOKER | trigger:aluno_presenca.trg_professor_presente_quando_aluno_presente |
