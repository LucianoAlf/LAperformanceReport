...
1151 funções. `ORFA` é **sinal, não veredito**: n8n, scripts da VPS e
...
| `aviso_previo_pendencias(p_unidade_id uuid, p_ref date)` | SO-INTERNA | INVOKER | funcao:aviso_previo_vencidos |
...
| `banda_roster_turma(p_unidade_id uuid, p_turma_nome text)` | ATIVA | DEFINER | front:src/hooks/useBandas.ts, funcao:banda_conciliacao_roster, funcao:banda_detalhe, funcao:banda_integrantes, funcao:banda_turmas_a_confirmar, funcao:bandas_kpis, funcao:bandas_listar |
...
| `fn_alunos_vinculo_emusys_anamnese()` | ATIVA | INVOKER · 🔓 anon | trigger:alunos.trg_alunos_vinculo_emusys_anamnese |
...
| `fn_atualizar_aluno_anamnese()` | ATIVA | INVOKER · 🔓 anon | trigger:anamneses.trg_anamnese_atualiza_aluno |
...
| `fn_pesquisa_evasao_usuario_interno_ativo()` | SO-INTERNA | DEFINER | funcao:cancelar_repescagem_evasao, funcao:classificar_resposta_evasao, funcao:concluir_acao_pesquisa_evasao_v1, funcao:contar_followups_pesquisa_evasao_grupos_v1, funcao:contar_followups_pesquisa_evasao_v1, funcao:enfileirar_repescagem_evasao, funcao:fn_pesquisa_evasao_followup_estado, funcao:get_conversa_pesquisa_evasao, funcao:listar_evadidos_para_pesquisa, funcao:listar_evadidos_para_pesquisa_v2, funcao:listar_followups_pesquisa_evasao_v1, funcao:listar_lia_alertas_pendencias_administrativas, funcao:listar_pesquisas_evasao_revisao, funcao:listar_pesquisas_evasao_teste_v1, funcao:listar_respostas_evasao_analytics_v1, funcao:obter_dados_classificacao_pesquisa_evasao_v1, funcao:pode_enviar_pesquisa_evasao, funcao:registrar_acao_pesquisa_evasao_v1, funcao:registrar_classificacao_pesquisa_evasao_v1, funcao:registrar_desfecho_pesquisa_evasao_v1, funcao:registrar_followup_pesquisa_evasao_v1, funcao:stats_pesquisa_evasao |
...
| `fn_vincular_anamnese_pendente()` | ATIVA | INVOKER · 🔓 anon | trigger:alunos.trg_vincular_anamnese_na_matricula |
...
| `get_kpis_alunos_canonicos_base_v131(p_unidade_id uuid, p_ano integer, p_mes integer, p_admin jsonb)` | SO-INTERNA | DEFINER | funcao:get_kpis_alunos_canonicos |
...
| `is_movimentacao_admin_retencao_valida(p_movimentacao_id integer)` | ATIVA | DEFINER | edge:supabase/functions/enviar-pesquisa-evasao/index.ts, view:vw_alertas_inteligentes, view:vw_dashboard_unidade, view:vw_evasoes_motivos, view:vw_evasoes_professores, view:vw_evasoes_resumo, view:vw_kpis_gestao_mensal, view:vw_kpis_professor_mensal, view:vw_professor_periodos_baseline_v3_sombra, view:vw_professores_performance_atual, view:vw_taxa_crescimento_professor, funcao:criar_pesquisa_evasao, funcao:filtrar_renovacoes_admin_retencao_validas_v1, funcao:get_dados_relatorio_coordenacao_legado_20260711, funcao:get_dados_retencao_ia, funcao:get_dados_retencao_ia_legacy_p01g, funcao:get_kpis_professor_periodo_base_legado_20260713, funcao:get_saidas_professor_periodo_agregadas_v1, funcao:get_saidas_professor_periodo_canonicas_v1, funcao:get_saidas_professor_periodo_detalhes_v1, funcao:listar_evadidos_para_pesquisa, funcao:listar_evadidos_para_pesquisa_v2, funcao:montar_relatorio_coordenacao_payload_v2, funcao:montar_relatorio_coordenacao_payload_v3, funcao:pode_enviar_pesquisa_evasao, funcao:snapshot_dados_mensais_unguarded, funcao:stats_pesquisa_evasao, funcao:sync_evasao_to_dados_mensais |
...
| `get_faturas_alunos_financeiro_v1_canonica_20260817(p_unidade_id uuid, p_ano integer, p_mes integer, p_modo_periodo text, p_status text, p_as_of_date date)` | SO-INTERNA | DEFINER | funcao:get_faturas_alunos_financeiro_v1_contrato_20260817 |
...
| `get_financeiro_faturas_emusys(p_unidade_id uuid, p_ano integer, p_mes integer)` | ATIVA | DEFINER | front:src/components/App/Administrativo/ModalRelatorio.tsx, front:src/lib/financeiroFaturasEmusys.ts, funcao:aplicar_retificacao_relatorio_gerencial_financeiro_v1, funcao:get_dados_relatorio_gerencial_legacy_rankings_p24_20260719 |
| `get_inadimplencia_canonica(p_unidade_id uuid, p_as_of_date date)` | ATIVA | DEFINER | front:src/components/App/Administrativo/PainelFarmer/hooks/useAlertas.ts, front:src/components/App/Alunos/AlunosPage.tsx, front:src/lib/faturasAlunosCanonicas.ts, edge:supabase/functions/atualizar-inadimplencia-emusys/index.ts, edge:supabase/functions/export-contas-receber/index.ts, funcao:get_faturas_alunos_financeiro_v1_base, funcao:get_faturas_alunos_financeiro_v1_canonica_20260817, funcao:get_faturas_alunos_financeiro_v1_reconciliacao_base, funcao:get_financeiro_faturas_emusys, funcao:sol_caixa_inadimplentes, funcao:sol_inadimplencia_v1 |
...
| `get_kpis_alunos_financeiro_vivo_canonico(p_unidade_id uuid, p_ano integer, p_mes integer)` | ATIVA | DEFINER | edge:supabase/functions/bi-agent-lamusic/tools.ts, funcao:get_kpis_alunos_canonicos_base_p01t |
...
| `sol_caixa_lancar_recebimento(p_payload jsonb)` | SO-INTERNA | DEFINER | funcao:sol_caixa_readonly_preflight_v2, funcao:sol_caixa_readonly_preflight_v3 |
...
| `capturar_relatorios_mensais_canonicos_v1(p_ano integer, p_mes integer, p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:diagnosticar_captura_fechamento_mensal_v1 |
...
| `fechar_competencia(p_unidade_id uuid, p_ano integer, p_mes integer, p_fechado_por text, p_motivo text, p_lote_id uuid)` | SO-INTERNA | DEFINER | funcao:fechar_competencia_mensal_canonica_v1, funcao:fechar_relatorio_mensal_canonico_unidade_v1 |
| `fechar_competencia_mensal_automatico()` | ATIVA | DEFINER | cron:fechamento-mensal-automatico |
| `fechar_competencia_mensal_canonica_v1(p_ano integer, p_mes integer, p_motivo text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_competencia_feedback(p_dia date)` | ATIVA | INVOKER | view:vw_radar_aluno_sinais_canonica_v2, view:vw_radar_aluno_sinais_legado_v1, funcao:app_coordenacao_feedback_mes, funcao:app_professor_feedback_mesa, funcao:app_professor_feedback_progresso, funcao:app_professor_feedback_salvar, funcao:fn_feedback_cobranca_do_dia |
...
| `get_kpis_comercial_canonicos_v2(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodo text, p_data date)` | ATIVA | DEFINER | front:src/hooks/useComercialOperacionalResumoV2.ts, front:src/hooks/useComercialResumoV2.ts, front:src/hooks/useComercialSeriesMensaisV2.ts, front:src/hooks/useCursosData.ts, front:src/hooks/useMatriculadorPrograma.ts, front:src/hooks/useOrigemData.ts, edge:supabase/functions/_shared/relatorio-comercial.test.ts, edge:supabase/functions/_shared/relatorio-publico.test.ts, edge:supabase/functions/gemini-insights-comercial/index.ts, edge:supabase/functions/relatorio-admin-whatsapp/index.ts, funcao:get_dados_relatorio_gerencial_legacy_p16_20260706, funcao:gravar_snapshot_fechamento_mensal, funcao:preview_fechamento_mensal |
...
| `get_relatorio_gerencial_ranking_mensal_v1(p_unidade_id uuid, p_ano integer, p_mes integer)` | SO-INTERNA | DEFINER | funcao:get_relatorio_gerencial_canonico_comparativos_base_v1 |
...
| `get_base_conhecimento(p_unidade_id uuid)` | ATIVA | DEFINER | front:src/components/App/PreAtendimento/hooks/useBaseConhecimento.ts, edge:supabase/functions/base-conhecimento/index.ts |
...
| `sync_aluno_to_leads()` | ATIVA | INVOKER · 🔓 anon | trigger:alunos.trigger_sync_aluno_to_leads |
...
| `is_atividade_extra_curso(p_curso_id integer)` | ATIVA | INVOKER · 🔓 anon | view:vw_alunos_sem_fatura_mes, view:vw_kpis_retencao_mensal, view:vw_renovacao_ciclos, funcao:get_relatorio_admin_mensal_rico_base_v1, funcao:montar_relatorio_admin_mensal_payload_base_v4, funcao:movimentacao_conta_nos_kpis_v1, funcao:preencher_campos_retencao_movimentacoes_admin |
...
| `fn_atualizar_fatia_core(p_professor_id integer, p_id uuid, p_texto text, p_campos jsonb)` | SO-INTERNA | DEFINER | funcao:app_atualizar_fatia, funcao:fabio_atualizar_fatia |
| `fn_audit_log()` | ATIVA | DEFINER | trigger:alunos.trg_audit, trigger:caixa_movimentacoes.trg_audit_caixa_movimentacoes, trigger:config_health_score_professor.trg_audit, trigger:crm_pipeline_etapas.trg_audit, trigger:cursos.trg_audit, trigger:dados_mensais.trg_audit, trigger:lead_experimentais.trg_audit, trigger:leads.trg_audit, trigger:loja_produtos.trg_audit, trigger:metas_kpi.trg_audit, trigger:metas.trg_audit, trigger:movimentacoes_admin.trg_audit, trigger:professor_360_avaliacoes.trg_audit, trigger:professor_360_ocorrencias.trg_audit, trigger:professor_acoes.trg_audit, trigger:professores.trg_audit, trigger:projeto_tarefas.trg_audit, trigger:projetos.trg_audit, trigger:renovacoes_legado.trg_audit, trigger:salas.trg_audit, trigger:turmas_alunos.trg_audit, trigger:turmas_explicitas.trg_audit, trigger:unidades.trg_audit |
...
| `fn_hoje_brt()` | SO-INTERNA | INVOKER | funcao:app_coordenacao_feedback_mes, funcao:app_professor_feedback_mesa, funcao:app_professor_feedback_progresso, funcao:fn_competencia_feedback, funcao:fn_feedback_cobranca_do_dia, funcao:fn_janela_feedback_aberta, funcao:fn_reservar_cobranca_feedback, funcao:fn_reservar_cobranca_feedback_coordenacao |
...
| `fn_janela_registro_dias()` | SO-INTERNA | INVOKER · 🔓 anon | funcao:app_abrir_rascunho_manual, funcao:app_enfileirar_audio_experimental, funcao:fabio_aulas_candidatas, funcao:fn_aplicar_comando_presenca_core_v2, funcao:fn_enfileirar_audio_core, funcao:fn_pendencias_escalonadas, funcao:fn_registrar_presencas_core |
...
| `get_user_unidade_ids()` | ATIVA | DEFINER · 🔓 anon | view:vw_absenteismo_aluno_canonica_v2, view:vw_alunos_sem_fatura_mes, view:vw_contratos_vencendo, view:vw_radar_aluno_sinais_canonica_v2, view:vw_renovacao_ciclos, funcao:decidir_professor_divergencia_emusys, funcao:fn_absenteismo_aluno_rollout_v1, funcao:fn_presenca_estado_publicacao_periodo_v2, funcao:fn_presenca_pendencias_do_dia_v2, funcao:fn_radar_aluno_sinais_rollout_v1, funcao:get_agenda_dia_v2, funcao:get_faltas_periodo_canonico_v2, funcao:get_faturas_alunos_financeiro_v1_base, funcao:get_faturas_alunos_financeiro_v1_canonica_20260817, funcao:get_faturas_alunos_financeiro_v1_reconciliacao_base, funcao:get_financeiro_faturas_emusys, funcao:get_frequencia_professor_periodo_canonica_v1, funcao:get_inadimplencia_canonica_v3_base, funcao:get_presenca_contexto_agente_canonico_v1, funcao:get_presenca_experimental_aluno_periodo_v1, funcao:get_presenca_ocorrencias_periodo_canonico_v2, funcao:get_presenca_ocorrencias_periodo_legado_v1, funcao:get_professores_divergencias_emusys, funcao:resolver_reconciliacao_fatura |
...
| `hash_jsonb_canonico(p_payload jsonb)` | SO-INTERNA | INVOKER | funcao:aplicar_retificacao_relatorio_admin_mensal_renovacoes_v1, funcao:aplicar_retificacao_relatorio_comercial_matricula_tardia_v1, funcao:aplicar_retificacao_relatorio_comercial_mensal_v1, funcao:aplicar_retificacao_relatorio_gerencial_financeiro_v1, funcao:aplicar_retificacao_relatorio_gerencial_retencao_v1, funcao:capturar_relatorio_coordenacao_canonico_v2, funcao:capturar_relatorios_mensais_canonicos_v1, funcao:get_comparativo_fechamento_mensal_v1, funcao:get_relatorio_admin_mensal_rico_base_v1, funcao:get_relatorio_admin_mensal_rico_v1, funcao:get_relatorio_coordenacao_canonico_v2, funcao:get_relatorio_mensal_canonico_v1, funcao:get_relatorio_mensal_snapshot_base_v1, funcao:gravar_snapshot_fechamento_mensal, funcao:montar_relatorio_admin_mensal_payload_base_v1, funcao:montar_relatorio_admin_mensal_payload_base_v2_legacy_20260811, funcao:montar_relatorio_comercial_mensal_payload_sem_adicionais_v1 |
...
| `is_admin()` | ATIVA | DEFINER · 🔓 anon | front:src/components/App/Agenda/Chamada/ChamadaDrawer.tsx, view:vw_absenteismo_aluno_canonica_v2, view:vw_alunos_sem_fatura_mes, view:vw_contratos_vencendo, view:vw_disponibilidade_professores, view:vw_radar_aluno_sinais_canonica_v2, view:vw_renovacao_ciclos, funcao:admin_alterar_presenca_rollout_v1, funcao:app_cancelar_aula, funcao:decidir_professor_divergencia_emusys, funcao:fn_absenteismo_aluno_rollout_v1, funcao:fn_presenca_estado_publicacao_periodo_v2, funcao:fn_presenca_pendencias_do_dia_v2, funcao:fn_radar_aluno_sinais_rollout_v1, funcao:get_agenda_dia_canonica_v2, funcao:get_agenda_dia_v2, funcao:get_faltas_periodo_canonico_v2, funcao:get_faturas_alunos_financeiro_v1_base, funcao:get_faturas_alunos_financeiro_v1_canonica_20260817, funcao:get_faturas_alunos_financeiro_v1_reconciliacao_base, funcao:get_financeiro_faturas_emusys, funcao:get_frequencia_professor_periodo_canonica_v1, funcao:get_inadimplencia_canonica_v3_base, funcao:get_presenca_contexto_agente_canonico_v1, funcao:get_presenca_experimental_aluno_periodo_v1, funcao:get_presenca_ocorrencias_periodo_canonico_v2, funcao:get_presenca_ocorrencias_periodo_legado_v1, funcao:get_professores_divergencias_emusys, funcao:resolver_reconciliacao_fatura |
...
| `set_updated_at()` | ATIVA | INVOKER · 🔓 anon | trigger:agente_conversas.set_updated_at_agente_conversas, trigger:agentes.set_updated_at_agentes, trigger:alunos_emusys_atributos_divergencias.set_updated_at_alunos_emusys_atributos_divergencias, trigger:bi_ai_query_playbooks.trg_bi_ai_query_playbooks_updated_at, trigger:bi_conversations_lamusic.trg_bi_conversations_updated_at, trigger:bi_messages_lamusic.trg_bi_messages_updated_at, trigger:campanhas.set_updated_at_campanhas, trigger:conversas_campanha.set_updated_at_conversas_campanha, trigger:disponibilidade_professor_propostas.set_updated_at_disponibilidade_professor_propostas, trigger:numeros_meta.set_updated_at_numeros_meta, trigger:professores_emusys_divergencias.set_updated_at_professores_emusys_divergencias, trigger:professores_unidades.set_updated_at_professores_unidades, trigger:relatorios_pedagogicos.trg_relped_updated_at, trigger:templates_meta.set_updated_at_templates_meta, trigger:vcards_unidade.trg_vcards_unidade_updated_at |
...
| `unaccent_imutavel(t text)` | SO-INTERNA | INVOKER · 🔓 anon | funcao:ficha_criar_pessoa, funcao:ficha_emitir_token |
...
| `update_updated_at_column()` | ATIVA | INVOKER · 🔓 anon | trigger:aluno_jornada_matricula_disciplina.trg_aluno_jornada_matricula_disciplina_updated_at, trigger:alunos_historico.update_alunos_historico_updated_at, trigger:base_conhecimento_blocos.trg_base_conhecimento_blocos_updated_at, trigger:colaboradores.update_colaboradores_updated_at, trigger:conversa_estado_whatsapp.tr_updated_at_conversa_estado, trigger:cursos.trg_cursos_updated_at, trigger:emusys_experimentais_raw.trg_emusys_experimentais_raw_updated_at, trigger:evasoes_v2.set_updated_at_evasoes_v2, trigger:integracao_tokens.trg_integracao_tokens_updated_at, trigger:leads.update_leads_updated_at, trigger:loja_carteira.update_loja_carteira_updated_at, trigger:loja_configuracoes.update_loja_configuracoes_updated_at, trigger:loja_estoque.update_loja_estoque_updated_at, trigger:loja_produtos.update_loja_produtos_updated_at, trigger:metas.update_metas_updated_at, trigger:movimentacoes.update_movimentacoes_updated_at, trigger:notificacao_config.update_notificacao_config_updated_at, trigger:pesquisa_evasao.tr_updated_at_pesquisa_evasao, trigger:professores.trg_professores_updated_at, trigger:projeto_comentarios.update_comentarios_updated_at, trigger:projeto_fases.update_projeto_fases_updated_at, trigger:projeto_tarefas.update_projeto_tarefas_updated_at, trigger:projeto_tipos.update_projeto_tipos_updated_at, trigger:projetos.update_projetos_updated_at, trigger:renovacoes_legado.update_renovacoes_updated_at, trigger:usuarios.update_usuarios_updated_at |
...
| `usuario_tem_permissao(p_usuario_id integer, p_codigo_permissao character varying, p_unidade_id uuid)` | ATIVA | DEFINER | view:vw_disponibilidade_professores, funcao:admin_confirmar_presencas_aula, funcao:admin_corrigir_presenca, funcao:admin_decidir_proposta_disponibilidade, funcao:admin_efetivar_proposta_disponibilidade, funcao:admin_revisar_presenca_conciliacao, funcao:app_atualizar_lead_campos, funcao:app_cancelar_aula, funcao:app_criar_comando_presenca_v1, funcao:app_falta_professor_cancelar_aulas, funcao:app_marcar_presenca_professor_aula, funcao:app_registrar_chamada_agenda, funcao:app_registrar_presenca_experimental, funcao:app_registrar_presenca_professor_dia, funcao:app_remover_presenca_professor_dia, funcao:fn_health_score_v3_unidades_permitidas_sombra, funcao:fn_usuario_atual_tem_permissao, funcao:get_conciliacao_experimentais_v2, funcao:get_conciliacao_presencas, funcao:get_conciliacao_roster_operacional_v1, funcao:get_kpis_professor_periodo_canonico_v2, funcao:get_kpis_turmas_canonicos_v1, funcao:get_kpis_turmas_canonicos_v2, funcao:get_prof_curso_modalidade_excecoes_v2_pre_nome_emusys, funcao:get_prof_curso_modalidade_excecoes_v2_raw, funcao:get_professor_curso_modalidade_reconciliacao_v1, funcao:get_saidas_professor_periodo_detalhes_v1, funcao:pode_gerar_relatorio_admin_v1, funcao:pode_gerar_relatorio_comercial_v1, funcao:pode_sincronizar_presenca_emusys_v1, funcao:promover_periodos_professor_ativos_exatos_v2, funcao:promover_troca_de_curso_mesmo_professor_v1, funcao:promover_trocas_confirmadas_pela_jornada_v1, funcao:salvar_professor_curso_modalidade_atribuicoes_v1 |
...
| `app_aplicar_comando_presenca_v1(p_request_id uuid)` | SO-INTERNA | DEFINER | funcao:app_marcar_presenca_professor_aula, funcao:app_registrar_chamada_agenda, funcao:app_registrar_presenca_professor_dia, funcao:app_registrar_presencas_aula_publicacao_legado_v1, funcao:app_remover_presenca_professor_dia, funcao:fabio_confirmar_chamada_acao_publicacao_legado_v1, funcao:fabio_emitir_presenca_por_registro_publicacao_legado_v1, funcao:fabio_registrar_presencas_aula |
...
| `app_historico_turma(p_turma_nome text, p_limite integer)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `capturar_relatorio_coordenacao_canonico_v2(p_ano integer, p_mes integer, p_unidade_id uuid, p_motivo text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fabio_acao_json(p_acao_id uuid)` | SO-INTERNA | DEFINER | funcao:fabio_acao_ativa, funcao:fabio_aplicar_evento_acao, funcao:fabio_claim_acoes_processando, funcao:fabio_concluir_limpeza, funcao:fabio_concluir_reconciliacao, funcao:fabio_confirmar_chamada_acao_canonica_v2_interno, funcao:fabio_confirmar_chamada_acao_publicacao_legado_v1, funcao:fabio_iniciar_acao |
...
| `fabio_corrigir_registro_confirmado(p_professor_id integer, p_registro_id uuid, p_campos jsonb, p_motivo text, p_canal text, p_acao_id text)` | ORFA | DEFINER | sem consumidor conhecido |
...
| `fn_aula_individual_do_aluno(p_aula_id integer, p_aluno_id integer)` | SO-INTERNA | DEFINER | funcao:app_abrir_rascunho_manual, funcao:app_registro_completo, funcao:fabio_criar_registro, funcao:fabio_registro_completo, funcao:fn_aula_ja_registrada, funcao:fn_confirmar_registro_core, funcao:fn_enfileirar_audio_core |
| `fn_aula_ja_registrada(p_aula_id integer)` | SO-INTERNA | DEFINER | funcao:app_registro_completo, funcao:fabio_registro_completo |
| `fn_aula_operacional_id(p_aula_id integer)` | ATIVA | DEFINER | view:vw_experimental_pendencia, view:vw_fila_audio_sem_roster, view:vw_presenca_pendencia, view:vw_registro_pendencia, funcao:app_minha_agenda_semana_v1, funcao:app_minha_agenda_sessao_base_v1, funcao:app_minha_agenda_sessao_base_v1_referencia_lenta, funcao:fabio_aulas_candidatas, funcao:fabio_fila_recusas_a_avisar, funcao:fabio_participacao_registrar_candidata, funcao:fabio_professor_presencas_periodo_legado_v1, funcao:fabio_professor_resumo_aulas, funcao:fn_enfileirar_audio_core, funcao:fn_fila_audio_retomar_por_roster |
...
| `fn_e_coordenacao_la_teacher()` | SO-INTERNA | DEFINER | funcao:app_coordenacao_em_aberto, funcao:app_coordenacao_feedback_mes, funcao:app_coordenacao_professor_detalhe, funcao:app_coordenacao_radar, funcao:app_meu_acesso, funcao:app_meu_perfil_coordenacao, funcao:app_professores_para_liberar, funcao:app_radar_config, funcao:app_radar_config_salvar |
...
| `fn_health_score_professor_v3_ator_gerenciador()` | SO-INTERNA | DEFINER | funcao:ativar_health_score_professor_v3_config, funcao:ativar_health_score_professor_v3_config_pre_catalogo_v1, funcao:ativar_health_score_professor_v3_config_revisao_ciclo_aberto, funcao:criar_health_score_professor_v3_config_rascunho, funcao:criar_health_score_professor_v3_config_revisao_ciclo_aberto, funcao:fechar_health_score_professor_v3_ciclo, funcao:fn_health_score_professor_v3_config_ui_competencia, funcao:get_health_score_professor_v3_config_ui_pre_catalogo_v1, funcao:get_health_score_professor_v3_snapshot_ui, funcao:salvar_health_score_professor_v3_config_rascunho, funcao:salvar_health_score_professor_v3_config_rascunho_pre_oferta_for, funcao:salvar_health_score_v3_config_pre_cursos_pedagogicos_v1, funcao:simular_health_score_professor_v3_config, funcao:simular_health_score_professor_v3_config_pre_catalogo_v1 |
| `fn_health_score_professor_v3_ator_leitura(p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:get_health_score_professor_v3_capacidade_diagnostico, funcao:get_health_score_professor_v3_performance, funcao:get_health_score_professor_v3_sinais, funcao:get_health_score_professor_v3_snapshot_modal, funcao:get_hs_prof_v3_performance_base_comp_legacy_20260803, funcao:get_hs_prof_v3_performance_comp_legacy_20260803, funcao:get_hs_prof_v3_snapshot_modal_base_comparabilidade, funcao:get_relatorio_coordenacao_canonico_v1, funcao:get_relatorio_coordenacao_canonico_v1_base_20260802, funcao:get_relatorio_coordenacao_canonico_v2, funcao:get_relatorio_coordenacao_canonico_v3 |
...
| `fn_health_score_v3_periodo(p_competencia date, p_periodicidade text)` | SO-INTERNA | INVOKER | funcao:get_health_score_prof_v3_metricas_base_20260728_c95, funcao:get_health_score_professor_v3_carteira_periodo, funcao:get_health_score_professor_v3_conversao_ciclo, funcao:get_health_score_professor_v3_metricas_periodo_base_20260719, funcao:get_health_score_professor_v3_metricas_segmentadas_agregadas_v1, funcao:get_health_score_professor_v3_numero_alunos_disponibilidade_raw, funcao:get_health_score_professor_v3_performance_base_comparabilidade, funcao:get_health_score_professor_v3_permanencia_periodo_v2, funcao:get_health_score_professor_v3_presenca_periodo_v2, funcao:get_health_score_professor_v3_totais_carteira_canonica_v1, funcao:get_hs_prof_v3_conversao_ciclo_base_20260803, funcao:get_hs_prof_v3_projecao_viva_before_presence_cycle_fix_20260803, funcao:get_professor_retencao_v3_governada, funcao:hs_v3_segmentos_detalhe_base_canonica, funcao:materializar_health_score_professor_v3_periodo_impl_base_202607, funcao:montar_relatorio_coordenacao_payload_v3 |
...
| `fn_health_score_v3_unidades_permitidas_sombra(p_unidade_id uuid)` | SO-INTERNA | DEFINER | funcao:get_health_score_prof_v3_metricas_base_20260728_c95, funcao:get_health_score_professor_v3_carteira_periodo, funcao:get_health_score_professor_v3_metricas_periodo_base_20260719, funcao:get_health_score_professor_v3_permanencia_periodo_v2, funcao:get_health_score_professor_v3_presenca_periodo_v2, funcao:get_health_score_professor_v3_totais_carteira_canonica_v1, funcao:get_hs_prof_v3_conversao_ciclo_base_20260803, funcao:get_professor_carteira_ativa_tempo_v3_sombra, funcao:get_professor_conversao_v3_sombra, funcao:get_professor_media_turma_v3_sombra, funcao:get_professor_numero_alunos_v3_sombra, funcao:get_professor_permanencia_v3_sombra, funcao:get_professor_presenca_v3_sombra, funcao:get_professor_retencao_v3_governada, funcao:get_professor_retencao_v3_sombra, funcao:hs_v3_segmentos_detalhe_base_canonica |
...
| `fn_pendencia_presenca(p_registro_alvo_id uuid, p_tipo_alvo text, p_aluno_id integer)` | SO-INTERNA | INVOKER | funcao:fn_confirmar_registro_core |
...
| `fn_presenca_declarada(p_campos jsonb)` | SO-INTERNA | INVOKER | funcao:fabio_claim_registro_recibo, funcao:fabio_enfileirar_devolutivas, funcao:fn_confirmar_registro_core, funcao:fn_materializar_presenca_padrao |
...
| `fn_presenca_e_forte(p_respondido_por text)` | ATIVA | INVOKER · 🔓 anon | view:vw_aluno_presenca_semantica_v1, view:vw_experimental_faltou_sem_afirmacao, view:vw_experimental_registro_comercial, view:vw_fabio_aulas_contexto, view:vw_presenca_slot_canonica_v1, funcao:app_confirmar_registro_experimental, funcao:app_experimental_do_professor, funcao:app_registrar_presencas_aula, funcao:fn_aplicar_comando_presenca_core_v2, funcao:fn_materializar_presenca_padrao, funcao:fn_presenca_fecha_chamada, funcao:fn_reagendamento_limpa_chamada_alunos, funcao:fn_reconciliar_experimental_aulas, funcao:fn_registrar_presenca_experimental, funcao:fn_registrar_presencas_core, funcao:fn_sincronizar_gemeos_presenca, funcao:trg_sincronizar_gemeos_presenca, funcao:upsert_presenca_emusys_bruta |
...
| `fn_presenca_rollout_modo_interno_v1(p_unidade_id uuid, p_superficie text)` | SO-INTERNA | DEFINER | funcao:app_registrar_presencas_aula, funcao:fabio_confirmar_chamada_acao, funcao:fabio_emitir_presenca_por_registro, funcao:fabio_professor_presencas_periodo, funcao:fn_absenteismo_aluno_rollout_v1, funcao:fn_presenca_rollout_modo_escopo_interno_v1, funcao:fn_radar_aluno_sinais_rollout_v1, funcao:fn_texto_relatorio_presenca, funcao:get_faltas_periodo_v2, funcao:get_presenca_contexto_agente_v1, funcao:get_presenca_ocorrencias_periodo_v2, funcao:get_presenca_rollout_modo_v1 |
...
| `fn_professor_do_usuario()` | ATIVA | DEFINER | view:vw_disponibilidade_professores, funcao:app_abrir_rascunho_manual, funcao:app_aluno_ficha, funcao:app_atualizar_fatia, funcao:app_atualizar_perfil, funcao:app_atualizar_preferencia_fabio, funcao:app_concluir_onboarding, funcao:app_confirmar_meu_whatsapp, funcao:app_confirmar_registro, funcao:app_confirmar_registro_experimental, funcao:app_criar_comando_chamada_professor_v2, funcao:app_criar_comando_presenca_v1, funcao:app_declarar_falta_experimental, funcao:app_devolutiva_definir_destinatario, funcao:app_devolutiva_marcar, funcao:app_devolutiva_salvar_texto, funcao:app_devolutivas_aguardando, funcao:app_devolutivas_pendentes, funcao:app_enfileirar_audio, funcao:app_enfileirar_audio_experimental, funcao:app_experimental_do_professor, funcao:app_historico_turma, funcao:app_meu_acesso, funcao:app_meu_onboarding, funcao:app_meu_perfil, funcao:app_meu_ponto, funcao:app_meus_registros, funcao:app_minha_agenda_mes, funcao:app_minha_agenda_semana_v1, funcao:app_minha_agenda_sessao, funcao:app_minha_agenda_sessao_base_v1, funcao:app_minha_agenda_sessao_base_v1_referencia_lenta, funcao:app_minha_agenda_sessao_canonica_v2, funcao:app_minha_agenda_sessao_canonica_v2_referencia_lenta, funcao:app_minha_agenda_sessao_publicacao_legado_v1, funcao:app_minha_carteira, funcao:app_minha_disponibilidade, funcao:app_minhas_pendencias, funcao:app_minhas_preferencias_fabio, funcao:app_preparar_rascunho_manual, funcao:app_professor_feedback_mesa, funcao:app_professor_feedback_progresso, funcao:app_professor_feedback_salvar, funcao:app_propor_disponibilidade, funcao:app_registrar_experimental, funcao:app_registrar_presencas_aula, funcao:app_registro_completo, funcao:app_registros_pendentes, funcao:app_reportar_audio_preso, funcao:app_responder_confirmacao_ponto, funcao:app_responder_presenca, funcao:app_salvar_rascunho_manual, funcao:app_status_audio_fila, funcao:fn_criar_comando_presenca_core_v2, funcao:fn_diag_porta_do_professor, funcao:fn_enfileirar_audio_core, funcao:fn_pode_ler_aluno_pedagogico, funcao:get_jornada_professor, funcao:get_jornada_professor_trancados |
...
| `fn_responder_presenca_core(p_professor_id integer, p_registro_alvo_id uuid, p_presenca text)` | SO-INTERNA | DEFINER | funcao:app_responder_presenca, funcao:fabio_responder_presenca |
...
| `validar_token_sync_presenca_interno_v1(p_token text)` | ATIVA | DEFINER | edge:supabase/functions/classificar-resposta-evasao/index.ts, edge:supabase/functions/processar-fila-repescagem-evasao/index.ts, edge:supabase/functions/sync-presenca-emusys/index.ts |
