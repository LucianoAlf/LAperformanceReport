...
608 objetos. Uma linha cada; colunas e FKs em `detalhe/<dominio>.md`.
...
| `anamnese_respostas_perfil` | tabela | aluno | 5 | 7039 | sim (1) | 1 | Respostas individuais das 11 perguntas de perfil comportamental. Armazena posição escolhida para auditoria e recálculo. |
...
| `kpis_alunos_cache` | tabela | aluno | 4 | 52 | sim (0) | 0 | LAPE-42. Cache por versao dos KPIs de alunos. Chave = funcao + usuario + parametros + data BRT + impressao digital das tabelas lidas (kpis_alunos_cache_impressao_v1). Lido/escrito so pelas funcoes *_cache_v1. |
...
| `pesquisa_evasao` | tabela | aluno | 55 | 151 | sim (1) | 12 |  |
...
| `pesquisa_evasao_processamento` | tabela | aluno | 8 | 3 | sim (0) | 1 | Fila service-only para consolidar rajadas de respostas da pesquisa de evasão. |
...
| `radar_sinais` | tabela | aluno | 31 | 1600 | sim (1) | 1 | Mapa de Sinais: cada detecção vira evento com evidência, orientação e desfecho. View=foto, esta tabela=filme. |
...
| `atendimento_conversa_estado` | tabela | comercial | 22 | 1694 | sim (1) | 0 | T2/1o andar/operacional. Espelho dos FATOS da conversa do Chatwoot (projeto SOL), ingerido pela edge `ingerir-calor-atendimento`. ⚠️ "humano" = agente que nao e Mila. ⚠️ minutos_ate_humano NEGATIVO = nos iniciamos a conversa. ⚠️ departamento comercial so existe desde 03/09/2026. |
...
| `google_ads_cliques` | tabela | comercial | 26 | 32 | sim (0) | 1 | Cliques do Google Ads recebidos do webhook da onpromedia (CQC). Guarda EVENTO de clique, nao lead -- `leads` continua fonte unica. Serve de fila de re-tentativa (o webhook chega segundos antes do lead existir), de registro do clique orfao e do historico de cliques por lead. Schema reaproveitado da Fase 2 abandonada (pagina-pedagio propria), que nunca chegou a rodar. |
...
| `lead_experimental_aulas` | tabela | comercial | 19 | 504 | sim (0) | 3 | Vinculo lead<->aula da experimental. RLS ligada e SEM policy: so security definer (dono postgres) e service_role entram. O cliente fala com app_experimental_do_professor / app_minha_agenda_sessao. |
...
| `leads` | tabela | comercial | 72 | 10327 | sim (5) | 15 | Leads comerciais - do primeiro contato até a conversão ou arquivamento |
...
| `faturas_leitura_cache` | tabela | financeiro | 3 | 141 | não | 0 | Cache do payload de get_faturas_alunos_financeiro_v1. Chave = md5(params + max(completed_at) dos runs completos + escopo de unidades). Lido/escrito apenas via SECURITY DEFINER. |
...
| `financeiro_sync_queue` | tabela | financeiro | 20 | 8040 | sim (0) | 1 | Fila unica do sync financeiro Emusys. Um job publica uma competencia completa das tres unidades. |
...
| `kpis_comercial_v2_cache` | tabela | gestao | 3 | 518 | não | 0 | Cache de get_kpis_comercial_canonicos_v2 (jsonb). Chave = md5(params+dia+fingerprint das fontes). TTL 30min. Lido/escrito apenas via SECURITY DEFINER. |
...
| `situacao_alunos_snapshot` | tabela | gestao | 62 | 11107 | sim (0) | 0 |  |
...
| `conciliacao_experimentais_v2_cache` | tabela | integracao | 3 | 517 | não | 0 | Cache de get_conciliacao_experimentais_v2 (jsonb). Chave = md5(params+dia+fingerprint das fontes). TTL 30min. Auth resolvida antes do cache. Lido/escrito apenas via SECURITY DEFINER. |
...
| `emusys_faturas` | tabela | integracao | 22 | 19223 | sim (1) | 1 |  |
...
| `hermes_patch_status` | tabela | integracao | 5 | 6 | sim (3) | 0 | Estado do patch local do Hermes por agente (Fabio/Mila/Lia/...). Escrito por um cron root (hermes-patch-guard.sh) e lido por monitor-saude-fabio. Se patched=false ou checado_em velho, alerta no WhatsApp. |
...
| `lia_followup_resumos` | tabela | integracao | 7 | 11 | sim (0) | 1 | Resumo privado diario por operador, produzido as 09:00 BRT e entregue pela outbox da Lia. |
...
| `cache_versao_log` | tabela | plataforma | 3 | 776 | sim (0) | 0 | LAPE-42: uma linha por comando de escrita nas tabelas lidas pelos caches de servidor. Versao de cache = count:max:sum dos ids visiveis. Podado por cache_versao_podar_v1. |
...
| `paginas_rpc_cache` | tabela | plataforma | 4 | 795 | não | 0 | Cache generico das RPCs pesadas de pagina (agenda, alunos, administrativo, professores). Chave = recorte logico (+escopo de unidades quando a funcao filtra por usuario). TTL na funcao wrapper. Criado em 2026-09-24. |
...
| `aula_alunos_emusys` | tabela | professor | 16 | 42393 | sim (3) | 3 | Roster operacional de aulas do Emusys, sem contato ou dados financeiros. |
...
| `aula_roster_sync_estado` | tabela | professor | 9 | 23180 | sim (0) | 2 |  |
| `aulas_emusys` | tabela | professor | 35 | 68591 | sim (4) | 3 | Metadados completos de cada aula importada do Emusys (turma, curso, sala, professor, horários) |
...
| `dash_prof_resumo_cache` | tabela | professor | 3 | 169 | não | 0 | Cache do resumo de professores do dashboard (5 colunas). TTL 5 min + fingerprint leve. Lido/escrito apenas via SECURITY DEFINER. |
...
| `fabio_devolutivas` | tabela | professor | 31 | 1105 | sim (0) | 2 |  |
...
| `fabio_fila_audios` | tabela | professor | 15 | 902 | sim (2) | 4 |  |
...
| `fabio_registros_aula` | tabela | professor | 19 | 2352 | sim (3) | 7 |  |
...
| `health_score_professor_v3_materializacao_execucoes` | tabela | professor | 22 | 404 | sim (3) | 1 |  |
...
| `presenca_emusys_escrita` | tabela | professor | 20 | 1555 | sim (0) | 0 | Livro de bordo do escritor de presenca no Emusys: uma linha por gatilho (evento de aluno ou ficha de professor), com estado antes, decisao, resposta e modo sombra/ativo. |
...
| `professor_periodos_reconstrucao_manifesto_v1` | tabela | professor | 13 | 305479 | sim (0) | 5 | Manifesto privado e imutavel por versao do recorte. Calcula uma vez a particao da pessoa canonica. |
