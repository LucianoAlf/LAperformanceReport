...
591 objetos. Uma linha cada; colunas e FKs em `detalhe/<dominio>.md`.
...
| `aluno_contratos_emusys` | tabela | aluno | 12 | 1218 | sim (1) | 2 |  |
...
| `aluno_jornada_matricula_disciplina` | tabela | aluno | 48 | 5100 | sim (2) | 4 |  |
...
| `alunos` | tabela | aluno | 73 | 1731 | sim (5) | 11 |  |
...
| `comunidade_wa_participantes` | tabela | aluno | 5 | 695 | sim (0) | 1 | Foto mais recente dos participantes de cada grupo. Linhas somem quando o participante some na captura seguinte (é estado atual, não histórico). |
...
| `radar_sinais` | tabela | aluno | 31 | 1331 | sim (1) | 1 | Mapa de Sinais: cada detecção vira evento com evidência, orientação e desfecho. View=foto, esta tabela=filme. |
...
| `atendimento_conversa_estado` | tabela | comercial | 22 | 1553 | sim (1) | 0 | T2/1o andar/operacional. Espelho dos FATOS da conversa do Chatwoot (projeto SOL), ingerido pela edge `ingerir-calor-atendimento`. ⚠️ "humano" = agente que nao e Mila. ⚠️ minutos_ate_humano NEGATIVO = nos iniciamos a conversa. ⚠️ departamento comercial so existe desde 03/09/2026. |
...
| `google_ads_cliques` | tabela | comercial | 26 | 13 | sim (0) | 1 | Cliques do Google Ads recebidos do webhook da onpromedia (CQC). Guarda EVENTO de clique, nao lead -- `leads` continua fonte unica. Serve de fila de re-tentativa (o webhook chega segundos antes do lead existir), de registro do clique orfao e do historico de cliques por lead. Schema reaproveitado da Fase 2 abandonada (pagina-pedagio propria), que nunca chegou a rodar. |
...
| `google_ads_metricas_diarias` | tabela | comercial | 17 | 307 | sim (1) | 0 | Alicerce/estrategica. Custo diario do Google Ads por CAMPANHA (grao de campanha atravessa Search e Performance Max; anuncio nao). Gemeo de meta_ads_metricas_diarias. Reescrita por janela — o Google revisa conversao por dias. gasto ja vem convertido de cost_micros na ingestao. |
| `instagram_sessoes` | tabela | comercial | 15 | 124 | sim (1) | 1 | Espelho das sessões da bridge de Instagram (la-hq, instagram-comments-bridge.js). Uma linha por (conta, pessoa). Alimentado pela edge ingerir-instagram-sessoes; a bridge segue sendo a fonte de verdade viva — isto é foto para leitura, relatório e Mapa de Sinais. |
| `lead_conciliacao_decisoes` | tabela | comercial | 10 | 318 | sim (1) | 1 |  |
| `lead_experimentais` | tabela | comercial | 19 | 1191 | sim (1) | 6 |  |
...
| `lead_experimental_aulas` | tabela | comercial | 19 | 468 | sim (0) | 3 | Vinculo lead<->aula da experimental. RLS ligada e SEM policy: so security definer (dono postgres) e service_role entram. O cliente fala com app_experimental_do_professor / app_minha_agenda_sessao. |
...
| `financeiro_emusys_lancamentos` | tabela | financeiro | 20 | 23575 | sim (0) | 1 | Espelho item a item de GET /financeiro/lancamentos (Emusys beta). Nunca apaga: item que some de varredura completa do dia ganha sumiu_em. Chave única (unidade_id, emusys_lancamento_id) porque o id é por token. |
...
| `financeiro_sync_queue` | tabela | financeiro | 20 | 6836 | sim (0) | 1 | Fila unica do sync financeiro Emusys. Um job publica uma competencia completa das tres unidades. |
...
| `pix_migracao_snapshot` | tabela | gestao | 18 | 4377 | sim (0) | 0 |  |
| `projecao_aulas` | tabela | gestao | 12 | 52534 | não | 2 |  |
...
| `situacao_alunos_snapshot` | tabela | gestao | 62 | 5032 | sim (0) | 0 |  |
...
| `emusys_faturas` | tabela | integracao | 22 | 12588 | sim (1) | 1 |  |
| `emusys_historico_backfill_execucoes_v1` | tabela | integracao | 20 | 135 | sim (0) | 2 | Checkpoint retomavel do coletor historico Emusys do Health Score Professor V3. |
| `emusys_matriculas_estado_atual` | tabela | integracao | 23 | 4796 | sim (1) | 2 | Fonte bruta backend-only do estado atual de cada matricula Emusys, escopada por unidade. |
...
| `fila_anamnese_sol_hermes` | tabela | integracao | 18 | 676 | sim (0) | 3 | Outbox Sol/Hermes para anamnese_professor. Substitui envio direto WAHA legado. |
...
| `sync_faturas_pagas_mes_queue` | tabela | integracao | 22 | 0 | sim (0) | 0 | Fila duravel de faturas pagas por competencia de pagamento. Evita perder a rotina das 05:00 UTC quando lancamentos ainda estao ativos. |
| `sync_financeiro_emusys_queue` | tabela | integracao | 21 | 24 | sim (0) | 0 | Fila serial da varredura de lancamentos Emusys. Uma tentativa inicial e no maximo tres retries de 30 minutos. |
...
| `sync_runs` | tabela | integracao | 20 | 6791 | sim (1) | 0 | Execucoes preservadas do sync financeiro. Somente live completo prova frescor; baseline serve apenas para comparacao. |
...
| `eventos_operacionais` | tabela | outros | 15 | 231 | sim (0) | 3 | Fatos operacionais append-only para consumo por servicos. Nunca armazena financeiro, saude, presenca, observacoes livres ou payload bruto. |
| `eventos_operacionais_audiencia` | tabela | outros | 4 | 282 | sim (0) | 2 | Audiencia por professor da projecao de eventos operacionais; detectado_em e denormalizado para leitura paginada. |
...
| `aula_alunos_emusys` | tabela | professor | 16 | 39337 | sim (3) | 3 | Roster operacional de aulas do Emusys, sem contato ou dados financeiros. |
...
| `aula_roster_sync_estado` | tabela | professor | 9 | 21075 | sim (0) | 2 |  |
| `aulas_emusys` | tabela | professor | 35 | 66465 | sim (4) | 3 | Metadados completos de cada aula importada do Emusys (turma, curso, sala, professor, horários) |
...
| `fabio_fila_audios` | tabela | professor | 15 | 795 | sim (2) | 4 |  |
...
| `fabio_notificacoes` | tabela | professor | 23 | 1028 | sim (2) | 1 | Notificacoes proativas do Fabio (Fase 2). O AGENDAMENTO nao mora aqui — decidido pelo Hermes/VPS via cron, sem pg_cron (mesmo padrao do Tom). tipo=pendencia_registro/reagendamento sao SEMPRE categoria=governanca. Os demais sao informativa. |
...
| `fabio_registros_aula` | tabela | professor | 19 | 2035 | sim (3) | 7 |  |
...
| `presenca_comandos` | tabela | professor | 18 | 4928 | sim (0) | 0 | Intencao duravel e idempotente de escrita humana de presenca; sem nomes ou payload bruto. |
...
| `presenca_sync_cobertura` | tabela | professor | 14 | 258 | sim (0) | 2 | Estado atual por unidade, modo e data; somente concluida com hash e publicavel. |
...
| `presenca_sync_execucoes` | tabela | professor | 15 | 12719 | sim (0) | 1 | Uma linha por tentativa de sync; inclui tentativas deduplicadas como abortadas. |
...
| `professor_periodos_reconstrucao_manifesto_v1` | tabela | professor | 13 | 428278 | sim (0) | 5 | Manifesto privado e imutavel por versao do recorte. Calcula uma vez a particao da pessoa canonica. |
| `professor_periodos_reconstrucao_particoes_v1` | tabela | professor | 20 | 375 | sim (0) | 2 | Resultados intermediarios, idempotentes e privados da reconstrucao V3. O detalhe diagnostico fica aqui; a camada final so nasce apos todas as particoes. |
