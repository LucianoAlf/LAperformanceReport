...
554 objetos. Uma linha cada; colunas e FKs em `detalhe/<dominio>.md`.
...
| `aluno_contratos_emusys` | tabela | aluno | 12 | 1181 | sim (1) | 2 |  |
...
| `aluno_jornada_matricula_disciplina` | tabela | aluno | 47 | 5064 | sim (2) | 4 |  |
...
| `aluno_presenca_administrativo` | tabela | aluno | 13 | 25643 | sim (2) | 4 | Camada administrativa read-only para o professor; justificada vem do Emusys. |
...
| `aluno_reposicoes` | tabela | aluno | 15 | 156 | sim (2) | 4 | Credito de reposicao: nasce de falta justificada ou cancelamento, morre quando a aula reposta acontece. Casamento por elo direto (reagendada) ou rede (aluno+disciplina+janela). |
...
| `alunos` | tabela | aluno | 73 | 1705 | sim (5) | 11 |  |
...
| `anamnese_convites` | tabela | aluno | 14 | 71 | sim (1) | 3 | Convites de anamnese remota. Um convite vivo por aluno; expira em 7 dias ou no uso. |
| `anamnese_respostas_perfil` | tabela | aluno | 5 | 3172 | sim (1) | 1 | Respostas individuais das 11 perguntas de perfil comportamental. Armazena posição escolhida para auditoria e recálculo. |
| `anamneses` | tabela | aluno | 51 | 293 | sim (1) | 3 | Anamnese do aluno - coleta de perfil pedagógico, saúde, temperamento comportamental. Vinculada a alunos.id (nullable para pré-matrícula). |
...
| `radar_entregas` | tabela | aluno | 12 | 0 | sim (1) | 2 | Log de entrega com idempotencia: o mesmo sinal nao e cobrado duas vezes da mesma pessoa no mesmo turno. |
...
| `radar_rodadas` | tabela | aluno | 7 | 29 | sim (1) | 0 | Uma linha por execucao do detector. Existe para desambiguar "a situacao parou de aparecer" (sanou) de "o detector nao rodou" (nao sei). Sem isso, cron parado esvazia a pauta em silencio. |
| `radar_sinais` | tabela | aluno | 31 | 945 | sim (1) | 1 | Mapa de Sinais: cada detecção vira evento com evidência, orientação e desfecho. View=foto, esta tabela=filme. |
...
| `atendimento_consultor_diario` | tabela | comercial | 15 | 42 | sim (0) | 1 | Instantâneo diário (19:10 BRT) de atendimento_conversa_estado por pessoa. ESTOQUE do que ficou pendurado, não velocidade de resposta — velocidade é do chatwoot-atendimento-insights, ao vivo. |
| `atendimento_conversa_estado` | tabela | comercial | 22 | 1026 | sim (1) | 0 | T2/1o andar/operacional. Espelho dos FATOS da conversa do Chatwoot (projeto SOL), ingerido pela edge `ingerir-calor-atendimento`. ⚠️ "humano" = agente que nao e Mila. ⚠️ minutos_ate_humano NEGATIVO = nos iniciamos a conversa. ⚠️ departamento comercial so existe desde 03/09/2026. |
...
| `instagram_sessoes` | tabela | comercial | 15 | 111 | sim (1) | 1 | Espelho das sessões da bridge de Instagram (la-hq, instagram-comments-bridge.js). Uma linha por (conta, pessoa). Alimentado pela edge ingerir-instagram-sessoes; a bridge segue sendo a fonte de verdade viva — isto é foto para leitura, relatório e Mapa de Sinais. |
...
| `lead_experimental_aulas` | tabela | comercial | 19 | 371 | sim (0) | 3 | Vinculo lead<->aula da experimental. RLS ligada e SEM policy: so security definer (dono postgres) e service_role entram. O cliente fala com app_experimental_do_professor / app_minha_agenda_sessao. |
...
| `leads` | tabela | comercial | 70 | 9913 | sim (5) | 15 | Leads comerciais - do primeiro contato até a conversão ou arquivamento |
...
| `financeiro_sync_queue` | tabela | financeiro | 20 | 4608 | sim (0) | 1 | Fila unica do sync financeiro Emusys. Um job publica uma competencia completa das tres unidades. |
...
| `sol_caixa_shadow_eventos_v1` | tabela | financeiro | 15 | 820 | sim (0) | 1 | Sol Caixa V3 shadow privado: eventos reais observados sem resposta pública e sem mutação financeira. |
| `sol_caixa_shadow_previews_v1` | tabela | financeiro | 11 | 1433 | sim (0) | 2 | Sol Caixa V3 shadow privado: previews calculados para auditoria, nunca enviados ao WhatsApp por esta tabela. |
...
| `projecao_aulas` | tabela | gestao | 12 | 51189 | não | 2 |  |
...
| `admin_conversas` | tabela | integracao | 15 | 270 | sim (1) | 3 | Conversas administrativas com alunos via WhatsApp - uma por aluno por unidade |
| `admin_mensagens` | tabela | integracao | 17 | 1807 | sim (1) | 2 | Mensagens das conversas administrativas com alunos |
| `alunos_emusys_atributos_decisoes` | tabela | integracao | 13 | 929 | sim (1) | 2 |  |
| `alunos_emusys_atributos_divergencias` | tabela | integracao | 18 | 11050 | sim (1) | 2 |  |
...
| `emusys_experimentais_raw` | tabela | integracao | 32 | 213561 | sim (1) | 8 | Bruto por aluno das aulas experimentais do Emusys. Nao altera lead_experimentais, status, presenca ou KPI canonico. |
...
| `emusys_fatura_source_events` | tabela | integracao | 12 | 4833873 | sim (1) | 4 | Trilha append-only das confirmacoes, ausencias e resolucoes observadas por competencia. |
| `emusys_faturas` | tabela | integracao | 22 | 5347 | sim (1) | 1 |  |
| `emusys_historico_backfill_execucoes_v1` | tabela | integracao | 20 | 105 | sim (0) | 2 | Checkpoint retomavel do coletor historico Emusys do Health Score Professor V3. |
| `emusys_matriculas_estado_atual` | tabela | integracao | 23 | 4921 | sim (1) | 2 | Fonte bruta backend-only do estado atual de cada matricula Emusys, escopada por unidade. |
...
| `emusys_professor_disciplinas_sync_execucoes` | tabela | integracao | 14 | 248 | sim (0) | 2 |  |
| `emusys_sync_log` | tabela | integracao | 16 | 6969 | sim (1) | 1 |  |
| `fila_anamnese_sol_hermes` | tabela | integracao | 18 | 429 | sim (0) | 3 | Outbox Sol/Hermes para anamnese_professor. Substitui envio direto WAHA legado. |
...
| `notificacao_log` | tabela | integracao | 13 | 498 | sim (4) | 3 |  |
...
| `sync_run_items` | tabela | integracao | 27 | 4774662 | sim (1) | 3 | Snapshot imutavel por run/competencia/unidade/fatura, incluindo tombstones de nao confirmacao pela origem. |
| `sync_run_overrides` | tabela | integracao | 10 | 0 | sim (1) | 2 |  |
| `sync_runs` | tabela | integracao | 20 | 4702 | sim (1) | 0 | Execucoes preservadas do sync financeiro. Somente live completo prova frescor; baseline serve apenas para comparacao. |
...
| `comunidade_wa_participantes` | tabela | outros | 5 | 643 | sim (0) | 1 | Foto mais recente dos participantes de cada grupo. Linhas somem quando o participante some na captura seguinte (é estado atual, não histórico). |
...
| `aula_alunos_emusys` | tabela | professor | 16 | 36355 | sim (3) | 3 | Roster operacional de aulas do Emusys, sem contato ou dados financeiros. |
| `aula_registros_fabio_log` | tabela | professor | 8 | 670 | sim (0) | 1 | Auditoria das gravações do Fábio em aulas_emusys.anotacoes_fabio. É trilha de rastreabilidade, não o lar do registro (o registro vive em anotacoes_fabio). |
| `aula_roster_sync_estado` | tabela | professor | 9 | 17855 | sim (0) | 2 |  |
| `aulas_emusys` | tabela | professor | 35 | 63184 | sim (4) | 3 | Metadados completos de cada aula importada do Emusys (turma, curso, sala, professor, horários) |
...
| `fabio_acoes_pendentes` | tabela | professor | 20 | 34 | sim (0) | 4 | Estado duravel e auditavel das acoes iniciadas pelo professor no WhatsApp. |
...
| `fabio_canario_execucao` | tabela | professor | 7 | 27 | sim (0) | 0 | Prova periodica de que um caminho de ESCRITA ainda funciona. Cada linha e uma corrida que escreveu de verdade e LEU DE VOLTA. `passou=false` e defeito; ausencia de linha e o canario que parou -- os dois sao alarme. |
...
| `fabio_correcao` | tabela | professor | 11 | 0 | sim (0) | 0 | O que foi CONSERTADO, no formato do TOM: codigo, a fala literal de quem sofreu, o mecanismo em uma frase, a prova e a referencia. Sem a fala e o mecanismo, "corrigi o gate" e frase que ninguem pode conferir. |
...
| `fabio_devolutivas` | tabela | professor | 31 | 660 | sim (0) | 2 |  |
| `fabio_diario_ciclo` | tabela | professor | 11 | 0 | sim (0) | 0 |  |
| `fabio_diario_mensagem` | tabela | professor | 13 | 218 | sim (0) | 1 | Fala capturada do grupo/DM para o diário do Fábio. Escuta, não responde. Grupo avisado em 04/09. |
| `fabio_diario_ocorrencia` | tabela | professor | 21 | 156 | sim (0) | 1 | Diário do Fábio: o que deu errado no dia, de onde veio e POR QUE. Quadrado do Fábio — não confundir com o schema monitoramento, que é da Maria/Tom. |
| `fabio_fila_audios` | tabela | professor | 15 | 607 | sim (2) | 4 |  |
...
| `fabio_mineracao_janela` | tabela | professor | 6 | 8 | sim (0) | 0 | Que janelas ja foram lidas. Linha ausente = nao minerado; `propostas = 0` = minerado e nao achou nada. As duas coisas sao diferentes e nao podem virar o mesmo zero. |
| `fabio_notificacoes` | tabela | professor | 23 | 736 | sim (2) | 1 | Notificacoes proativas do Fabio (Fase 2). O AGENDAMENTO nao mora aqui — decidido pelo Hermes/VPS via cron, sem pg_cron (mesmo padrao do Tom). tipo=pendencia_registro/reagendamento sao SEMPRE categoria=governanca. Os demais sao informativa. |
...
| `fabio_registros_aula` | tabela | professor | 19 | 1448 | sim (3) | 7 |  |
| `fabio_relato_proposto` | tabela | professor | 16 | 27 | sim (0) | 1 | Propostas de atrito mineradas da conversa do professor. CONTEM CITACAO LITERAL do que ele disse -- fechada para anon/authenticated e com RLS desde 08/09/2026, quando a auditoria achou que a chave publica lia e apagava. |
...
| `health_score_professor_v3_materializacao_execucoes` | tabela | professor | 22 | 170 | não | 1 |  |
...
| `presenca_acao_eventos` | tabela | professor | 15 | 7330 | sim (0) | 1 | Recibo append-only por request_id: recebido, resultado por item e conclusao. |
| `presenca_comando_itens` | tabela | professor | 9 | 2376 | sim (0) | 1 |  |
...
| `presenca_comandos` | tabela | professor | 18 | 2501 | sim (0) | 0 | Intencao duravel e idempotente de escrita humana de presenca; sem nomes ou payload bruto. |
...
| `presenca_sync_cobertura` | tabela | professor | 14 | 171 | sim (0) | 2 | Estado atual por unidade, modo e data; somente concluida com hash e publicavel. |
| `presenca_sync_eventos` | tabela | professor | 5 | 22110 | sim (0) | 1 | Transicoes append-only do sync; a aplicacao nao possui UPDATE nem DELETE. |
| `presenca_sync_execucoes` | tabela | professor | 15 | 9364 | sim (0) | 1 | Uma linha por tentativa de sync; inclui tentativas deduplicadas como abortadas. |
...
| `professor_periodos_reconstrucao_manifesto_v1` | tabela | professor | 13 | 263639 | sim (0) | 5 | Manifesto privado e imutavel por versao do recorte. Calcula uma vez a particao da pessoa canonica. |
| `professor_periodos_reconstrucao_particoes_v1` | tabela | professor | 20 | 109 | sim (0) | 2 | Resultados intermediarios, idempotentes e privados da reconstrucao V3. O detalhe diagnostico fica aqui; a camada final so nasce apos todas as particoes. |
...
| `professor_ponto_confirmacoes` | tabela | professor | 9 | 4287 | sim (1) | 3 |  |
...
| `vw_registro_pendencia` | view | professor | 20 | — | não | 0 | Aulas sem registro. "Tem registro" tem DUAS linguas: anotacoes_fabio para a aula comum e lead_experimental_registros (status confirmado) para a experimental. So a segunda foi ensinada em 07/09/2026 -- antes disso o Fabio cobrava experimental ja registrada. |
