          ...
          {items.map((item) => {
            ...
            return (
              <tr key={`${item.unidade_id}|${item.canonical_fatura_id}`} className="group bg-slate-900/15 transition hover:bg-cyan-500/[0.035]">
                ...
                <td className="px-3 py-3.5 text-right"><p className={cn('font-semibold tabular-nums', item.status === 'paga' ? 'text-emerald-300' : 'text-cyan-200')}>{valorPrincipal == null ? '—' : moeda(valorPrincipal)}</p><p className="mt-0.5 text-[10px] text-slate-500">{item.status === 'paga' ? 'Valor pago' : 'Valor hoje'}</p></td>
