 ...
 */
export function ChamadaLeadCard({ lead, podeOperar, salvando, onMarcar }: Props) {
  ...
  // Campos faltantes: sem curso de interesse OU sem telefone
  const faltamCampos = !lead.curso_interesse_id || !lead.telefone;
    ...
    >
      ...
      {faltamCampos && (
        <div className="mt-2 flex items-center gap-1.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[10px] text-amber-300">
          ...
          {!lead.curso_interesse_id && !lead.telefone
            ? 'Falta curso de interesse e telefone'
            : !lead.curso_interesse_id
              ? 'Falta curso de interesse'
              : 'Falta telefone'}
        </div>
      )}
