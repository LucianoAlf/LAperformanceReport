...
const CAMPO_GRANDE_UNIDADE_ID = '2ec861f6-023f-4d7b-9927-3960ad8c2a92';
const SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026 = {
  ano: 2026,
  mes: 5,
  unidadeId: CAMPO_GRANDE_UNIDADE_ID,
  kids: 202,
  school: 294,
...
export function TabGestao({ ano, mes, mesFim, unidade }: TabGestaoProps) {
  ...
  // Usar mesFim se fornecido, senao usar mes (para filtro mensal)
  const mesInicio = mes;
  ...
  const segmentacaoKidsSchoolReconstruida =
    mesFechado &&
    unidade === SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.unidadeId &&
    ano === SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.ano &&
    mesInicio === SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.mes &&
    mesFinal === SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.mes;
    ...
    async function fetchDados() {
      ...
      try {
        ...
        if (dadosMesAnteriorData && dadosMesAnteriorData.length > 0) {
          const ticketMesAnteriorFallback = calcularTicketMedioCanonico(
            dadosMesAnteriorData.map((d: any) => ({
          ...
          const consolidadoMesAnterior = dadosMesAnteriorData.reduce((acc, d) => ({
            ...
            faturamento_estimado: acc.faturamento_estimado + (Number(d.mrr_contratual ?? d.faturamento_estimado) || 0),
        ...
        if (dadosAnoAnteriorData && dadosAnoAnteriorData.length > 0) {
          const ticketAnoAnteriorFallback = calcularTicketMedioCanonico(
            dadosAnoAnteriorData.map((d: any) => ({
              mrr: Number(d.mrr_contratual ?? d.faturamento_estimado) || 0,
              ticketMedio: Number(d.ticket_medio_contratual ?? d.ticket_medio) || 0,
              ticketDenominadorPagantes: d.ticket_denominador_pagantes ?? null,
            })),
        ...
        if (gestaoData && gestaoData.length > 0) {
          const g = gestaoData.reduce((acc, item) => ({
            ...
            ticket_medio_sum: acc.ticket_medio_sum + ((Number(item.ticket_medio) || 0) * (obterDenominadorTicketCanonico({
              mrr: Number(item.mrr) || 0,
              ticketMedio: Number(item.ticket_medio) || 0,
              ticketDenominadorPagantes: item.ticket_denominador_pagantes ?? null,
              ticketDenominadorFaturas: item.ticket_denominador_faturas ?? null,
            }) || 0)),
            ticket_denominador_sum: acc.ticket_denominador_sum + (obterDenominadorTicketCanonico({
              mrr: Number(item.mrr) || 0,
              ticketMedio: Number(item.ticket_medio) || 0,
              ticketDenominadorPagantes: item.ticket_denominador_pagantes ?? null,
              ticketDenominadorFaturas: item.ticket_denominador_faturas ?? null,
          ...
          (alunosAtivosData || []).forEach((a: any) => {
            ...
            const pessoa = pessoasMap.get(pessoaKey) || {
              idade: a.idade_atual,
              temRegular: false,
              temAtivo: false,
              matriculas: [],
            ...
            pessoasMap.set(pessoaKey, pessoa);
          });
          ...
          if (
            ...
            totalLaKids = SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.kids;
            totalLaAdultos = SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.school;
            totalLaSemClassificacao = Math.max(mediaAlunos - totalLaKids - totalLaAdultos, 0);
          }
        ...
        let financeiroQuery = supabase
          .from('dados_mensais')
          .select('mes, ano, alunos_pagantes, ticket_medio, ticket_denominador_pagantes, ticket_medio_contratual, mrr_contratual, faturamento_estimado, inadimplencia, reajuste_parcelas, churn_rate, taxa_renovacao, unidade_id')
          .gte('ano', 2023)
          .order('ano', { ascending: true })
          .order('mes', { ascending: true });
        ...
        if (financeiroData) {
          ...
          financeiroData.forEach((item: any) => {
            ...
            item.ticket_denominador_pagantes = row.ticketDenominadorPagantes ?? null;
        ...
        if (financeiroData) {
          ...
          financeiroData.forEach((item: any) => {
            ...
            if (key <= mesAtualKey) {
              ...
              atual.denominador += obterDenominadorTicketCanonico({
                mrr: Number(item.mrr_atual ?? item.faturamento_estimado) || 0,
                ticketMedio: Number(item.ticket_medio) || 0,
                ticketDenominadorPagantes: item.ticket_denominador_pagantes ?? null,
                ticketDenominadorFaturas: item.ticket_denominador_faturas ?? null,
              }) || 0;
        ...
        <div className="space-y-6">
          ...
          <div data-tour="analytics-kpis" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <KPICard
              ...
              tooltip="Total de alunos/pessoas únicos ativos. Inclui pagantes, bolsistas e alunos só em banda/projeto; múltiplas matrículas não duplicam."
          ...
          {segmentacaoKidsSchoolReconstruida && (
            <div className="inline-flex max-w-full items-center gap-2 rounded-lg border border-cyan-500/25 bg-cyan-500/10 px-3 py-2 text-xs text-cyan-100">
              <Info className="h-3.5 w-3.5 flex-shrink-0 text-cyan-300" />
              <span>{SEGMENTACAO_KIDS_SCHOOL_CG_MAIO_2026.nota}</span>
            </div>
          )}
