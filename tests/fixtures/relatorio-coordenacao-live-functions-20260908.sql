-- Snapshot de codigo PostgreSQL, sem dados de negocio, medido em producao em 08/09/2026.
-- Usado somente para provar em Docker que a migration protegida recompila o corpo auditado.

CREATE OR REPLACE FUNCTION public.enriquecer_relatorio_coordenacao_v2_comparabilidade(p_payload jsonb, p_unidade_id uuid, p_competencia date)
 RETURNS jsonb
 LANGUAGE plpgsql
...
$function$;
CREATE OR REPLACE FUNCTION public.montar_relatorio_coordenacao_payload_v3(p_unidade_id uuid, p_ano integer, p_mes integer, p_periodicidade text DEFAULT 'mensal'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
begin
  ...
  if jsonb_array_length(v_performance) = 0 then
    ...
    with base_professores as (
    select value as item
    from jsonb_array_elements(coalesce(v_base->'professores', '[]'::jsonb))
    where nullif(value->>'professor_id', '') is not null
  )
  select coalesce(
    jsonb_object_agg(
      item->>'professor_id',
      coalesce(item->'operacional', '{}'::jsonb)
    ),
    '{}'::jsonb
  )
  into v_operacional
  from base_professores;
  ...
  select coalesce(
    jsonb_agg(
      ...
      || jsonb_build_object(
        ...
        'ranking_habilitado', (coalesce(p.ranking_habilitado, false) or coalesce(p.estado_publicacao, '') = 'oficial'),
  ...
  operacional as (
    select value as item from jsonb_each(v_operacional)
  )
  select jsonb_build_object(
  ...
  with equipe as (
    ...
    where value->>'estado_publicacao' = 'oficial'
      and coalesce(nullif(value->>'score_comparavel', '')::numeric, nullif(value->>'score', '')::numeric) is not null
  )
  select jsonb_agg(
    jsonb_build_object(
      'professor_id', (p->>'professor_id')::integer,
      'nome', p->>'nome',
      'score', coalesce((p->>'score_comparavel')::numeric, (p->>'score')::numeric),
      'cobertura', (p->>'cobertura')::numeric,
      'classificacao', p->>'classificacao'
    )
    order by coalesce((p->>'score_comparavel')::numeric, (p->>'score')::numeric) desc, p->>'nome'
  )
  into v_ranking
  from equipe;
  ...
  return v_base || jsonb_build_object(
    ...
    'auditoria', coalesce(v_base->'auditoria', '{}'::jsonb) || jsonb_build_object(
      ...
      'fonte_health_score', 'get_health_score_professor_v3_performance_snapshot_v3',
      'periodicidade', p_periodicidade,
      'ciclo_codigo', v_ciclo_codigo,
      'data_corte', v_data_corte,
