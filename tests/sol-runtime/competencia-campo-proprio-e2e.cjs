...
function novo({ canonicaFn, casarFn } = {}) {
  const enviadas = [];
  const ids = [];
  const logs = [];
  const lancados = [];
  ...
  const canonicaEfetiva = canonicaFn || (async () => (++chamadasCanonica === 1
    ? { ok: false, motivo: 'nenhuma_fatura_aberta' }
    : CANONICA_09));
  const casarEfetivo = casarFn || (async () => ({
  ...
  const h = mod.criarHandlerFinanceiro({
    ...
    canonicaFn: canonicaEfetiva,
    casarFn: casarEfetivo,
    responsavelFn: async () => ({
