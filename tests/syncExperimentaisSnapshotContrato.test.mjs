...
test("reconciliacao troca a referencia de evento pela aula fisica", () => {
  const resultado = montarPatchReconciliacaoExperimental({
    atual: {
      status: "experimental_agendada",
      cursoId: 9,
      professorId: 12,
      // ID de evento, e nao a aula de aulas_emusys.
      emusysAulaId: 105644,
      emusysLeadId: 8501,
      alunoId: null,
    },
    desejado: {
      ...
      cursoId: 9,
      professorId: 12,
      // ID real da aula fisica retornada pelo Emusys.
      emusysAulaId: 859556,
      emusysLeadId: 8501,
      alunoId: null,
    },
    atualizadoEm: "2026-09-19T13:00:00.000Z",
  });

  assert.equal(resultado.patch.emusys_aula_id, 859556);
});

test("fallback por lead nao cruza experimentais em horarios diferentes", () => {
...
test("webhook nao grava ID de evento como ID de aula fisica", () => {
  ...
  assert.match(source, /emusys_agendamento_id/);
});
