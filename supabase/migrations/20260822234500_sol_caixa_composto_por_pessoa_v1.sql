-- Sol Caixa V3 — composto resolve a PESSOA pelo emusys_student_id.
-- `alunos.id` representa matrícula: uma mesma pessoa pode ter várias matrículas/cursos.
-- O vínculo financeiro de saída, porém, continua sendo a matrícula do item/fatura.
...
begin
  ...
  -- O aluno_id de saída nasce do item escolhido: é a matrícula correta daquela fatura.
  with candidatos as (
    select x
    from jsonb_array_elements(coalesce(v_env->'items','[]'::jsonb)) x
