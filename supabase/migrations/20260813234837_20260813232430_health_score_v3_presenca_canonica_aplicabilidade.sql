...
as $function$
  with candidatos as (
    ...
    select m.professor_id, m.id as snapshot_id, m.config_id,
      ...
      case
        when coalesce(sum(m.peso) filter (
          where m.pilar_aplicavel and m.peso_disponivel and m.nota is not null
        ), 0) > 0
          then round(
            sum(m.nota * m.peso) filter (
              where m.pilar_aplicavel and m.peso_disponivel and m.nota is not null
      ...
      end as score_observado_calculado,
      coalesce(bool_or(
    ...
    select c.*,
      public.avaliar_health_score_professor_v3_comparabilidade(
        coalesce(s.score, c.score_observado_calculado),
        c.cobertura_pilares, c.pilares_validos, c.tem_fidelizacao,
  ...
  select
    ...
    m.ranking_habilitado, m.config_versao, m.revisao,
    coalesce(m.score, a.score_observado_calculado),
    a.cobertura_pilares,
    case
      when (a.avaliacao ->> 'estado') <> 'comparavel' then null::text
      when coalesce(m.score, a.score_observado_calculado) >= a.faixa_saudavel_min then 'saudavel'
      when coalesce(m.score, a.score_observado_calculado) >= a.faixa_atencao_min then 'atencao'
      else 'critico'
    end,
    ...
    case when (a.avaliacao ->> 'estado') = 'comparavel'
      then coalesce(m.score, a.score_observado_calculado)
      else null::numeric
    end,
