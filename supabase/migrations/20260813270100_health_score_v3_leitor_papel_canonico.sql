...
-- Leitor canonico: preserva retratos historicos materializados e corrige campos derivados.
-- Nao promove fechamento nem reescreve snapshots append-only.
...
revoke all on function public.get_health_score_professor_v3_performance_snapshot_v1(date, uuid, text) from public, anon;
grant execute on function public.get_health_score_professor_v3_performance_snapshot_v1(date, uuid, text) to authenticated, service_role;
revoke all on function public.get_health_score_professor_v3_performance_snapshot_v2(date, uuid, text) from public, anon;
grant execute on function public.get_health_score_professor_v3_performance_snapshot_v2(date, uuid, text) to authenticated, service_role;
...
comment on function public.get_health_score_professor_v3_performance_snapshot_v1(date, uuid, text) is
  'Leitor V3 canonico: retratos historicos materializados permanecem visiveis; numero_alunos e diagnostico e cobertura usa cinco pilares pontuaveis.';
comment on function public.get_health_score_professor_v3_performance_snapshot_v2(date, uuid, text) is
  'Compatibilidade V3 delegada ao leitor canonico para manter o mesmo contrato em retratos legados.';
comment on function public.get_health_score_professor_v3_performance_snapshot_v3(date, uuid, text) is
  'Leitor V3 canonico sem fastpath divergente: corrige papel diagnostico e campos derivados sem reescrever snapshots.';
