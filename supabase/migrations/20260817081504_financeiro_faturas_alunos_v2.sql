-- Valores financeiros de uma fatura de aluno.
-- O desconto fixo permanece depois do vencimento; somente o desconto
-- condicional e perdido. Esta funcao e a unica formula usada pelos leitores
-- financeiros do LA Report.
...
begin
  ...
  if v_status = 'aberta' and p_data_vencimento < p_as_of_date then
    v_multa := round(v_valor_sem_desconto_condicional * 0.02, 2);
    v_mora := round(v_valor_sem_desconto_condicional * 0.01 * v_dias_atraso / 30, 2);
...
-- Mantem a assinatura publica da carteira canônica. O corpo anterior continua
-- sendo a fonte de elegibilidade D+0/D+2; este invólucro substitui somente a
-- fórmula monetária por aquela compartilhada com a página financeira.
do $do$
begin
  if to_regprocedure('public.get_inadimplencia_canonica_v3_base(uuid,date)') is null
     and to_regprocedure('public.get_inadimplencia_canonica(uuid,date)') is not null then
    execute 'alter function public.get_inadimplencia_canonica(uuid, date) rename to get_inadimplencia_canonica_v3_base';
  end if;
end;
$do$;
...
begin
  ...
  with itens as (
    ...
    from jsonb_array_elements(coalesce(v_base -> 'items', '[]'::jsonb)) with ordinality as linhas(item, ordinality)
  ),
  recalculados as (
    select
      itens.ordinality,
      itens.item || jsonb_build_object(
    ...
    left join lateral (
      ...
      where i.unidade_id = (itens.item ->> 'unidade_id')::uuid
        and i.canonical_fatura_id::text = itens.item ->> 'canonical_fatura_id'
        and i.emusys_fatura_id::text = itens.item ->> 'emusys_fatura_id'
        and i.competencia = (itens.item ->> 'competencia')::date
      limit 1
    ) snapshot on true
    cross join lateral (
      select public.calcular_valores_fatura_financeiro_v1(
        (itens.item ->> 'valor_original')::numeric,
        snapshot.desconto_fixo,
        coalesce(
          nullif(itens.item ->> 'desconto_condicional_perdido', '')::numeric,
          nullif(itens.item ->> 'desconto_condicional', '')::numeric,
          0
        ),
        (itens.item ->> 'data_vencimento')::date,
        coalesce(itens.item ->> 'status', 'aberta'),
        p_as_of_date
      ) as calculo
    ) valores
  )
  select
    ...
    coalesce(sum((item ->> 'valor_atualizado')::numeric), 0)
  into v_items, v_total_atualizado
  from recalculados;

  v_base := jsonb_set(v_base, '{items}', v_items, true);
  v_base := jsonb_set(v_base, '{totals,total_atualizado}', to_jsonb(v_total_atualizado), true);
  return v_base;
end;
$function$;

revoke all on function public.get_inadimplencia_canonica_v3_base(uuid, date)
  from public, anon, authenticated;
...
-- Leitura global para a tela de Faturas de Alunos. Ela nunca consulta o
-- Emusys pelo navegador: toda a resposta vem do ultimo snapshot completo de
-- cada competencia. A carteira D+2, por sua vez, e derivada da mesma leitura
-- canonica que atende a Sol.
