...
-- Migration consolidada (estado final) para versionar em supabase/migrations/.
-- Tudo é ADITIVO (prefixo banda_*). Nenhum objeto existente é alterado.
--
-- Inclui as correções da auditoria de 2026-08-24:
--   #1 roster considera status canônico ('ativo'), não só is_ex_aluno  -> banda_aluno_ativo
--   #2 anon/PUBLIC sem EXECUTE; só authenticated + service_role        -> bloco de grants no fim
--   #3 (capacidade removida — o alerta certo é por função faltante: baterista/baixista/etc., fase 2)
--   #6 RPCs de resolução (desativar integrante, editar/remover repertório, editar/cancelar evento)
--   #9 CHECKs de domínio (tipo/status)
--
-- Padrão: RLS ligada SEM policy (fail-closed). Acesso só via RPC SECURITY DEFINER.
-- Front NUNCA faz supabase.from('banda') — só supabase.rpc(...).
-- =============================================================================
...
comment on table public.banda_curso_depara is 'De-para curado de quais cursos contam como banda. Emusys nao tem conceito de banda.';

create table if not exists public.banda (
  id bigserial primary key,
  unidade_id uuid not null,
  curso_id integer not null,
  turma_chave text not null unique,
  dia_semana text,
  horario time,
...
comment on table public.banda is 'Identidade da banda por cima da turma canonica. Roster/professor/horario derivam do canonico via RPC; aqui fica so identidade + ancora (turma_chave).';
comment on column public.banda.turma_chave is 'unidade_id|curso_id|dia_normalizado(sem -feira)|horario|professor_atual_id.';
...
comment on table public.banda_integrante is 'Overlay por aluno (instrumento/funcao). Roster BASE vem do canonico; aluno_id -> public.alunos.id (sem FK rigida).';
...
comment on table public.banda_repertorio is 'Repertorio da banda. letra/cifra alimentados pela integracao Cifra Club (edge function).';
...
CREATE OR REPLACE FUNCTION public.bandas_listar(p_unidade_id uuid DEFAULT NULL::uuid, p_status text DEFAULT NULL::text)
 RETURNS TABLE(banda_id bigint, nome text, unidade_id uuid, unidade_nome text, curso_id integer, curso_nome text, dia_semana text, horario time without time zone, produtor_nome text, integrantes integer, precisa_revisar_nome boolean, status text, proximo_evento timestamp with time zone)
 LANGUAGE sql
...
AS $function$
  ...
  select b.id, b.nome, b.unidade_id, u.nome, b.curso_id, c.nome, b.dia_semana, b.horario, p.nome,
         coalesce(r.n,0), b.precisa_revisar_nome, b.status,
...
CREATE OR REPLACE FUNCTION public.banda_integrantes(p_banda_id bigint)
 RETURNS TABLE(aluno_id integer, nome text, instrumento text, funcao text, status_aluno text, tempo_permanencia_meses integer, saiu_da_escola boolean, responsavel_nome text, responsavel_telefone text, whatsapp text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  select al.id, al.nome, i.instrumento_na_banda, i.funcao, al.status, al.tempo_permanencia_meses,
         coalesce(al.is_ex_aluno,false), al.responsavel_nome, al.responsavel_telefone, al.whatsapp
  from public.alunos al
  left join public.banda_integrante i on i.banda_id=p_banda_id and i.aluno_id=al.id
  where public.banda_aluno_ativo(al.status, al.is_ex_aluno)
    and public.banda_chave_turma(al.unidade_id, al.curso_id, al.dia_aula, al.horario_aula, al.professor_atual_id)
        = (select turma_chave from public.banda where id=p_banda_id)
  order by al.nome;
$function$;

CREATE OR REPLACE FUNCTION public.banda_detalhe(p_banda_id bigint)
 RETURNS TABLE(banda_id bigint, nome text, unidade_id uuid, unidade_nome text, curso_id integer, curso_nome text, dia_semana text, horario time without time zone, produtor_nome text, genero text, descricao text, logo_url text, status text, precisa_revisar_nome boolean, integrantes integer, musicas integer, proximos_eventos integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  select b.id, b.nome, b.unidade_id, u.nome, b.curso_id, c.nome, b.dia_semana, b.horario, p.nome,
         b.genero, b.descricao, b.logo_url, b.status, b.precisa_revisar_nome,
         (select count(*)::int from public.alunos al where public.banda_aluno_ativo(al.status, al.is_ex_aluno) and public.banda_chave_turma(al.unidade_id,al.curso_id,al.dia_aula,al.horario_aula,al.professor_atual_id)=b.turma_chave),
...
CREATE OR REPLACE FUNCTION public.bandas_kpis(p_unidade_id uuid DEFAULT NULL::uuid, p_min_integrantes integer DEFAULT 3)
...
AS $function$
  with alu as (
    select al.unidade_id,
           ...
           al.tempo_permanencia_meses as perm,
  ...
  banda_vaga as (
    select b.unidade_id, count(*)::int total, count(*) filter (where coalesce(bc.n,0) < p_min_integrantes)::int com_vaga
    from public.banda b left join band_cnt bc on bc.tc=b.turma_chave
  ...
  alu_agg as (
    ...
    from alu group by unidade_id
  )
  select u.id, u.nome, coalesce(bv.total,0), coalesce(aa.alunos,0), aa.perm_media, coalesce(bv.com_vaga,0)
  from public.unidades u
  left join banda_vaga bv on bv.unidade_id=u.id
  left join alu_agg aa on aa.unidade_id=u.id
  where (p_unidade_id is null or u.id=p_unidade_id)
  order by u.nome;
$function$;

CREATE OR REPLACE FUNCTION public.bandas_para_garimpar(p_unidade_id uuid DEFAULT NULL::uuid, p_min_integrantes integer DEFAULT 3)
...
-- (CRUD completo) arquivar/reativar banda, excluir evento, remover overlay de integrante
...
-- -----------------------------------------------------------------------------
-- 5) GRANTS (auditoria #2): sem PUBLIC/anon; só authenticated + service_role
-- -----------------------------------------------------------------------------
do $$
declare r record;
begin
...
-- -----------------------------------------------------------------------------
-- 6) SEED (idempotente) — de-para + capacidade + popular bandas do canônico
-- -----------------------------------------------------------------------------
