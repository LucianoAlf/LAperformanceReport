-- 2026-08-11 — Filtra alunos evadidos/trancados da grade da chamada.
--
-- Problema: a grade (aula_alunos_emusys) e upsert-only — quando o Emusys remove
-- um aluno da aula (trancamento, evasao), a nossa grade nao remove. Alunos
-- evadidos apareciam na Chamada como "sem destino", gerando cobranca indevida.
-- Casos reais: Rafael Gusmao (evadido 04/08), Claudia Fonseca (evadida 05/08).
--
-- Solucao: a fonte 2 (grade sem presenca) filtra alunos com status evadido/trancado.
-- A fonte 1 (aluno_presenca) NAO filtra — se o aluno tem presenca registrada,
-- ele aparece para historico mesmo se evadido depois.
...
vinculos_cru as (
  -- Fonte 1: alunos com presenca registrada (inclusive evadidos — historico)
  select b.chave,
         ap.aluno_id,
  ...
  union all
  -- Fonte 2: grade (aula_alunos_emusys) — FILTRA evadidos/trancados sem presenca
  select b.chave,
         aa.aluno_id,
  ...
  where not exists (
    ...
    where al_filter.id = aa.aluno_id
      and al_filter.status in ('evadido', 'trancado')
  )
),
vinculos_ident as (
  select x.*,
