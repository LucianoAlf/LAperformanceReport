...
begin
  ...
  select jsonb_build_object(
    ...
    'responsaveis', coalesce((
      select jsonb_agg(jsonb_build_object('nome', c.nome, 'parentesco', c.parentesco, 'principal', c.is_principal))
    ...
    -- HISTORICO: anotacoes das aulas, dedup por data, SO dos cursos deste professor.
    -- FIX: o limit 10 agora corta as ENTRADAS (antes cortava a linha do agregado = trazia tudo).
