...
with evidencia as not materialized (
  ...
  from base b
), snapshot_roster as not materialized (
  select
    slots.slot_key,
    slots.aluno_id,
    slots.unidade_id,
    slots.data_aula,
    count(distinct ae2.id) as qtd_roster_ativo,
    count(distinct n2.aula_emusys_id)
      filter (where n2.emusys_presenca_normalizada is not null) as qtd_presencas_recebidas
  from (
    select distinct
      slot_key,
      aluno_id,
      ...
      curso_nome
    from normalizada
    where identidade_completa
  ) slots
  left join public.aula_alunos_emusys aa
    on aa.unidade_id = slots.unidade_id
   and aa.aluno_id = slots.aluno_id
   and aa.ativo_operacional
  left join public.aulas_emusys ae2
    on ae2.id = aa.aula_emusys_id
   and ae2.unidade_id = slots.unidade_id
   and ae2.data_aula = slots.data_aula
  ...
  left join normalizada n2
    on n2.slot_key = slots.slot_key
   and n2.aluno_id = slots.aluno_id
   and n2.unidade_id = slots.unidade_id
   and n2.data_aula = slots.data_aula
  group by slots.slot_key, slots.aluno_id, slots.unidade_id, slots.data_aula
), normalizada_roster as not materialized (
  select
    n.*,
    (
      coalesce(r.qtd_roster_ativo, 0) > 0
      and coalesce(r.qtd_roster_ativo, 0) = coalesce(r.qtd_presencas_recebidas, 0)
    ) as emusys_snapshot_roster_completo
  from normalizada n
  left join snapshot_roster r
    on r.slot_key = n.slot_key
   and r.aluno_id = n.aluno_id
   and r.unidade_id = n.unidade_id
   and r.data_aula = n.data_aula
  ...
  select
    ...
    ('presenca-ocorrencia-canonica-v2.5'
...
comment on view public.vw_presenca_ocorrencia_canonica_v2 is
  'Read model v2.5: raw Emusys ausente nao e terminal; gemeas presente + ausente nao geram conflito; positiva so contradiz falta humana quando o snapshot publicavel de presenca e coerente, atual, vinculado a roster ativo e dentro da janela de execucao. Experimental segue contrato proprio.';
