...
-- - uma presenca so contradiz falta humana quando o snapshot positivo e
--   coerente entre as gemeas, pertence a uma execucao concluida e seus
--   registros estao dentro da janela dessa execucao.
...
with evidencia as not materialized (
        ...
        and (
          ...
          or (
            count(distinct c.aula_emusys_id)
              filter (where c.aula_emusys_id is not null) <= 1
            or (
              count(*) filter (
                where c.emusys_presenca_normalizada is not null
              ) = count(distinct c.aula_emusys_id)
                filter (where c.aula_emusys_id is not null)
              and count(*) filter (
                where c.emusys_presenca_normalizada <> 'presente'
              ) = 0
              and count(distinct c.emusys_snapshot_run_id)
                filter (where c.emusys_presenca_normalizada is not null) = 1
              and count(*) filter (
                where c.emusys_presenca_normalizada is not null
  ...
  select
    ...
    ('presenca-ocorrencia-canonica-v2.3'
...
comment on view public.vw_presenca_ocorrencia_canonica_v2 is
  'Read model v2.3: raw Emusys ausente nao e terminal; gemeas presente + ausente nao geram conflito; positiva so contradiz falta humana quando o snapshot e coerente, concluido, atual e dentro da janela de execucao. Experimental segue contrato proprio.';
