...
-- Relaxar anti-duplicata de telefone para leads vindos do Emusys com emusys_lead_id
-- Problema: irmãos (leads diferentes, mesmo telefone do responsável) ficavam sem telefone
-- porque a proteção anti-duplicata bloqueava o UPDATE quando outro lead já tinha o mesmo número.
-- Fix: se source é emusys E tem source_id (emusys_lead_id), confia no CRM e permite telefone compartilhado.
...
BEGIN
  ...
  IF v_lead_id IS NOT NULL THEN
    ...
    IF p_telefone IS NOT NULL THEN
      -- Emusys com emusys_lead_id: confia no CRM, permite telefone compartilhado (irmãos)
      IF p_source_type = 'emusys' AND p_source_id IS NOT NULL THEN
        v_telefone_safe := p_telefone;
      ELSIF NOT EXISTS (SELECT 1 FROM leads WHERE telefone = p_telefone AND unidade_id = p_unidade_id AND id != v_lead_id AND arquivado = false) THEN
        v_telefone_safe := p_telefone;
      END IF;
    END IF;
