...
-- 017: trabalho_feito ganha a MESMA cascata de fallback do resumo.
-- Motivo: registros em formato antigo (texto consolidado / anotações, sem campos
-- estruturados) faziam `ultima_aula` vir só com `data` — e o Fábio anunciava
-- "Sem conteúdo registrado" para um aluno que TEM conteúdo (caso Amanda, 03/08).
-- Conserto na fonte: quem consome `ultima_aula` nunca mais perde conteúdo legado.
