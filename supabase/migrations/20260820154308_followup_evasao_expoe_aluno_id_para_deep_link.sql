-- A fila de follow-up mostra a conversa da PESQUISA (so o que a familia respondeu). Quando ninguem
-- respondeu — que e o caso dos 25 abertos hoje — o painel fica vazio e nao da para distinguir
-- "nao respondeu" de "algo quebrou". O caminho para conferir e a Caixa de Entrada do Sucesso do
-- Aluno, que ja tem a conversa real: os 25 casos abertos tem `aluno_id` na movimentacao E conversa
-- em `admin_conversas` no departamento 'sucesso_aluno' (caixa 3, "Lia - Sucesso do Aluno").
--
-- A tela ja sabe abrir a Caixa por aluno (`abrirConversaAluno` em SucessoClientePage), mas a RPC da
-- fila nao devolvia `aluno_id`. Esta migration expoe o campo; o botao no front usa esse mesmo
-- caminho.
--
-- DROP + CREATE porque muda o RETURNS TABLE (nao da para CREATE OR REPLACE). Por isso:
--   * a ordem importa — `listar_...` faz `select count(*) over (), estado.*`, entao as duas
--     assinaturas tem que casar coluna a coluna;
--   * `contar_followups_pesquisa_evasao_v1` tambem chama a funcao de estado, mas so conta linhas —
--     nao precisa ser recriada;
--   * recriar funcao REABRE execute para `anon` neste projeto (ha ALTER DEFAULT PRIVILEGES no
--     schema public), entao o revoke/grant no fim nao e decorativo. ACL alvo:
--     {postgres, authenticated, service_role} — a mesma de antes.
...
returns table(total_count bigint, pesquisa_id uuid, evasao_id integer, aluno_id integer, aluno_nome text, unidade_id uuid, unidade_nome text, enviado_em timestamp with time zone, vencido_em timestamp with time zone, operador_usuario_id integer, operador_nome text, estado_visivel text, followup_pendente boolean, interagiu_sem_resposta_valida boolean, alerta_enviado_em timestamp with time zone, acao text, acao_canal text, acao_observacao text, acao_registrada_em timestamp with time zone, acao_operador_nome text)
language plpgsql
stable security definer
set search_path to 'public', 'pg_temp'
as $function$
