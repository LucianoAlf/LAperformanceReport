...
--
-- EFEITO MEDIDO (ciclo 2026-JUN-AGO): vinculos em revisao 11 -> 5 no consolidado
-- (**Recreio zerou**, Barra 3 -> 1, CG 7 -> 4), expostos 1.375 -> 1.380, estado `ok` 27 -> 33
-- de 44. `encerramentos_penalizadores` seguiu 114 -- ninguem penalizado, como em todo o dia.
--
-- ⚠️ A regra (A) NAO virou automacao ainda, de proposito. A primeira tentativa de generalizar
-- ("periodo orfao cujo aluno tem outra jornada ativa") pega **524 periodos**, 96 deles com o
-- mesmo professor -- numero grande demais para ser so troca de curso, porque "tem outra jornada
-- ativa" tambem acontece com quem faz DOIS instrumentos ao mesmo tempo. Generalizar exige
-- separar as duas coisas (provavelmente exigindo que a jornada antiga tenha encerrado no mesmo
-- momento em que a nova comecou). Fica como proxima frente, ja medido.
do $mig$
declare
...
begin
  ...
  select p.id, p.reconstrucao_id, 'corrigido',
         'Curadoria da unidade Barra (10/08/2026): a troca foi feita no sabado e esta correta. '
  ...
  select p.id, p.reconstrucao_id, 'rejeitado',
         ...
         || 'Alexandre; devido a escala de embarcacoes dele, o Jereh autorizou algumas '
         || 'reposicoes dessa aula com o Israel. Reposicao autorizada NAO cria vinculo de '
