-- Acrescenta g.data_aula = a.data_aula no filtro de aula gemea cancelada/justificada.
-- Logicamente redundante (a gemea tem o mesmo data_hora_inicio; medido: 0 divergencias em
-- 65.715 aulas entre data_aula e a data derivada de data_hora_inicio), mas e o que permite
-- o indice (unidade_id, data_aula) ser usado NESTE contexto.
--
-- ⚠️ ESPECIFICO DESTA FUNCAO. Medido em 17/09 nas 3 funcoes de presenca que ja tinham o
-- filtro (LAPE-37): a mesma mudanca PIORA todas elas em 3-24%, porque la o planner ja
-- escolhe Hash Join sozinho. Aqui o contexto e outro (CTE materializada, universo vindo
-- da uniao roster+presenca) e o plano cai em Nested Loop com Bitmap por linha.
-- Nao replicar sem medir.
...
begin
  ...
  with alvo as materialized (
    ...
    from (
      ...
      union all
      -- rastro da reconciliacao de grade: ela remove o vinculo e preserva a presenca.
      -- Sem este lado perdem-se 17 pendencias reais so em agosto (12 na Barra).
