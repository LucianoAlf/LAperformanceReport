-- Emissao de token para colaborador ja existente no painel Time.
-- O colaborador e o departamento sao a fonte de verdade; o cliente nao escolhe cargo.

create unique index if not exists uq_ficha_tokens_colaborador_ativo
...
begin
  ...
  where t.colaborador_id = p_colaborador_id
    and t.ativo
  ...
  if found then
    return query
    select v_token.token,
           v_cargo,
           true,
           v_token.usado_em is not null,
           v_token.criado_em;
    return;
  end if;
