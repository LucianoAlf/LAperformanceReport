-- ============================================================================
-- P09K - Compatibilidade dados_mensais: respeitar colunas geradas
-- Projeto alvo: ouqwbbermlzqqvtqwlul
--
-- Contexto:
-- - dados_mensais.faturamento_estimado e dados_mensais.saldo_liquido sao
--   colunas geradas pelo Postgres e nao aceitam INSERT/UPDATE explicito.
-- - A P09J corrigiu a leitura de retencao/reajuste, mas a gravacao ainda
--   passava essas colunas geradas.
--
-- Nao faz:
-- - Nao chama a funcao.
    ...
    IF NOT p_dry_run THEN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'colunas_geradas_respeitadas', jsonb_build_array('faturamento_estimado', 'saldo_liquido'),
          'valores', v_valores
        ),
        auth.uid()
      );
...
COMMENT ON FUNCTION public.atualizar_dados_mensais_por_snapshot(integer, integer, uuid, boolean) IS
  'Atualiza dados_mensais de compatibilidade usando snapshots aprovados/fechados; respeita colunas geradas. Dry-run por padrao; restrita a service_role.';
