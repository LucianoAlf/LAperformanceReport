-- ============================================================================
-- P09H - Relatorio gerencial: MRR perdido canonico de retencao
-- Projeto alvo: ouqwbbermlzqqvtqwlul
--
-- Objetivo:
-- - Manter o wrapper atual de get_dados_relatorio_gerencial.
-- - Corrigir somente kpis_retencao[].mrr_perdido para usar a mesma fonte
--   operacional canonica da pagina administrativa:
--   movimentacoes_admin, tipos evasao + nao_renovacao, com exclusao de
--   atividades extras/bandas/coral.
--
-- Nao faz:
-- - Nao altera regras de status de aluno.
-- - Nao mexe em Health Score/professores.
-- - Nao grava dados historicos nem snapshots.
-- ============================================================================
...
begin
  ...
  retencao_patch AS (
    SELECT jsonb_agg(
      CASE
        WHEN elem ? 'unidade_id' THEN
          jsonb_set(
            elem,
            '{mrr_perdido}',
            to_jsonb(coalesce(m.mrr_perdido, 0)),
            true
          )
        ELSE elem
      END
    ...
    LEFT JOIN mrr_canonico m ON m.unidade_id::text = elem->>'unidade_id'
  )
  SELECT coalesce(arr, coalesce(v_result->'kpis_retencao', '[]'::jsonb))
    INTO v_kpis_retencao
  FROM retencao_patch;
  ...
  v_result := jsonb_set(
    ...
    to_jsonb('p09h_movimentacoes_admin_retencao_canonica'::text),
    true
  );
...
COMMENT ON FUNCTION public.get_dados_relatorio_gerencial(uuid, integer, integer) IS
  'P09H: wrapper gerencial com comercial canonico P02V e mrr_perdido de retencao calculado por movimentacoes_admin filtrando atividades extras.';
