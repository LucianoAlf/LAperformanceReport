-- O leitor de inadimplencia so exige frescor para faturas que ja venceram.
-- Competencias futuras nao podem bloquear uma fotografia de atraso passada.
...
begin
  ...
  alunos_ativos as (
    select distinct
           a.unidade_id,
           nullif(btrim(a.emusys_matricula_id), '') as emusys_matricula_id,
           nullif(btrim(a.emusys_student_id), '') as emusys_student_id
    from public.alunos a
    where lower(trim(a.status)) = 'ativo'
    ...
    cross join janela_competencias jc
     where i.competencia between jc.inicio and jc.fim
       ...
       and exists (
         ...
         where aa.unidade_id = i.unidade_id
           and (
             (
               i.emusys_matricula_id is not null
               and aa.emusys_matricula_id = i.emusys_matricula_id::text
             )
             or (
               i.emusys_matricula_id is null
               and i.emusys_student_id is not null
               and aa.emusys_student_id = i.emusys_student_id::text
             )
           )
       )
  ),
  ...
  integridade_canonica as (
    select lr.canonical_fatura_id,
           ...
           bool_or(jsonb_array_length(lr.validation_issues) > 0) as tem_identidade_invalida,
  ...
  itens_identidade_invalida as (
    ...
    from (
      ...
      from linhas_relevantes lr
      where jsonb_array_length(lr.validation_issues) > 0
    ) ranked
    where ranked.rn = 1
  ),
  itens_confirmados as (
    ...
    from (
      ...
      where lr.is_fresh
        ...
        and ic.tem_identidade_invalida is false
    ) ranked
    where ranked.rn = 1
  ),
  resumo_itens as (
  ...
  resumo_integridade as (
    select
      ...
      coalesce((
        select sum(jsonb_array_length(i.validation_issues))::integer
        from itens_identidade_invalida i
      ), 0) as validation_issue_count
  )
  select jsonb_build_object(
    'schema_version', 2,
    'policy', jsonb_build_object(
      'student_scope', 'alunos.status=ativo AND arquivado_em IS NULL AND data_saida IS NULL',
    ...
    'status', case
      ...
      when rint.validation_issue_count > 0 then 'incomplete'
      else 'ok'
    end,
    ...
    'reconciliation', jsonb_build_object(
      'status', case
        when ri.source_missing_count > 0
          or rint.duplicate_fatura_count > 0
          or rint.validation_issue_count > 0 then 'pending'
        else 'clear'
      end,
      'source_missing_count', ri.source_missing_count,
      'duplicate_fatura_count', rint.duplicate_fatura_count,
      'invalid_identity_invoice_count', rint.invalid_identity_invoice_count,
      'validation_issue_count', rint.validation_issue_count,
      'duplicate_invoices', coalesce((
        select jsonb_agg(jsonb_build_object(
          ...
          'tem_identidade_invalida', d.tem_identidade_invalida
    ...
    'totals', case
      when rf.competencias_stale > 0 or rint.validation_issue_count > 0 then jsonb_build_object(
        'total_faturas', 0,
        'total_matriculas', 0,
        'total_original', 0,
        'total_atualizado', 0,
    ...
    'items', case
      when rf.competencias_stale > 0 or rint.validation_issue_count > 0 then '[]'::jsonb
      else coalesce((
        select jsonb_agg(jsonb_build_object(
          'canonical_fatura_id', i.canonical_fatura_id,
          'unidade_id', i.unidade_id,
...
comment on function public.get_inadimplencia_canonica(uuid, date) is
  'Canonical delinquency read with freshness gate, global dedupe, identity quarantine and contract interest.';
