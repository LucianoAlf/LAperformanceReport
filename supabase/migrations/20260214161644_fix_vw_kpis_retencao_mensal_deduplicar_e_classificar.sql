...
-- Corrigir vw_kpis_retencao_mensal:
-- 1. Deduplicar evasoes_v2 por aluno_id (pegar a mais recente se duplicada)
-- 2. Classificar cancelamentos vs não-renovações pelo tipo_saida_id (não pattern matching)
-- 3. Não-renovações vêm da evasoes_v2 tipo_saida=2, não da tabela renovacoes

CREATE OR REPLACE VIEW vw_kpis_retencao_mensal AS
WITH evasoes_dedup AS (
  -- Deduplicar por aluno: se mesmo aluno aparece 2x no mesmo mês, pegar a mais recente
...
evasoes_mes AS (
  SELECT e.unidade_id,
    ...
    -- tipo_saida_id: 1=Interrompido (cancelamento), 2=Não Renovou, 3=Aviso Prévio, 4=Transferência
...
total_alunos AS (
  SELECT unidade_id, count(*) AS total_ativos
  FROM alunos
  WHERE status = 'ativo'
  GROUP BY unidade_id
)
SELECT u.id AS unidade_id,
  ...
  CASE WHEN COALESCE(ta.total_ativos, 0) > 0
    THEN ((COALESCE(em.total_evasoes, 0) - COALESCE(em.transferencias, 0))::numeric / ta.total_ativos::numeric * 100)::numeric(5,2)
    ELSE 0 END AS taxa_evasao,
  COALESCE(em.mrr_perdido, 0)::numeric(12,2) AS mrr_perdido,
  -- Renovações previstas = renovações realizadas + não-renovações (da tabela renovacoes) + pendentes
  -- Mas se não-renovações estão na evasoes_v2, usar o maior entre as duas fontes
  ...
  -- Não-renovações: usar evasoes_v2 tipo_saida=2 como fonte primária
