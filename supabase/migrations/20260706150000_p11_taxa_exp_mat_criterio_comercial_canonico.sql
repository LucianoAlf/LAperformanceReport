-- P11: corrige o numerador da taxa Experimental -> Matricula p/ usar o MESMO
-- criterio comercial canonico ja aplicado em "Matriculas no periodo"
-- (relatorio-admin-whatsapp: ehMatriculaComercialCanonicaEdge).
--
-- Problema (validado com Clayton, comercial): o numerador de
-- conversoes_exp_mat_canonicas hoje conta LINHA (nao pessoa) e nao confere
-- se a matricula caiu na mesma competencia nem se ela e uma matricula
-- comercial de verdade (passaporte pago, sem ser 2o curso/banda/coral/bolsista).
-- Isso inflou a taxa do Recreio em 04/07 (mostrou 100%/133% quando o real,
-- batendo com "Matriculas no periodo", era 33.3% = 2/6).
--
-- Correcao: no numerador, exige por pessoa (nao por linha):
--   - matricula com data_matricula DENTRO da competencia medida
--   - valor_passaporte > 0
--   - nao e 2o curso, banda/coral, nem tipo BOLSISTA_INT/BOLSISTA_PARC/BANDA/
--     SEGUNDO_CURSO/TRANSFERENCIA
--   - decisao humana de conciliacao (contar_conversao_exp_mat) continua
--     valendo direto, sem reaplicar o filtro (ja foi revisada por humano)
--
-- Escopo: NAO reabre meses fechados. A regra nova so vale para periodos
-- com inicio >= 2026-07-01 (regra_nova_p11); meses anteriores mantem o
-- calculo antigo intacto (importante pq essa RPC tambem alimenta a
-- pontuacao/bonus do Matriculador+ em useMatriculadorPrograma.ts).
--
-- Afeta: relatorio-admin-whatsapp (funil), ComercialPage.tsx,
-- ComercialConciliacaoExperimentais.tsx, useMatriculadorPrograma.ts (Matriculador+).
-- O denominador (experimentais realizadas confirmadas) NAO muda.
...
comment on function public.get_conciliacao_experimentais_v2(uuid, integer, integer, text, date) is
  'Conciliacao v2 de experimentais (P11). Numerador da taxa exp->mat exige pessoa unica com passaporte + matricula comercial canonica na mesma competencia (a partir de 2026-07-01); meses anteriores mantem o calculo legado para nao reabrir pontuacao ja fechada do Matriculador+.';
