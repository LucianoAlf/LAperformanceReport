-- ⛔ NÃO APLICADA EM PRODUÇÃO. Reconciliação da cadeia do caixa — parte 3/3.
--
-- 🔴 A FUNÇÃO QUE RESOLVE `aluno.id`. O composto filtra por
--    `nullif(x->'aluno'->>'id','') ~ '^[0-9]+$'` — sem esse campo ele descarta
--    TODAS as faturas e recusa com `composicao_exige_duas_faturas` mesmo com 4
--    na competência. Quem monta o objeto do aluno é esta canônica, e ela era a
--    peça que faltava no manifesto: eu tinha delimitado a cadeia por quem
--    aparece nas chamadas diretas, e quem resolve o aluno ficou de fora.
--
--    Medido: produção 28.255 bytes · container 25.854. A versão do container é
--    ANTERIOR ao ramo `fallback_cadastro_pre_espelho` (ledger 20260820002814),
--    que é justamente quem lê `public.alunos` direto quando o espelho canônico
--    ainda não materializou a matrícula. Sem esse ramo, `aluno.id` só sai da
--    view canônica.
--
-- ⚠️ POR QUE NÃO DÁ PARA REPRODUZIR PELO REPO: nenhuma migration do ledger
--    contém `create or replace function ... _canonica_20260817`. As cinco
--    últimas que a tocam são TODAS patch-style (`pg_get_functiondef` +
--    `replace` + `execute`) — a mais recente é 20260908135918
--    (`alunos_financeiro_leitura_unica`). Patch guarda a transformação, não o
--    corpo resultante: aplicado sobre outro estado inicial, produz outra coisa.
--    É a dívida estrutural que esta frente encontrou, e esta migration é o
--    remédio para ESTA função.
--
-- Mesmo método das partes 1 e 2: gerador em SQL, SELECT-only, corpo final
-- materializado do catálogo vivo, sem edição. A pós-condição confere o md5 do
-- corpo instalado contra o de produção.
--
-- ⚠️ A ACL DESTA É DIFERENTE DAS OUTRAS, de propósito: ela É executável por
--    `authenticated` (a tela do financeiro a consome), e por isso a
--    pós-condição NÃO exige ausência de `authenticated` aqui — exigir seria
--    quebrar a tela para "cumprir" uma regra que não é dela.

\set ON_ERROR_STOP on

-- ═══════════════════════════════════════════════════════════════════════════
-- get_faturas_alunos_financeiro_v1_canonica_20260817
-- procedência: sem `create or replace` no ledger; último patch conhecido é
--              20260908135918 (alunos_financeiro_leitura_unica)
-- hash do corpo vivo: 97fefb3d7f8826f92ceac09c8f2c97d7
-- ═══════════════════════════════════════════════════════════════════════════
...
declare
  ...
  v_esp text := '97fefb3d7f8826f92ceac09c8f2c97d7';
  v_obt text;
  v_falhas text[] := '{}';
begin
  ...
  if v_obt is distinct from v_esp then
    v_falhas := v_falhas || format('corpo %s, esperado %s', left(coalesce(v_obt,'<nulo>'),12), left(v_esp,12));
  end if;
  ...
  -- o ramo que resolve aluno.id sem o espelho canonico
  if pg_get_functiondef(v_sig::regprocedure) not like '%fallback_cadastro_pre_espelho%' then
    v_falhas := v_falhas || 'o ramo fallback_cadastro_pre_espelho sumiu — aluno.id volta a depender so do espelho';
  end if;

  if array_length(v_falhas,1) > 0 then
    raise exception E'RECONCILIACAO DA CANONICA NAO FECHOU:\n  %', array_to_string(v_falhas, E'\n  ');
  end if;
  raise notice 'reconciliacao 3/3 ok: canonica, corpo + ACL + ramo de fallback conferidos';
end $pos$;
