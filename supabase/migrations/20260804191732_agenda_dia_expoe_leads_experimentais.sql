-- Agenda: expõe quem vai na aula EXPERIMENTAL e a observação do atendimento.
--
-- Problema: `aula_alunos_emusys` não tem NENHUMA linha para aula experimental
-- (medido: 0 de 0), então a Agenda mostrava essas aulas sem saber quem viria.
-- E a `observacoes` da experimental — que chega em 100% dos payloads do Emusys e
-- é preenchida em ~64% — vive em `lead_experimentais`, tabela que a RPC não tocava.
--
-- Vínculo: (unidade, data, horário BRT). NÃO dá para usar `emusys_aula_id` porque
-- esse campo guarda o id da ENTREGA do webhook (faixa 72k-73k), não o id da aula
-- (faixa 254k-830k) — nenhum dos dois casa com `aulas_emusys.emusys_id`.
--
-- Aditivo: coluna nova no fim do RETURNS TABLE. `alunos` e `qtd_alunos` NÃO mudam,
-- de propósito — um lead não é aluno, e somá-lo em qtd_alunos distorceria a lotação
-- (a Beatriz Romero tem 2 experimentais no mesmo horário e viraria "2 alunos").
--
-- ⚠️ `lead_experimentais` tem RLS com policy `USING (true)` para `authenticated` e
-- GRANT de leitura — conferido ANTES de escrever este JOIN, porque a RPC é
-- SECURITY INVOKER e adicionar tabela sem acesso do usuário já quebrou a Agenda
-- duas vezes (aula_alunos_emusys em 02/08, emusys_disciplinas_catalogo em 03/08).
--
-- Validado com `set local role authenticated` + JWT real nos 3 perfis (admin,
-- unidade, professor): 179 aulas em cada, 179 linhas = 179 chaves distintas
-- (o JOIN não multiplica), zero permission denied.

drop function if exists public.get_agenda_dia(date, uuid);

create function public.get_agenda_dia(p_data date, p_unidade_id uuid default null::uuid)
returns table(
  chave text, unidade_id uuid, unidade_nome text, professor_nome text, professor_id integer,
  sala_nome text, curso_nome text, turma_nome text, hora_inicio text, hora_fim text,
  duracao_minutos integer, categoria text, tipo text, cancelada boolean, justificada boolean,
  reagendada boolean, hora_original text, nr_da_aula integer, qtd_aulas_contrato integer,
  qtd_alunos integer, anotacoes text, anotacoes_fabio text, professor_presenca text,
  alunos jsonb, experimental_leads jsonb
)
language sql
stable
...
-- Leads da aula experimental. Agregado por chave para NUNCA multiplicar a aula:
-- um mesmo horário pode ter 2 linhas (aluno com 2 cursos no mesmo slot).
experimentais as (
  select b.chave,
...
-- ⚠️ DROP+CREATE reabre EXECUTE para `anon` (ALTER DEFAULT PRIVILEGES do projeto).
-- `revoke from public` NÃO basta: precisa ser nominal em anon.
