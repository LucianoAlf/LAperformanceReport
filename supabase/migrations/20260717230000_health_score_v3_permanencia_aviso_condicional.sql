-- Health Score Professor V3 - aviso de parcialidade somente quando aplicavel.
-- O diagnostico permanece transparente sem rotular um KPI completo como parcial.
...
select
  ...
  case
    ...
    when coalesce(x.elegiveis_nao_publicaveis, 0) > 0 then null
  ...
  case
    ...
    when coalesce(x.elegiveis_nao_publicaveis, 0) > 0
      then 'aguardando_revisao_historica'
  ...
  (
    ...
    and coalesce(x.elegiveis_nao_publicaveis, 0) = 0
  ) as publicavel,
  case
    when coalesce(e.amostra, 0) = 0 then 'sem_base'
    when coalesce(c.historico_incompleto, false) then 'parcial'
    when coalesce(x.elegiveis_nao_publicaveis, 0) > 0 then 'parcial'
    when e.amostra < 3 then 'baixa_amostra'
    else 'alta'
  end as confianca,
  'vw_professor_periodos_efetivos_v3_sombra'::text as fonte,
  'health-score-professor-v3-permanencia-5'::text as regra_versao,
  case
    ...
    when coalesce(x.elegiveis_nao_publicaveis, 0) > 0
      then 'existem vinculos elegiveis aguardando revisao; amostra parcial nao representa o historico'
    when e.amostra < 3
  ...
  jsonb_build_object(
    ...
    'aviso_diagnostico', case
      when coalesce(c.historico_incompleto, false)
        or coalesce(x.elegiveis_nao_publicaveis, 0) > 0
        then 'valor parcial nao pode ser exibido como KPI oficial'
      else null
    end
  ) as detalhes
...
comment on function public.get_professor_permanencia_v3_sombra(date, uuid) is
  'V3 sombra: permanencia acumulada; aviso de parcialidade somente quando o KPI permanece bloqueado.';
