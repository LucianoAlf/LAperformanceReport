...
-- 010-janela-3-dias.sql — janela do professor (gravar + chamada) 24h -> 3 dias.
...
begin
  ...
  if coalesce(v_aula.data_hora_fim, v_aula.data_hora_inicio) < now() - interval '3 days' then raise exception 'janela_de_gravacao_encerrada'; end if;
...
create or replace function public.fn_registrar_presencas_core(
  p_aula_ancora_id integer, p_professor_id integer, p_alunos_ausentes integer[] default '{}'::integer[],
...
begin
  ...
  if p_estrito then
    ...
    if coalesce(v_aula.data_hora_fim, v_aula.data_hora_inicio) < now() - interval '3 days' then raise exception 'janela_de_chamada_encerrada'; end if;
  end if;
