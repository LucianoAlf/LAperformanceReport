...
--
-- O usuário pediu: se clicou Presente, clicar Presente de novo remove o status
-- e o aluno volta para 'sem destino' (indeterminado). O mesmo para Falta e
-- Falta justificada.
--
-- Implementação: a RPC agora aceita status = 'indeterminado'. Quando recebe,
-- deleta o registro de aluno_presenca (e limpa o administrativo + cancela
-- crédito de reposição pendente). O estado natural sem registro = indeterminado.
--
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_itens) loop
    ...
    -- estado natural sem destino). Registra retificação se havia
    ...
    if v_status = 'indeterminado' then
      if found then
        ...
        -- Registra retificação para auditoria (status anterior -> indeterminado)
        ...
        -- Deleta o registro de presença (volta ao estado natural)
        ...
        -- Cancela crédito de reposição pendente (se era falta justificada)
    ...
    if found then
      ...
      -- O fallback do Emusys pode parecer "falta", mas ainda nao e decisao humana.
      -- So uma decisao final ja preenchida pode ser idempotente.
      ...
      -- Retificação: registra no histórico para auditoria, mas SEM exigir
      -- motivo. A equipe pode corrigir erros livremente (falta->presente,
      -- presente->falta, etc). O motivo é opcional.
      if v_existente.respondido_por = any(v_humanos) then
        insert into public.aluno_presenca_retificacoes (
          aluno_presenca_id, unidade_id, status_anterior, status_novo,
          respondido_por_anterior, respondido_em_anterior,
    ...
    -- justificativa: detalhe administrativo + crédito de reposição
    if v_status = 'falta_justificada' then
      ...
      -- rebaixou justificada -> falta seca: crédito pendente morre junto
      update public.aluno_reposicoes
      set status = 'cancelada', updated_at = now()
      where aluno_id = v_aluno_id and aula_origem_id = v_aula.id
        and origem = 'falta_justificada' and status = 'pendente';
