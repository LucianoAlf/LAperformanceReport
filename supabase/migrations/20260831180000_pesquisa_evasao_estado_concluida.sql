-- Pesquisa de evasao: estado visivel "concluida".
--
-- PROBLEMA (medido em 31/08/2026): registrar desfecho nao mudava NADA na lista.
-- O `case` de `estado_visivel` nunca consultava `pesquisa_evasao_desfechos`, entao
-- uma pesquisa parava em 'revisada' para sempre. A operadora clicava, o desfecho
-- gravava, a tela ficava identica -- e ela clicava de novo. A assinatura disso no
-- banco: Clarice com 3 desfechos (28/08) e Miguel com 3 (31/08), mesma quantidade
-- em dias diferentes. Nao e coincidencia, e alguem procurando um efeito que nunca vinha.
--
-- ORDEM DOS RAMOS IMPORTA: 'nova_rodada' vem ANTES de 'concluida'. Se chegar conteudo
-- novo depois do desfecho, a pesquisa precisa voltar para a fila de revisao em vez de
-- ficar presa em "concluida" -- por isso 'concluida' tambem exige
-- `not conteudo_novo_desde_revisao`.
--
-- ⚠️ A ASSINATURA NAO MUDA de proposito: `listar_followups_pesquisa_evasao_v1` faz
-- `select estado.*` com RETURNS TABLE proprio; acrescentar coluna aqui quebraria a
-- chamadora. O desfecho especifico (recuperou/prometeu_voltar/confirmou_saida) fica
-- fora desta entrega por causa disso.

create or replace function public.fn_pesquisa_evasao_followup_estado(
  p_agora timestamp with time zone default clock_timestamp()
)
returns table(
  pesquisa_id uuid,
  evasao_id integer,
...
with base as (
  select
    ...
    exists (
      select 1
      from public.pesquisa_evasao_desfechos desfecho
      where desfecho.pesquisa_id = pe.id
    ) as tem_desfecho,
    exists (
      select 1
...
select
  ...
  case
    ...
    when conteudo_novo_desde_revisao
      ...
      then 'nova_rodada'
    when tem_desfecho and not conteudo_novo_desde_revisao
      then 'concluida'
...
-- O filtro da tela precisa aceitar o estado novo, senao "Concluidas" devolve
-- `estado_followup_invalido`.
...
-- e recriar conta como nova. Revogar nominalmente (regra ja documentada no CLAUDE.md).
