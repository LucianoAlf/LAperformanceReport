...
-- Corrigir cálculo do ticket médio na view vw_kpis_gestao_mensal
-- Regra: soma de todas as parcelas / total de alunos ÚNICOS pagantes
-- Aluno único = agrupado por nome + data_nascimento + unidade_id
...
-- Calcular ticket médio por aluno único (não por matrícula)
alunos_ticket AS (
  SELECT 
    a.unidade_id,
    LOWER(TRIM(a.nome)) || '-' || COALESCE(a.data_nascimento::text, '') || '-' || a.unidade_id AS chave_aluno,
  ...
  GROUP BY a.unidade_id, LOWER(TRIM(a.nome)) || '-' || COALESCE(a.data_nascimento::text, '') || '-' || a.unidade_id
),
...
permanencia_combinada AS (
  SELECT alunos_historico.unidade_id AS uid, alunos_historico.tempo_permanencia_meses AS meses
...
SELECT 
  ...
  -- Ticket médio agora é calculado por aluno único
  ...
  CASE 
    ...
    WHEN COALESCE(am.alunos_pagantes, 0) > 0 
    THEN ROUND(COALESCE(em.total_evasoes, 0)::numeric / am.alunos_pagantes * 100, 2)
    ELSE 0
