-- REVERSAO: a versao que usava fn_presenca_candidatos_periodo_v1 custou 6.450ms / 1,27M buffers
-- para UM dia de Campo Grande, contra ~80ms da original. Regressao de ~80x no relatorio que roda
-- todo dia as 9h para as ADMs. Restaurando a definicao original enquanto a funcao compartilhada
-- e otimizada. A paridade LOGICA ja foi provada (85/85 dias); o que falta e custo.
...
begin
  ...
  if v_status = 'atualizados' then
    with ocorrencias as materialized (
      select
        slot_key, unidade_id, data_aula, resultado_canonico, fecha_chamada,
