...
-- Migration: Adicionar valores brutos ao retorno da RPC get_programa_fideliza_dados
-- para exibir no tooltip de cada métrica

CREATE OR REPLACE FUNCTION public.get_programa_fideliza_dados(p_ano integer, p_trimestre integer, p_unidade_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  -- Soma total de evasões no trimestre por unidade
  evasoes_trimestre AS (
    SELECT unidade_id, SUM(total_evasoes) AS total_evasoes
    FROM evasoes_mes
    GROUP BY unidade_id
  ),
  alunos_base_mes AS (
    SELECT 
      u.id AS unidade_id, mt.mes,
      COALESCE(
  ...
  metricas_trimestre AS (
    SELECT u.id AS unidade_id, u.nome AS unidade_nome,
      ...
      COALESCE(et.total_evasoes, 0) AS total_evasoes,
      COALESCE(abt.alunos_base_media, 0) AS alunos_base,
      COALESCE(rt.renovados, 0) AS renovados,
      COALESCE(rt.total_contratos, 0) AS total_contratos
    FROM unidades u 
  ...
  penalidades_totais AS (
    ...
    WHERE ano = p_ano AND trimestre = v_trim_atual AND (p_unidade_id IS NULL OR unidade_id = p_unidade_id)
    GROUP BY unidade_id
  )
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'metricas', jsonb_build_object(
      ...
      'vendas_lojinha', 0,
      -- NOVOS: valores brutos para tooltip
      'churn_bruto', jsonb_build_object(
        'evasoes', mt.total_evasoes,
        'alunos_base', mt.alunos_base
      ),
      'renovacao_bruto', jsonb_build_object(
        'renovados', mt.renovados,
        'total_contratos', mt.total_contratos
      )
    ),
