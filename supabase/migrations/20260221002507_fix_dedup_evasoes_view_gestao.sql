...
-- Fix: deduplicação de evasões na vw_kpis_gestao_mensal (mesmo fix da retencao)
...
WITH matriculas_mes AS (
    ...
    FROM leads l
    WHERE (l.status::text = ANY (ARRAY['matriculado'::text, 'convertido'::text]))
        AND (l.tipo_aluno IS NULL OR (l.tipo_aluno::text <> ALL (ARRAY['bolsista_integral'::text, 'nao_pagante'::text])))
    ...
    WHERE (a.status::text = ANY (ARRAY['ativo'::text, 'trancado'::text]))
        AND tm.entra_ticket_medio = true AND a.valor_parcela > 0::numeric
    ...
    SELECT alunos_historico.unidade_id, alunos_historico.tempo_permanencia_meses AS meses
    ...
    FROM alunos a
    LEFT JOIN tipos_matricula tm ON tm.id = a.tipo_matricula_id
    WHERE (a.status::text = ANY (ARRAY['inativo'::text, 'evadido'::text]))
        AND a.tempo_permanencia_meses >= 4
        AND (tm.codigo IS NULL OR (tm.codigo::text <> ALL (ARRAY['BOLSISTA_INT'::text, 'BOLSISTA_PARC'::text, 'BANDA'::text])))
    ...
    SELECT DISTINCT ON (lower(trim(e.aluno_nome)), e.unidade_id, EXTRACT(year FROM e.data_evasao), EXTRACT(month FROM e.data_evasao))
        ...
        e.valor_parcela
    FROM evasoes_v2 e
    WHERE e.tipo_saida_id = ANY (ARRAY[1, 2])
    ...
    SELECT l.unidade_id,
        EXTRACT(year FROM l.data_contato)::integer AS ano,
        EXTRACT(month FROM l.data_contato)::integer AS mes,
        sum(CASE WHEN l.status::text = ANY (ARRAY['novo'::text, 'agendado'::text]) THEN COALESCE(l.quantidade, 1) ELSE 0 END) AS total_leads,
        ...
        sum(CASE WHEN l.status::text = ANY (ARRAY['experimental_realizada'::text, 'compareceu'::text]) THEN COALESCE(l.quantidade, 1) ELSE 0 END) AS experimentais_realizadas,
