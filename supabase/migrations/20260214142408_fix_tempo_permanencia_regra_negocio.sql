...
-- =====================================================
-- CORREÇÃO: Tempo de Permanência — Regra de Negócio
-- =====================================================
-- Regras:
-- 1. Apenas alunos que SAÍRAM (evasoes_v2)
-- 2. Excluir bolsistas (BOLSISTA_INT, BOLSISTA_PARC) e BANDA
-- 3. Mínimo 4 meses de permanência (≤3 não conta)
-- 4. Excluir segundo curso
-- 5. Excluir tempo_permanencia_meses <= 0 (dados inconsistentes)
-- =====================================================

-- 1) Criar RPC fonte de verdade para tempo de permanência
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(ROUND(AVG(ee.tempo_permanencia_meses)::NUMERIC, 1), 0) AS tempo_permanencia_medio,
    COALESCE(COUNT(ee.tempo_permanencia_meses)::INTEGER, 0) AS total_evasoes_elegiveis,
    COALESCE(SUM(ee.tempo_permanencia_meses)::INTEGER, 0) AS soma_meses
  FROM unidades u
...
-- 2) Recriar vw_dashboard_unidade com tempo de permanência correto
...
-- Tempo de permanência: baseado em evasões (não ativos)
-- Regra: apenas pagantes, ≥4 meses, excluir bolsistas/banda/segundo curso
...
SELECT 
  ...
  COALESCE(pe.tempo_permanencia_medio, 0)::numeric(5,1) AS tempo_permanencia,
...
-- 3) Recriar vw_kpis_gestao_mensal com tempo de permanência correto
...
-- Tempo de permanência: baseado em evasões (TODO o histórico, não filtrado por mês)
-- Regra: apenas pagantes, ≥4 meses, excluir bolsistas/banda/segundo curso
permanencia_evasoes AS (
  SELECT 
    e.unidade_id,
...
SELECT 
  ...
  COALESCE(pe.tempo_permanencia_medio, 0)::numeric(5,1) AS tempo_permanencia_medio,
  (COALESCE(am.ticket_medio, 0) * COALESCE(pe.tempo_permanencia_medio, 0))::numeric(12,2) AS ltv_medio,
