...
-- BUG PEGO NO 2o TESTE: o roster (aula_alunos_emusys) so cobre ~7 dias (janela do sync).
-- Pra qualquer aula com mais de uma semana, o vinculo aluno->aula historico SOME do roster —
-- mesmo problema que a app_aluno_ficha ja resolveu ontem (roster OU presenca). Aplicando
-- o mesmo padrao aqui: sem isso, o briefing so enxergaria historico da ULTIMA semana,
-- o que mata boa parte do valor (mostrar "o que rolou na ultima aula" de verdade).
                 ...
                 select jsonb_agg(jsonb_build_object(
                          ...
                          'resumo_ultima_aula', (
                            ...
                            where ae2.professor_id = p_professor_id
                              ...
                              -- VINCULO: roster (aulas recentes) OU presenca (historico >7 dias)
                              and (
                                ...
                                or exists (select 1 from public.aluno_presenca ap
                                            where ap.aula_emusys_id = ae2.id and ap.aluno_id = a.id)
                              )
