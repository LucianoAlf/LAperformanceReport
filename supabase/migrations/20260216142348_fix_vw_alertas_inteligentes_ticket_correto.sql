...
-- Corrigir vw_alertas_inteligentes para usar ticket médio correto
-- Ticket = MRR (todos pagantes) / COUNT(pagantes únicos sem segundo curso)
...
WITH churn_realtime AS (
  SELECT 
    ...
    CASE
      ...
      THEN round(COALESCE(em.evasoes, 0)::numeric / ap.alunos_pagantes::numeric * 100, 2)
      ELSE 0
    END AS churn_rate
  FROM unidades u
  LEFT JOIN (
    -- Evasões do mês atual, excluindo Aviso Prévio (tipo_saida_id = 3)
    SELECT 
      e.unidade_id,
      count(*) AS evasoes
    FROM evasoes_v2 e
  ...
  LEFT JOIN (
    -- Alunos pagantes atuais (sem segundo curso, valor > 0)
    SELECT 
      a.unidade_id,
...
ticket_realtime AS (
  -- CORREÇÃO: Ticket = MRR (todos pagantes) / COUNT(pagantes únicos sem segundo curso)
  SELECT 
    mrr.unidade_id,
    CASE 
      WHEN COALESCE(pag.pagantes_unicos, 0) > 0 
  ...
  LEFT JOIN (
    -- Pagantes únicos = COUNT sem segundo curso
    SELECT 
      unidade_id,
...
FROM (
  ...
  SELECT 
    ...
    concat(count(*), ' alunos com contrato vencendo até ', to_char(CURRENT_DATE + '30 days'::interval, 'DD/MM')) AS detalhe,
  ...
  -- TICKET_CAINDO (usando fórmula correta: MRR / pagantes únicos)
  SELECT 
    'TICKET_CAINDO'::text,
    'atencao'::text,
    tr.unidade_id,
