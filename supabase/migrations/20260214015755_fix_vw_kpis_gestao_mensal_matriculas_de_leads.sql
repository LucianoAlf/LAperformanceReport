...
-- Corrigir view vw_kpis_gestao_mensal: matrículas devem vir da tabela leads (igual ao Comercial)
-- Antes: contava da tabela alunos por data_matricula (incluía testes, cadastros manuais, etc)
-- Depois: conta da tabela leads com status IN ('matriculado','convertido')

CREATE OR REPLACE VIEW vw_kpis_gestao_mensal AS
WITH matriculas_mes AS (
    -- CORRIGIDO: usar tabela leads como fonte de matrículas (consistente com página Comercial)
    SELECT 
        l.unidade_id,
