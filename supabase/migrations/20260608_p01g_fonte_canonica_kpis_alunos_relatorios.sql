-- ============================================================================
-- P0.1G - Fonte canonica de KPIs de alunos para relatorios/RPCs
-- Projeto alvo: ouqwbbermlzqqvtqwlul
-- Status: PROPOSTA EXECUTAVEL SOMENTE COM APPROVE EXPLICITO DO ALF.
--
-- Este arquivo altera funcoes/RPCs quando executado.
-- NAO executar sem validacao humana.
--
-- Objetivo:
-- - Criar uma fonte canonica de banco para KPIs executivos de alunos.
-- - Fazer relatorios/IA deixarem de beber cegamente de vw_kpis_gestao_mensal.
-- - Preservar as funcoes legadas via wrapper cirurgico.
--
-- Nao faz:
-- - DDL em tabelas de negocio.
-- - UPDATE/DELETE/INSERT em dados historicos.
-- - recalculo de dados_mensais.
-- - backfill.
-- - fechamento de competencia.
-- - alteracao de KPI live/view/frontend.
--
-- Regra canonica:
-- - Mes fechado/bloqueado: fonte = dados_mensais.
-- - Mes atual aberto: fonte = calculo vivo canonico direto de alunos/movimentacoes.
-- - Mes passado aberto com snapshot: fonte = preliminar.
-- - Mes passado aberto sem snapshot: fonte = indisponivel.
-- ============================================================================
...
IS 'P0.1G: Fonte canonica de KPIs executivos de alunos. Fechado usa dados_mensais; mes atual aberto usa calculo vivo canonico; passado aberto sem snapshot fica indisponivel.';

-- Wrapper cirurgico: preserva corpo legado e substitui a chave kpis_gestao
-- pela fonte canonica. O legado fica disponivel para rollback.
DO $$
BEGIN
  IF to_regprocedure('public.get_dados_relatorio_gerencial(uuid, integer, integer)') IS NOT NULL
     AND to_regprocedure('public.get_dados_relatorio_gerencial_legacy_p01g(uuid, integer, integer)') IS NULL THEN
    EXECUTE 'ALTER FUNCTION public.get_dados_relatorio_gerencial(uuid, integer, integer) RENAME TO get_dados_relatorio_gerencial_legacy_p01g';
  ELSIF to_regprocedure('public.get_dados_relatorio_gerencial_legacy_p01g(uuid, integer, integer)') IS NULL THEN
    RAISE EXCEPTION 'Funcao legado get_dados_relatorio_gerencial nao encontrada para criar wrapper P0.1G';
  END IF;
END $$;

-- O legado continua sendo SECURITY DEFINER; fixa search_path para nao manter
-- a fragilidade antiga ao ser chamado pelo wrapper.
ALTER FUNCTION public.get_dados_relatorio_gerencial_legacy_p01g(uuid, integer, integer)
  SET search_path = public, pg_temp;
...
IS 'P0.1G wrapper: preserva relatorio gerencial legado e substitui kpis_gestao por get_kpis_alunos_canonicos.';

DO $$
BEGIN
  IF to_regprocedure('public.get_dados_retencao_ia(uuid, integer, integer)') IS NOT NULL
     AND to_regprocedure('public.get_dados_retencao_ia_legacy_p01g(uuid, integer, integer)') IS NULL THEN
    EXECUTE 'ALTER FUNCTION public.get_dados_retencao_ia(uuid, integer, integer) RENAME TO get_dados_retencao_ia_legacy_p01g';
  ELSIF to_regprocedure('public.get_dados_retencao_ia_legacy_p01g(uuid, integer, integer)') IS NULL THEN
    RAISE EXCEPTION 'Funcao legado get_dados_retencao_ia nao encontrada para criar wrapper P0.1G';
  END IF;
END $$;
...
IS 'P0.1G wrapper: preserva analise de retencao legado e substitui kpis_gestao por get_kpis_alunos_canonicos. kpis_retencao ainda e pendencia P0.1H.';

-- Hardening minimo P0.1G:
-- As RPCs atuais estavam executaveis por PUBLIC/anon. Como estes endpoints
-- expoem dados gerenciais, a proposta bloqueia acesso anonimo e mantem uso por
-- usuario autenticado e backend service_role.
...
REVOKE ALL ON FUNCTION public.get_dados_relatorio_gerencial_legacy_p01g(uuid, integer, integer) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_dados_retencao_ia(uuid, integer, integer) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_dados_retencao_ia_legacy_p01g(uuid, integer, integer) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.get_dados_relatorio_gerencial_legacy_p01g(uuid, integer, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_dados_retencao_ia(uuid, integer, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_dados_retencao_ia_legacy_p01g(uuid, integer, integer) TO authenticated, service_role;

-- ============================================================================
-- CHECKLIST SELECT-ONLY POS-EXECUCAO
-- ============================================================================
--
-- 1) Confirmar helper e wrappers:
-- SELECT proname, pg_get_function_identity_arguments(oid) AS args, prosecdef,
--        COALESCE(array_to_string(proconfig, ', '), '') AS proconfig
...
-- ORDER BY proname;
--
-- 2) Campo Grande / Junho aberto:
-- SELECT public.get_kpis_alunos_canonicos(
--   '2ec861f6-023f-4d7b-9927-3960ad8c2a92'::uuid, 2026, 6
-- ) #> '{totais}' AS totais;
-- Esperado hoje: alunos_ativos=479, alunos_pagantes=449,
-- Kids=194, School=285, matriculas_ativas=543, banda=39, 2o curso=27.
--
-- 3) Campo Grande / Maio fechado:
-- SELECT public.get_kpis_alunos_canonicos(
--   '2ec861f6-023f-4d7b-9927-3960ad8c2a92'::uuid, 2026, 5
-- ) #> '{totais}' AS totais;
-- Esperado: fonte=dados_mensais, alunos_ativos=496, pagantes=470,
-- ticket=368.66, evasoes=13, churn=2.77.
--
-- 4) Relatorio gerencial passa a expor kpis_alunos_canonicos:
-- SELECT public.get_dados_relatorio_gerencial(
--   '2ec861f6-023f-4d7b-9927-3960ad8c2a92'::uuid, 2026, 6
-- ) ? 'kpis_alunos_canonicos' AS tem_chave;
--
-- ============================================================================
-- ROLLBACK FASE P0.1G
-- ============================================================================
--
-- DROP FUNCTION IF EXISTS public.get_dados_relatorio_gerencial(uuid, integer, integer);
-- ALTER FUNCTION public.get_dados_relatorio_gerencial_legacy_p01g(uuid, integer, integer)
--   RENAME TO get_dados_relatorio_gerencial;
--
-- DROP FUNCTION IF EXISTS public.get_dados_retencao_ia(uuid, integer, integer);
-- ALTER FUNCTION public.get_dados_retencao_ia_legacy_p01g(uuid, integer, integer)
--   RENAME TO get_dados_retencao_ia;
--
