-- ============================================================================
-- P08A-fix - KPIs operacionais administrativos incluem trancados na carteira
-- Projeto alvo: ouqwbbermlzqqvtqwlul
--
-- Regra de negocio LA Report:
-- - Trancados continuam pertencendo a carteira operacional.
-- - Portanto contam em alunos ativos, pagantes e matriculas ativas.
-- - O total de trancados segue aparecendo separado como diagnostico.
-- - Nao altera evasoes, historico, snapshots nem dados_mensais.
-- ============================================================================
...
pessoas_ativas AS (
  SELECT
    ...
    SUM(
      CASE
        WHEN ac.entra_ticket_medio = true AND ac.valor_parcela > 0 THEN ac.valor_parcela
        ELSE 0
      END
    ) AS mrr,
    BOOL_OR(
  ...
  WHERE ac.status IN ('ativo', 'trancado')
...
trancados AS (
  SELECT
    ub.unidade_id,
    COUNT(DISTINCT ac.pessoa_key) FILTER (WHERE ac.status = 'trancado')::integer AS alunos_trancados
  FROM unidades_base ub
  LEFT JOIN alunos_classificados ac ON ac.unidade_id = ub.unidade_id
  GROUP BY ub.unidade_id
),
...
SELECT jsonb_build_object(
  ...
  'alertas_fonte', jsonb_build_array(
    'Relatorio administrativo operacional: alunos ativos/pagantes incluem status ativo e trancado; trancados aparecem tambem em linha separada.'
  )
)
FROM totais;
$function$;
...
IS 'KPIs vivos para relatorio administrativo: carteira operacional inclui ativos e trancados, mantendo trancados em linha separada; sem alterar snapshots ou KPI executivo.';
