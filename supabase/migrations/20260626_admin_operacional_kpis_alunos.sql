-- ============================================================================
-- P08A - KPIs operacionais do relatorio administrativo
-- Projeto alvo: ouqwbbermlzqqvtqwlul
--
-- Contexto:
-- - A RPC executiva get_kpis_alunos_canonicos mistura a leitura viva com campos
--   historicos/financeiros e pode incluir trancados em alguns totais.
-- - O relatorio diario administrativo precisa de carteira operacional:
--   alunos ativos = status ativo; trancados aparecem em linha separada.
-- - Esta funcao nao altera snapshots, dados_mensais nem a RPC executiva global.
-- ============================================================================
...
SELECT jsonb_build_object(
  ...
  'alertas_fonte', jsonb_build_array(
    'Relatorio administrativo operacional: alunos ativos usam status ativo; trancados aparecem separados.'
  )
)
FROM totais;
$function$;
...
IS 'KPIs vivos para relatorio administrativo: carteira ativa operacional separada de trancados, sem alterar snapshots ou KPI executivo.';
