...
-- Fix 1: Remover duplicata antiga que causa ambiguidade em chamadas com named parameters
DROP FUNCTION IF EXISTS public.get_kpis_professor_periodo(uuid, integer, integer);

-- Fix 2: Dropar versao atual para mudar RETURNS TABLE
DROP FUNCTION IF EXISTS public.get_kpis_professor_periodo(integer, integer, uuid);

-- Recriar funcao com:
--   - Filtro correto por data_experimental / data_conversao (antes era data_contato)
--   - Campos novos: experimentais_agendadas, experimentais_faltas
  ...
  WITH
    ...
    experimentais AS (
      SELECT l.professor_experimental_id AS prof_id, l.unidade_id AS uid,
        -- Agendadas: tudo que tem data_experimental no mes
        COUNT(*) FILTER (
          WHERE l.data_experimental BETWEEN v_inicio AND v_fim
        )::integer AS experimentais_agendadas,

        -- Realizadas: aconteceu no mes E experimental_realizada = true
        SUM(CASE
        ...
        -- Faltas: agendada no mes E faltou (ou matriculou sem ter realizado a experimental)
        COUNT(*) FILTER (
          WHERE l.data_experimental BETWEEN v_inicio AND v_fim
        ...
        -- Matriculas pos-exp: experimental aconteceu no mes E converteu
        SUM(CASE
          WHEN l.data_experimental BETWEEN v_inicio AND v_fim
        ...
        -- Matriculas diretas: converteu no mes SEM ter feito experimental
        SUM(CASE
        ...
        -- Matriculas totais = pos-exp + diretas
        SUM(CASE
