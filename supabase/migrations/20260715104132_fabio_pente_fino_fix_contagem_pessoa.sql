...
-- BUG MEU: total_pessoas fazia count(*) na CTE 'pessoas', que ainda tinha 1 linha por
-- aluno_id (necessario pro join com aluno_presenca, que e' keyed por aluno_id). A chave
-- de pessoa (pessoa_key) estava certa, mas eu nunca apliquei DISTINCT nela na contagem final.
-- Prova: Luiza Pimentel (aluno_id 265 Teclado + 1465 Canto, mesmo emusys_aluno_id 3183)
-- continuava contando 2. Fix: count(distinct pessoa_key), nao count(*).
...
as $function$
declare
  v_perfil text; v_unidade_id uuid; v_res jsonb;
...
begin
  ...
  if v_unidade_id is null then return jsonb_build_object('ok', false, 'motivo', 'unidade_nao_encontrada'); end if;
      ...
      from (
        select jsonb_build_object(
                 ...
                 'motivo_suspeita', case when v.pct_30_atual is not null and v.pct_30_anterior is not null
                    and v.pct_30_anterior >= 30 and v.pct_30_atual < v.pct_30_anterior * 0.5
                    then format('presenca registrada caiu de %s%% (30-60d atras) pra %s%% (ultimos 30d)',
        ...
        join lateral (
          ...
          select (select count(distinct pessoa_key) from pessoas) as total_pessoas,   -- FIX: distinct pessoa_key, nao count(*)
