...
BEGIN
  FOR v_unidade IN SELECT id, nome FROM unidades WHERE ativo = true LOOP
    ...
    SELECT 
      ...
      (SELECT COUNT(*) FROM evasoes_v2 WHERE unidade_id = v_unidade.id
       AND EXTRACT(YEAR FROM data_evasao) = p_ano AND EXTRACT(MONTH FROM data_evasao) = p_mes)::INTEGER,
      COALESCE((SELECT CASE WHEN dm_ant.alunos_pagantes > 0 
        THEN ROUND(((SELECT COUNT(*) FROM evasoes_v2 WHERE unidade_id = v_unidade.id 
                     AND EXTRACT(YEAR FROM data_evasao) = p_ano AND EXTRACT(MONTH FROM data_evasao) = p_mes)::NUMERIC 
