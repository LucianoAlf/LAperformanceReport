-- Uma identidade, um credito D+30 e uma elegibilidade para mensal e ciclo.
-- O denominador e experimental/evento; nao se exige matricula de quem nao converteu.
-- A atribuicao do credito e decidida antes do corte do relatorio para impedir
-- duplicidade entre meses. Eventos futuros nao entram no denominador.
begin;
...
begin
  ...
  select
    ...
    'health-score-professor-v3-conversao-periodo-canonico-2'::text
      as regra_versao,
    case
...
revoke all on function public.get_health_score_professor_v3_conversao_periodo_canonico(date,uuid,text)
  from public, anon, authenticated;
grant execute on function public.get_health_score_professor_v3_conversao_periodo_canonico(date,uuid,text)
  to service_role;


create or replace function public.get_health_score_professor_v3_conversao_mensal(
  p_competencia date, p_unidade_id uuid default null
)
 RETURNS TABLE(metrica text, professor_id integer, professor_nome text, unidade_id uuid, competencia date, valor_bruto numeric, numerador numeric, denominador numeric, amostra integer, estado_base text, publicavel boolean, confianca text, fonte text, regra_versao text, motivo_sem_base text, detalhes jsonb)
language sql stable security definer
set search_path = public, pg_temp
as $function$
  select c.* from public.get_health_score_professor_v3_conversao_periodo_canonico(
