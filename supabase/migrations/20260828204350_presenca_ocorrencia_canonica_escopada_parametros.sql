-- Especializa o kernel para o unico contrato publicado: unidade e periodo (ledger alinhado)
-- obrigatorios. Sem OR anulavel, o planner usa os indices compostos na origem.
...
with evidencia as not materialized (
  ...
  where ap.unidade_id = p_unidade_id
    and ap.data_aula between p_data_inicio and p_data_fim
    and not exists (
    select 1
    from public.aulas_emusys ae
...
comment on function public.fn_presenca_ocorrencia_canonica_escopada_v2(
  uuid, date, date
) is 'Ocorrencia canonica v2.7 parametrizada; unidade e periodo entram antes do ranking e da agregacao.';
