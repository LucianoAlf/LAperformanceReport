...
-- Contrato V3 dos cinco relatórios da Coordenação.
-- A competência continua sendo a referência de navegação, mas todos os fatos
-- pedagógicos são lidos do mesmo produtor do Health Score V3, no grão mensal
-- ou no ciclo oficial selecionado. Percentuais de ciclo são recompostos pelos
-- numeradores e denominadores, nunca pela média de percentuais mensais.
...
begin
  ...
  -- O V2 permanece como base compatível para identidade, coordenação, agenda,
  -- sinais físicos e histórico fechado. Os números pedagógicos são substituídos
    ...
    jsonb_agg(
      coalesce(
        ...
        jsonb_build_object(
          ...
          'nome', coalesce(prof.nome, 'Professor não informado')
        )
      )
      || jsonb_build_object(
      ...
      order by coalesce(b.item->>'nome', prof.nome, 'Professor não informado')
    ),
    '[]'::jsonb
  )
  into v_professores
  ...
  select jsonb_build_object(
    ...
    'regra_publica', 'Movimentações válidas do período, com impacto no professor separado do total operacional'
  )
  into v_saidas
  from movimentos;
  ...
  select jsonb_build_object(
    ...
    'direcionamento', 'completar vínculo de turma e sala'
  )
  ...
  return v_base || jsonb_build_object(
    ...
    'auditoria', coalesce(v_base->'auditoria', '{}'::jsonb) || jsonb_build_object(
      ...
      'fonte_publica', 'Health Score Professor V3 e indicadores oficiais do período',
...
comment on function public.montar_relatorio_coordenacao_payload_v3(
  ...
  'Produtor interno dos cinco relatórios da Coordenação. Mensal mostra evidências da competência; ciclo agrega fatos brutos no ciclo oficial.';

comment on function public.get_relatorio_coordenacao_canonico_v3(
  uuid, integer, integer, text
) is
  'Contrato canônico V3 dos relatórios da Coordenação, auditável por período, configuração e regra do Health Score Professor V3.';
