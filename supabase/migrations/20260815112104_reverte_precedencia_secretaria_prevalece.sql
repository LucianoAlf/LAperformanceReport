...
-- REVERSÃO da 20260815010000. Eu inverti uma REGRA DE NEGÓCIO da casa achando
-- que era defeito técnico. A regra real, confirmada pelo Alf em 15/08/2026:
--
--   A SECRETARIA é quem dá presença (no Emusys / LA Report), e é ela que
--   PREVALECE. O professor lança CONTEÚDO (o relatório da aula). O professor
--   pode dar presença — dentro da janela dele — mas a resposta da secretaria
--   não é sobrescrita por ele. Se o professor discorda, ele REPORTA à
--   secretaria, e ela corrige na fonte.
--
-- Ou seja: o `first write wins` entre fontes fortes era intencional, não bug.
-- Volta exatamente o corpo anterior (capturado de pg_get_functiondef antes da
-- minha alteração) nas três funções, e some a régua que eu tinha inventado.
--
-- O que o Valdo relatou CONTINUA valendo como pendência, mas é outro problema:
-- quando a secretaria já deu a presença, o app do professor tem que MOSTRAR
-- aquela presença já dada (em tempo real), pra ele não ter que lançar de novo
-- na hora de gravar o relatório. Isso é exibição/sincronismo, não precedência.
