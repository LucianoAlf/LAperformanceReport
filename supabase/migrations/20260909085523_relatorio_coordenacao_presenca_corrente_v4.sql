...
-- Em uma base que recebeu a primeira versao desta correcao, amplia a regra
-- para qualquer fotografia ainda em andamento. Em uma instalacao nova, a
-- definicao anterior ja nasce com esta forma e o bloco apenas confirma o
-- contrato esperado.
do $patch_reader_periodo_em_andamento$
declare
  v_definicao text;
  v_antigo text := $antigo$
        current_date between b.periodo_inicio and b.periodo_fim
        and coalesce(b.ranking_habilitado, false) is false
$antigo$;
  v_novo text := $novo$
...
begin
  ...
  if position(v_novo in v_definicao) > 0 then
    ...
    raise exception 'HEALTH_SCORE_V3_LEITOR_EVIDENCIA_CORRENTE_INESPERADO';
  end if;
end;
$patch_reader_periodo_em_andamento$;

-- O acumulado progressivo de presenca passa pela mesma leitora materializada
-- usada na pagina. A consulta deixa de abrir as tabelas internas e deixa de
-- deduzir uma competencia efetiva a partir da referencia historica.
...
as $function$
  with periodos as materialized (
    ...
    select
      m.professor_id,
      sum(m.numerador) filter (where m.denominador > 0) as numerador,
      sum(m.denominador) filter (where m.denominador > 0) as denominador,
      sum(coalesce(m.amostra, m.denominador::integer, 0))
        filter (where m.denominador > 0)::integer as amostra,
...
comment on function public.relatorio_coordenacao_presenca_v4(
  ...
  'Presenca progressiva da Coordenacao composta pelas mesmas fotografias mensais materializadas exibidas na pagina, sem referencia de competencia anterior.';
