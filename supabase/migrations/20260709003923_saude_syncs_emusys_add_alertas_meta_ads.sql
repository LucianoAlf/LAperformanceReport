...
AS $function$
  ...
  -- Alertas de projeto (alertas-diarios, alertas-tarefas-atrasadas, resumo-semanal):
  -- todos disparam a mesma edge projeto-alertas-whatsapp e gravam evidencia real em
  -- notificacao_log (status 'enviado'/'erro' por destinatario, sem retry - erro e
  -- imediato e definitivo, ao contrario da fila de relatorios).
  ...
  -- Meta Ads (enriquecer-meta-ads-diario): meta_ads_cache ja guarda erro/enriquecido_em
  -- por linha (upsert grava null no sucesso, mensagem no erro).
  meta_ads_stats as (
    select
