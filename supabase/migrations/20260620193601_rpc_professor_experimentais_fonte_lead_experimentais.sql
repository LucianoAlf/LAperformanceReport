...
BEGIN
  ...
  WITH
    ...
    -- FONTE TROCADA: lê de lead_experimentais (1 linha por aula, presença real),
    -- não mais de leads (que agregava 1 por pessoa e subcontava realizadas).
    -- denominador (experimentais) = realizada + convertido; numerador (matriculas_pos_exp) =
    -- realizadas cujo lead converteu. Matrícula direta sem experimental NÃO entra (=0).
    experimentais AS (
      SELECT le.professor_experimental_id AS prof_id, le.unidade_id AS uid,
        ...
        COUNT(*) FILTER (WHERE le.status IN ('experimental_realizada','convertido')
            AND (l.converteu = true OR l.status IN ('convertido','matriculado')))::integer AS matriculas_pos_exp,
        0::integer AS matriculas_diretas,
        COUNT(*) FILTER (WHERE le.status IN ('experimental_realizada','convertido')
            AND (l.converteu = true OR l.status IN ('convertido','matriculado')))::integer AS matriculas
