...
-- Corrigir vw_kpis_gestao_mensal: calcular inadimplência real e faturamento realizado
-- Antes: inadimplencia_pct hardcoded como 0, faturamento_realizado = faturamento_previsto
...
alunos_mes AS (
  SELECT a.unidade_id,
    ...
    -- NOVO: contar inadimplentes e somar valor das parcelas inadimplentes
...
dados_anterior AS (
  SELECT dados_mensais.unidade_id, dados_mensais.ano, dados_mensais.mes, dados_mensais.alunos_pagantes
  FROM dados_mensais
)
SELECT u.id AS unidade_id,
  ...
  -- CORRIGIDO: inadimplência real baseada em status_pagamento
  CASE WHEN COALESCE(am.alunos_pagantes, 0) > 0
    THEN round(COALESCE(am.qtd_inadimplentes, 0)::numeric / am.alunos_pagantes * 100, 2)
    ELSE 0
  END::numeric(5,2) AS inadimplencia_pct,
  -- Faturamento previsto = MRR total
  COALESCE(am.mrr, 0)::numeric(12,2) AS faturamento_previsto,
  -- CORRIGIDO: faturamento realizado = MRR - parcelas inadimplentes
  ...
  (CASE
    WHEN COALESCE(da.alunos_pagantes, 0) > 0 THEN round(COALESCE(em.total_evasoes, 0)::numeric / da.alunos_pagantes * 100, 2)
    WHEN COALESCE(am.alunos_pagantes, 0) > 0 THEN round(COALESCE(em.total_evasoes, 0)::numeric / am.alunos_pagantes * 100, 2)
    ELSE 0
  END)::numeric(5,2) AS churn_rate,
  COALESCE(rm.renovacoes, 0)::integer AS renovacoes,
  (CASE
    WHEN COALESCE(rm.total_contratos, 0) > 0 THEN round(rm.renovacoes::numeric / rm.total_contratos * 100, 2)
    ELSE 0
