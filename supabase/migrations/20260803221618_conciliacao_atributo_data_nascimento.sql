...
-- Adiciona o campo data_nascimento a RPC aplicar_conciliacao_aluno_atributo.
-- Motivo: o sync-matriculas-emusys passa a detectar divergencia de data de nascimento
-- (tipo data_nascimento_divergente, severidade alta) e a aba de Conciliacao precisa
-- do botao "Aplicar Emusys" para corrigir com 1 clique (decisao humana, nunca auto).
-- Validacao: o cast v_valor::date falha com erro se o formato for invalido.

CREATE OR REPLACE FUNCTION public.aplicar_conciliacao_aluno_atributo(p_divergencia_id bigint, p_decisao text, p_decidido_por text DEFAULT 'usuario_app'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_decisao = 'aplicar_emusys' THEN
    ...
    CASE v_div.campo
      ...
      WHEN 'data_nascimento' THEN
        v_valor := COALESCE(v_div.sugestao->>'data_nascimento', v_div.valor_emusys->>'data_nascimento');
        IF v_valor IS NULL OR btrim(v_valor) = '' THEN
          RAISE EXCEPTION 'data_nascimento do Emusys ausente';
        END IF;
        PERFORM v_valor::date;
        UPDATE public.alunos SET data_nascimento = v_valor::date, updated_at = v_agora WHERE id = v_div.aluno_id;
        v_valor_aplicado := jsonb_build_object('data_nascimento', v_valor);
