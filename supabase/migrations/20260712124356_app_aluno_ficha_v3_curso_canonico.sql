...
-- v3: o filtro de curso do historico passa a usar fn_curso_base nos DOIS lados.
-- Bug que isso conserta: 14 dos 19 alunos do Matheus tinham historico VAZIO porque as aulas
-- vinham como "Canto T" e a jornada como "Canto". Atingia tambem Piano/Teclado IND, GarageBand,
-- Ukulelê e Musicalização em toda a escola. O escopo por curso CONTINUA (nao reabre o vazamento).
...
begin
  ...
  -- CANONICO: guarda o curso normalizado, nao o nome cru
      ...
      from (
        ...
        from (
          ...
          where ap.aluno_id = p_aluno_id
            and public.fn_curso_base(ae.curso_nome) = any(v_cursos)   -- <<< CANONICO
