...
DROP VIEW IF EXISTS vw_distribuicao_permanencia;
DROP VIEW IF EXISTS vw_ltv_por_categoria;
DROP VIEW IF EXISTS vw_ltv_por_unidade;
DROP VIEW IF EXISTS vw_ltv_rede;
DROP VIEW IF EXISTS vw_dashboard_unidade;
DROP VIEW IF EXISTS vw_kpis_gestao_mensal;

ALTER TABLE alunos_historico ALTER COLUMN tempo_permanencia_meses TYPE numeric(5,2);

CREATE VIEW vw_distribuicao_permanencia AS
 SELECT
        CASE
            WHEN tempo_permanencia_meses >= 4 AND tempo_permanencia_meses <= 6 THEN '04-06 meses'::text
            WHEN tempo_permanencia_meses >= 7 AND tempo_permanencia_meses <= 12 THEN '07-12 meses'::text
            WHEN tempo_permanencia_meses >= 13 AND tempo_permanencia_meses <= 24 THEN '13-24 meses'::text
            WHEN tempo_permanencia_meses >= 25 AND tempo_permanencia_meses <= 36 THEN '25-36 meses'::text
            WHEN tempo_permanencia_meses >= 37 AND tempo_permanencia_meses <= 48 THEN '37-48 meses'::text
            ELSE '49+ meses'::text
        END AS faixa,
    count(*) AS quantidade,
    round(100.0 * count(*)::numeric / sum(count(*)) OVER (), 1) AS percentual
...
CREATE VIEW vw_ltv_por_categoria AS
...
CREATE VIEW vw_ltv_por_unidade AS
 ...
 WITH alunos_ativos AS (
        ...
        UNION ALL
         ...
         SELECT DISTINCT ON ((COALESCE(m.aluno_id, - m.id)), m.unidade_id) COALESCE(m.aluno_id, - m.id) AS aluno_key,
          ...
          WHERE EXTRACT(year FROM m.data) = EXTRACT(year FROM CURRENT_DATE) AND EXTRACT(month FROM m.data) = EXTRACT(month FROM CURRENT_DATE) AND (m.tipo::text = ANY (ARRAY['evasao'::character varying, 'nao_renovacao'::character varying]::text[])) AND (a.is_segundo_curso IS NULL OR a.is_segundo_curso = false OR a.id IS NULL)
         ...
         SELECT m.unidade_id,
            ...
            count(*) FILTER (WHERE m.tipo::text = ANY (ARRAY['renovacao'::character varying, 'nao_renovacao'::character varying]::text[])) AS total_contratos,
            avg(
                CASE
           ...
           FROM movimentacoes_admin m
          WHERE EXTRACT(year FROM m.data) = EXTRACT(year FROM CURRENT_DATE) AND EXTRACT(month FROM m.data) = EXTRACT(month FROM CURRENT_DATE) AND (m.tipo::text = ANY (ARRAY['renovacao'::character varying, 'nao_renovacao'::character varying]::text[]))
         ...
         SELECT a.unidade_id,
           ...
           FROM alunos a
             LEFT JOIN tipos_matricula tm ON tm.id = a.tipo_matricula_id
          WHERE (a.status::text = ANY (ARRAY['ativo'::character varying, 'trancado'::character varying]::text[])) AND (tm.conta_como_pagante = true OR tm.id IS NULL)
          GROUP BY a.unidade_id
        )
...
CREATE VIEW vw_kpis_gestao_mensal AS
 WITH matriculas_mes AS (
         ...
         SELECT a.unidade_id,
           ...
           FROM alunos a
             LEFT JOIN tipos_matricula tm ON tm.id = a.tipo_matricula_id
          WHERE (a.status::text = ANY (ARRAY['ativo'::character varying::text, 'trancado'::character varying::text])) AND tm.entra_ticket_medio = true AND a.valor_parcela > 0::numeric
