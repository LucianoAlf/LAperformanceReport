...
AS $function$
  ...
  -- desconectado) OU item preso ha +3h sem resolver (nem enviado, nem falho).
  relatorio_stats as (
    select
      count(*) filter (where status = 'falhou' and created_at >= now() - interval '3 days') as falhas,
      count(*) filter (where status in ('pendente','erro') and agendada_para < now() - interval '3 hours') as presos,
