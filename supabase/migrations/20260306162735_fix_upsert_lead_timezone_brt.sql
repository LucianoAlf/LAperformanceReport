...
CREATE OR REPLACE FUNCTION public.upsert_lead(p_nome text, p_telefone text, p_email text, p_unidade_id uuid, p_curso text, p_canal text, p_source_id integer, p_source_type text, p_arquivar boolean)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
  ...
  v_log_nome      TEXT;
BEGIN
  -- Tratar "Não informado" do Emusys como null
  IF UPPER(TRIM(COALESCE(p_nome, ''))) = 'NÃO INFORMADO' THEN
    p_nome := NULL;
  END IF;

  -- Nome seguro para log (nunca NULL)
  ...
  -- 3. Resolver canal_origem_id
  SELECT id INTO v_canal_id
  FROM canais_origem
  WHERE LOWER(nome) = LOWER(TRIM(p_canal))
  LIMIT 1;

  -- 4. Buscar lead existente (prioridade: source_id > telefone+unidade)
  ...
  -- Helper: montar detalhes do log
  v_detalhes := json_build_object(
    'source_id', p_source_id,
    'telefone', p_telefone,
    'canal', p_canal,
    'curso', p_curso,
    'sem_nome', (p_nome IS NULL OR TRIM(p_nome) = ''),
    'sem_telefone', (p_telefone IS NULL OR TRIM(p_telefone) = '')
  )::jsonb;
  ...
  -- 7. INSERT se não encontrado (data_contato em BRT)
  IF p_source_type = 'emusys' THEN
    INSERT INTO leads (
      ...
      1, 'novo', (NOW() AT TIME ZONE 'America/Sao_Paulo')::date, NOW(), NOW()
    )
