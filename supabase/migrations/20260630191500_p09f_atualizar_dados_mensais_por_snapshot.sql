-- ============================================================================
-- P09F - Compatibilidade dados_mensais a partir do snapshot aprovado
-- Projeto alvo: ouqwbbermlzqqvtqwlul
--
-- Objetivo:
-- - Criar RPC service_role para atualizar a tabela compacta dados_mensais
--   usando somente snapshots aprovados/fechados.
-- - Manter modo dry-run por padrao.
--
-- Nao faz:
-- - Nao chama a funcao.
...
BEGIN
  ...
  LOOP
    ...
    v_valores := jsonb_build_object(
      ...
      'churn_rate', COALESCE(NULLIF(v_exec.payload->>'churn_rate', '')::numeric, v_dm.churn_rate, 0),
      'ticket_medio', COALESCE(NULLIF(v_exec.payload->>'ticket_medio', '')::numeric, v_dm.ticket_medio, 0),
      'taxa_renovacao', COALESCE(NULLIF(v_exec.payload->>'taxa_renovacao', '')::numeric, v_dm.taxa_renovacao, 0),
      'tempo_permanencia', COALESCE(NULLIF(v_exec.payload->>'tempo_permanencia', '')::numeric, v_dm.tempo_permanencia, 0),
      'inadimplencia', COALESCE(NULLIF(v_exec.payload->>'inadimplencia', '')::numeric, v_dm.inadimplencia, 0),
      'reajuste_parcelas', COALESCE(NULLIF(v_exec.payload->>'reajuste_parcelas', '')::numeric, v_dm.reajuste_parcelas, 0),
      'faturamento_estimado', COALESCE(NULLIF(v_exec.payload->>'faturamento_estimado', '')::numeric, NULLIF(v_exec.payload->>'mrr', '')::numeric, v_dm.faturamento_estimado, 0),
      'saldo_liquido', COALESCE(NULLIF(v_exec.payload->>'saldo_liquido', '')::integer, v_dm.saldo_liquido, 0),
...
COMMENT ON FUNCTION public.atualizar_dados_mensais_por_snapshot(integer, integer, uuid, boolean) IS
  'Atualiza dados_mensais de compatibilidade usando snapshots aprovados/fechados. Dry-run por padrao; restrita a service_role.';
