...
DROP FUNCTION IF EXISTS public.get_faltas_periodo(uuid, date, date);

CREATE FUNCTION public.get_faltas_periodo(
  p_unidade_id uuid,
  p_data_inicio date,
  p_data_fim date
)
...
GRANT EXECUTE ON FUNCTION public.get_faltas_periodo(uuid, date, date) TO authenticated, anon, service_role;
