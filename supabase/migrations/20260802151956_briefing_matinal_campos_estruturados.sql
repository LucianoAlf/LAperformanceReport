...
comment on function public.fn_briefing_txt(text) is
  'Normaliza texto de campo do registro pro briefing: colapsa espacos, NULL quando vazio, teto de 300 chars.';
          ...
          'alunos', COALESCE((
            ...
            LEFT JOIN LATERAL (
              SELECT jsonb_strip_nulls(jsonb_build_object(
                ...
                'trabalho_feito', public.fn_briefing_txt(COALESCE(fat.campos->>'progresso', fat.campos->>'atividades', reg.campos->>'atividades')),
...
comment on function public.fabio_briefing_matinal(integer, date) is
  'Briefing matinal do professor: agenda do dia + ultima aula registrada por aluno em CAMPOS SEPARADOS (ultima_aula: data/foco/trabalho_feito/repertorio/dever_casa/proximo_passo/observacao) + totais do dia. Mantem resumo_ultima_aula por compatibilidade. Read-only.';
