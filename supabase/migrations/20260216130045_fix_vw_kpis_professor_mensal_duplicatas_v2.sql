...
-- FASE 1.1: Corrigir vw_kpis_professor_mensal para eliminar duplicatas
-- Problema: JOINs com CTEs de experimentais/matriculas/renovacoes/evasoes geram múltiplas linhas
-- Solução: Usar DISTINCT ON (professor_id, unidade_id) para garantir uma linha por professor/unidade
-- Regras de negócio aplicadas:
-- 1. Evasões = só tipo_saida_id IN (1, 2) - excluir Aviso Prévio
-- 2. Ticket médio = só alunos pagantes (valor_parcela > 0)
-- 3. Evasão só conta aluno pagante (valor_parcela > 0)
-- 4. Segundo curso não é evasão (verificado via alunos.is_segundo_curso)
-- 5. Renovações vêm de movimentacoes_admin (fonte de verdade)

DROP VIEW IF EXISTS vw_kpis_professor_mensal CASCADE;

CREATE OR REPLACE VIEW vw_kpis_professor_mensal AS
WITH 
-- Carteira atual: alunos ativos por professor/unidade
-- Regra: só alunos pagantes (valor_parcela > 0) para ticket médio
carteira AS (
    SELECT 
        a.professor_atual_id AS professor_id,
        a.unidade_id,
        COUNT(*) AS carteira_alunos,
        AVG(CASE WHEN a.valor_parcela > 0 THEN a.valor_parcela END) AS ticket_medio,
...
-- Turmas implícitas por professor/unidade
turmas_calc AS (
    SELECT 
        vt.professor_id,
        vt.unidade_id,
...
-- Experimentais e matrículas do MÊS ATUAL (não histórico)
-- Agrupado por professor/unidade apenas (sem ano/mes para evitar duplicatas)
...
-- Evasões do MÊS ATUAL
-- Regras: só tipo_saida_id IN (1, 2), só alunos pagantes, excluir segundo curso
evasoes_atual AS (
    SELECT 
        e.professor_id,
        e.unidade_id,
    ...
    WHERE e.professor_id IS NOT NULL
      AND e.tipo_saida_id IN (1, 2) -- Só Cancelamento e Não Renovação (excluir Aviso Prévio)
      AND (e.valor_parcela > 0 OR e.valor_parcela IS NULL) -- Só alunos pagantes
      AND (a.is_segundo_curso IS NULL OR a.is_segundo_curso = false OR a.id IS NULL) -- Segundo curso não é evasão
...
-- Query principal: DISTINCT ON para garantir uma linha por professor/unidade
SELECT DISTINCT ON (p.id, COALESCE(c.unidade_id, ea.unidade_id, ra.unidade_id, ev.unidade_id))
    ...
    -- Retenção (de movimentacoes_admin)
    ...
    -- Evasões (só tipo 1 e 2, só pagantes, excluir segundo curso)
    ...
    -- Rankings (calculados depois no frontend para evitar problemas com DISTINCT ON)
