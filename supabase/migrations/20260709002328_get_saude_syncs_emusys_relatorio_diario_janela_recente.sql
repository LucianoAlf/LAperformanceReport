...
AS $function$
  ...
  -- fila_relatorios_whatsapp.status, nao do disparo fire-and-forget do pg_cron (que so
  -- confirma o enqueue). 'falhou' cobre falha terminal apos esgotar retries (ex: numero
  -- desconectado) OU item preso ha +3h sem resolver. Janela de 3 dias evita que entradas
  -- antigas travadas (bug: item que bate o teto de tentativas nunca vira 'falhou',
  -- so fica preso em 'erro' pra sempre) gerem falso alarme permanente.
  relatorio_stats as (
    select
      count(*) filter (where status = 'falhou' and created_at >= now() - interval '3 days') as falhas,
      count(*) filter (where status in ('pendente','erro') and created_at >= now() - interval '3 days' and agendada_para < now() - interval '3 hours') as presos,
