-- Hotfix aditivo da classificacao de conflitos de presenca.
--
-- Evidencia comprovada em producao em 27/08/2026:
-- - as gemeas turma + individual do Emusys podem trazer ausente + presente;
-- - ausente bruto e estado nao terminal/default e nao contradiz decisao humana;
-- - falta humana contra presente Emusys continua sendo conflito verdadeiro.
-- Nenhum dado e reescrito, nenhuma flag de rollout e alterada e nenhum backfill e feito.
-- O caminho legado/sombra permanece intocado: muda-lo aqui alteraria o universo
-- operacional publicado antes do cutover governado.
...
with evidencia as not materialized (
  ...
  select
    ...
    ('presenca-ocorrencia-canonica-v2.2'
...
comment on view public.vw_presenca_ocorrencia_canonica_v2 is
  'Read model v2.2: ausencia bruta do Emusys e nao terminal; presenca Emusys vence gemeas brutas; falta humana x presenca Emusys permanece conflito. Experimental segue contrato proprio.';
