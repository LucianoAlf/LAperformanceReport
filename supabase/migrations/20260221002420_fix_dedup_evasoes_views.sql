...
-- Fix: deduplicação de evasões por nome do aluno (antes usava COALESCE(aluno_id, -id) que não deduplicava quando aluno_id era NULL)

-- 1. Corrigir vw_kpis_retencao_mensal
CREATE OR REPLACE VIEW vw_kpis_retencao_mensal AS
WITH evasoes_dedup AS (
    SELECT DISTINCT ON (lower(trim(e.aluno_nome)), e.unidade_id, EXTRACT(year FROM e.data_evasao), EXTRACT(month FROM e.data_evasao))
        e.id,
        e.aluno_id,
        e.aluno_nome,
        e.unidade_id,
        ...
        e.valor_parcela
    FROM evasoes_v2 e
    ORDER BY lower(trim(e.aluno_nome)), e.unidade_id, EXTRACT(year FROM e.data_evasao), EXTRACT(month FROM e.data_evasao), e.aluno_id DESC NULLS LAST, e.data_evasao DESC
), evasoes_mes AS (
    SELECT e.unidade_id,
    ...
    SELECT r.unidade_id,
        ...
        count(*) FILTER (WHERE r.status::text = 'renovado'::text) AS renovacoes_realizadas,
        count(*) FILTER (WHERE r.status::text = ANY (ARRAY['nao_renovado'::text, 'nao_renovada'::text])) AS nao_renovacoes_renovacao,
        count(*) FILTER (WHERE r.status::text = 'pendente'::text) AS renovacoes_pendentes,
        count(*) FILTER (WHERE r.status::text = 'pendente'::text AND r.data_renovacao < CURRENT_DATE) AS renovacoes_atrasadas
    FROM renovacoes r
...
SELECT u.id AS unidade_id,
    ...
    CASE
        WHEN COALESCE(ta.total_pagantes, 0::bigint) > 0 THEN ((COALESCE(em.total_evasoes, 0::bigint) - COALESCE(em.transferencias, 0::bigint))::numeric / ta.total_pagantes::numeric * 100::numeric)::numeric(5,2)
        ELSE 0::numeric
    END AS taxa_evasao,
    ...
    CASE
        WHEN COALESCE(rm.renovacoes_previstas, 0::bigint) > 0 THEN (rm.renovacoes_realizadas::numeric / rm.renovacoes_previstas::numeric * 100::numeric)::numeric(5,2)
        ELSE 0::numeric
    END AS taxa_renovacao,
    CASE
        WHEN COALESCE(rm.renovacoes_previstas, 0::bigint) > 0 THEN (COALESCE(rm.nao_renovacoes_renovacao, 0::bigint)::numeric / rm.renovacoes_previstas::numeric * 100::numeric)::numeric(5,2)
        ELSE 0::numeric
