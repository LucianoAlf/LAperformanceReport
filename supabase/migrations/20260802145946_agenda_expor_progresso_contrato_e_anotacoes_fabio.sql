-- Acrescenta 2 colunas ao retorno de get_agenda_dia. Como o RETURNS TABLE muda,
-- e DROP + CREATE (o CREATE OR REPLACE nao aceita mudanca de tipo de retorno).
-- O corpo e IDENTICO ao anterior exceto pelos dois campos novos.
--
--   qtd_aulas_contrato -> junto com nr_da_aula permite "aula 5 de 48". Segue a
--     MESMA regra do nr_da_aula: so quando a aula tem exatamente 1 contrato,
--     senao o numero seria de um aluno arbitrario da turma.
--   anotacoes_fabio    -> registro do professor pelo LA Teacher.
--
-- ⚠️ CREATE FUNCTION concede EXECUTE a PUBLIC por padrao. O REVOKE no fim nao
-- basta: o projeto tambem tem ALTER DEFAULT PRIVILEGES concedendo a `anon`.
-- A migration 20260802150015 revoga nominalmente de anon logo em seguida.
...
grant execute on function public.get_agenda_dia(date, uuid) to authenticated, service_role;
