-- P25 - Ajusta denominador do ticket medio de faturas Emusys
--
-- O valor financeiro continua vindo das parcelas da API Emusys.
-- O denominador do ticket medio deve contar aluno unico elegivel, nao vinculo/curso.
-- Ex.: segundo curso entra no valor recebido, mas nao duplica o aluno no divisor.
...
begin
  ...
  base as (
    select
      ...
      (
        ...
        and coalesce(apm.conta_como_pagante, false)
        and coalesce(apm.entra_ticket_medio, false)
        and not coalesce(apm.is_segundo_curso, false)
        and not coalesce(apm.is_projeto_banda, false)
      ) as ticket_elegivel,
  ...
  agregada as (
    select
      ...
      count(distinct emusys_student_id) filter (where eh_parcela and status = 'paga' and ticket_elegivel) as alunos_locais_com_parcela_paga,
      count(distinct aluno_local_id) filter (where eh_parcela and aluno_local_id is not null) as vinculos_locais_com_parcela,
      count(distinct aluno_local_id) filter (where eh_parcela and status = 'paga' and aluno_local_id is not null) as vinculos_locais_com_parcela_paga,
  ...
  return jsonb_build_object(
    ...
    'regra', 'Somente descricoes Parcela; MRR atual usa valor_pago; faturamento previsto usa pagas + abertas por vencimento; ticket usa aluno Emusys unico com vinculo local elegivel, sem duplicar segundo curso/banda.',
