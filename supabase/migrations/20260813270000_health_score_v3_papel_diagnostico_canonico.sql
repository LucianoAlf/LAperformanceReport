...
-- Corrige o contrato do papel da metrica sem alterar configuracao ativa ou snapshots.
-- numero_alunos continua visivel como diagnostico, mas nunca entra na nota/cobertura.
begin;
...
as $function$
  ...
  parametros as (
    ...
    select
      ...
      case
        when cm.metrica = 'numero_alunos' then 'diagnostico'::text
        else coalesce(nullif(cm.parametros ->> 'papel', ''), 'nota')::text
      end as papel
  ...
  select
    ...
    case when b.metrica = 'numero_alunos' then 'diagnostico'::text else b.papel end,
    b.detalhes,
    ...
    b.classificacao_referencia, b.data_corte, b.config_id,
    b.regra_fingerprint, b.peso_pontuavel_total,
    b.peso_disponivel_total, b.cobertura_normalizada,
    b.cobertura_minima_aplicada, b.comparabilidade_motivos
  from base_unica b
...
comment on function public.get_health_score_professor_v3_performance(
  ...
  'Read model V3: monta a matriz canonica roster elegivel x seis metricas; numero_alunos e diagnostico e a cobertura usa os cinco pilares pontuaveis.';
