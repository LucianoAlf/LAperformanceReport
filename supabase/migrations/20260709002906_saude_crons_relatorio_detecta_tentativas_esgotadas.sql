...
AS $function$
  ...
  -- 'falhou' cobre: (a) falha terminal explicita ('falhou'); (b) tentativas esgotou o
  -- teto (RELATORIO_MAX_TENTATIVAS=8) sem sucesso — sinal imediato, nao precisa esperar
  -- o relogio; (c) item preso ha +3h sem resolver (fallback pra outros casos travados).
  -- Janela de 3 dias evita alarme permanente por lixo antigo (bug conhecido: item que
  -- bate o teto de tentativas nunca vira 'falhou' sozinho, fica preso em 'erro' pra sempre).
  relatorio_stats as (
    select
