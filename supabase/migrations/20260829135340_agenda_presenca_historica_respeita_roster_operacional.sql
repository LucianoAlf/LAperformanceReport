-- Complemento aditivo: aluno_presenca preserva a evidencia historica, mas nao
-- pode reativar na Agenda um vinculo explicitamente inativado pelo roster.
-- Registros legados sem qualquer linha de roster continuam visiveis.

-- A Agenda operacional deve consumir apenas o roster vigente. A reconciliacao
-- preserva os vinculos antigos em aula_alunos_emusys para auditoria, marcando-os
-- como ativo_operacional=false; eles nao podem reaparecer na chamada nem fazer um
-- professor sem aula vigente entrar na lista de presenca.
--
-- A funcao mantem assinatura, SECURITY INVOKER, filtros de cancelamento e todo o
-- contrato publicado. As unicas mudancas sao os dois filtros do roster nominal.
...
with base as (
  ...
  where ae.data_aula = p_data
    ...
    and (
      ae.cancelada is not true
      or ae.cancelada_origem is distinct from 'sync_ausente_emusys'
    )
),
...
participantes as (
  ...
  where ap.aluno_id is not null
    and (
      not exists (
        select 1
        from aula_alunos_emusys aa_roster
        where aa_roster.aula_emusys_id = ap.aula_emusys_id
          and aa_roster.aluno_id = ap.aluno_id
      )
      or exists (
        select 1
        from aula_alunos_emusys aa_roster
        where aa_roster.aula_emusys_id = ap.aula_emusys_id
          and aa_roster.aluno_id = ap.aluno_id
          and aa_roster.ativo_operacional
      )
    )
),
experimentais as (
...
vinculos_cru as (
  ...
  where aa.ativo_operacional
    and not exists (
    select 1 from alunos al_filter
    where al_filter.id = aa.aluno_id
      and (
      al_filter.status = 'evadido'
      or (al_filter.status = 'trancado'
          and exists (
              select 1 from aluno_jornada_matricula_disciplina j_tranc
              where j_tranc.aluno_id = al_filter.id
                and j_tranc.trancamento_data_inicial is not null
                and j_tranc.trancamento_data_inicial <= b.data_aula
                and j_tranc.status_matricula = 'trancada'
          ))
    )
  )
),
