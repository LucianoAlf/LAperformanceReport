-- P02AB: reaplica a definicao P02Y da conciliacao de experimentais.
-- Motivo: uma migracao posterior preservou o comentario de P02Y, mas
-- sobrescreveu o corpo da RPC com a logica anterior baseada em lead_experimentais.
...
raw_por_unidade as (
  select
    ...
    count(r.*) filter (where r.situacao_operacional = 'faltou')::int as raw_faltas_emusys,
    count(r.*) filter (where r.situacao_operacional = 'cancelada')::int as raw_canceladas_emusys,
...
resumo_base as (
  select
    ...
    case
      ...
      then coalesce((select sum(raw_realizadas_emusys) from raw_por_unidade), 0)::int
...
select jsonb_build_object(
  ...
  'fonte_taxa_exp_mat', jsonb_build_object(
    'status', 'p02y_raw_emusys_denominador',
    'denominador', 'experimental realizada no raw Emusys quando disponivel; fallback para conciliacao/funil',
...
comment on function public.get_conciliacao_experimentais_v2(uuid, integer, integer, text, date) is
  'Conciliacao v2 de experimentais. Usa raw Emusys como denominador canonico de realizadas quando disponivel e ignora eventos antigos substituidos por reagendamento.';
