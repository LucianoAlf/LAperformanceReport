-- P10B: separa experimentais comerciais de remanejamentos internos.
-- Motivo: o raw Emusys e a fonte canonica de presenca, mas linhas com
-- aluno.id_lead = 0 e aluno.id_aluno preenchido representam aluno ja cadastrado,
-- usadas para troca de professor/curso e nao devem inflar o funil comercial.
...
select jsonb_build_object(
  ...
  'fonte_taxa_exp_mat', jsonb_build_object(
    'status', 'p10b_raw_emusys_comercial_sem_remanejamento',
    'denominador', 'experimental realizada no raw Emusys, removendo remanejamento interno de aluno ja cadastrado e decisoes humanas que nao entram na taxa; fallback para conciliacao/funil',
...
comment on function public.get_conciliacao_experimentais_v2(uuid, integer, integer, text, date) is
  'Conciliacao v2 de experimentais. Usa raw Emusys como fonte canonica, mas separa remanejamento interno de aluno ja cadastrado e decisoes humanas excluidas do denominador comercial.';
