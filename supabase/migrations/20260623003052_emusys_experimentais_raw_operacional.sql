...
as $$
  ...
  por_unidade as (
    select
      ...
      count(*) filter (where b.situacao_operacional = 'presente')::integer as presentes,
      count(*) filter (where b.situacao_operacional = 'matriculado')::integer as matriculados,
      count(*) filter (where b.situacao_operacional in ('presente', 'matriculado'))::integer as realizadas_emusys,
      count(*) filter (where b.situacao_operacional = 'faltou')::integer as faltas,
      count(*) filter (where b.situacao_operacional = 'cancelada')::integer as canceladas,
      count(*) filter (where b.aluno_id is null)::integer as sem_aluno_id,
      count(*) filter (where b.lead_id is null)::integer as sem_lead_id
