-- Evita que o planner recalcule os agregados canonicos para cada linha ranqueada.
-- Mudanca de plano apenas: hashes de agosto/2026 identicos nas tres unidades.
...
with evidencia as not materialized (
  ...
  where coalesce(ae.categoria, 'normal') = 'normal'
    and ae.unidade_id = %L::uuid
    and ae.data_aula between %L::date and %L::date
  ...
  where ap.unidade_id = %L::uuid
    and ap.data_aula between %L::date and %L::date
    and not exists (
    select 1
    from public.aulas_emusys ae
...
), agregada_regular as materialized (
  select
    c.slot_key,
    c.unidade_id,
    c.data_aula,
...
), agregada_multidata as materialized (
  select
    c.slot_key,
    array_agg(distinct c.aula_emusys_id order by c.aula_emusys_id)
      filter (where c.aula_emusys_id is not null) as ids_aulas_emusys,
...
comment on function public.fn_presenca_ocorrencia_canonica_escopada_v2(
  uuid, date, date
) is 'Ocorrencia canonica v2.7 parametrizada com plano literal seguro e recorte anterior ao ranking.';
