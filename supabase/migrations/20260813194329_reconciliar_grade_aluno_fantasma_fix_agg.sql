...
begin
  ...
  select
    ...
    -- decisao de nao mexer fica invisivel no log)
    ...
    -- ids PLANOS. array_agg(j.ids) devolveria int[][] e quebra quando os slots tem
    -- numeros diferentes de linhas (2 vs 3); o unnest achata antes de reagregar.
...
revoke all on function public.reconciliar_grade_aluno_v1(integer, uuid, date, date, integer[], boolean) from public, anon, authenticated;
grant execute on function public.reconciliar_grade_aluno_v1(integer, uuid, date, date, integer[], boolean) to service_role;
