-- get_candidatos_pesquisa_primeira_aula e SECURITY INVOKER e le whatsapp_caixas,
-- que tem RLS ligado e ZERO policies (tabela trancada de proposito: guarda
-- credencial de caixa). Resultado: 42501 para TODO usuario autenticado desde
-- 06/07/2026 -- a aba Pos-1a Aula nao abre. O cron nunca sentiu porque roda
-- como service_role, que ignora RLS.
--
-- A RPC so precisa do campo `departamento`. Expomos uma view minima com
-- security_invoker = false, mantendo a tabela trancada.
--
-- ATENCAO (regra do projeto): ALTER DEFAULT PRIVILEGES tambem pega VIEW -- toda
-- relacao nova em public nasce com authenticated=arwdDxtm. View simples e
-- auto-atualizavel e, com security_invoker = false, a escrita rodaria com os
-- direitos do DONO. Por isso: revoke all PRIMEIRO, grant select depois.

create or replace view public.vw_whatsapp_caixas_departamento
with (security_invoker = false) as
select id, departamento
from public.whatsapp_caixas;

revoke all on public.vw_whatsapp_caixas_departamento from public, anon, authenticated;
grant select on public.vw_whatsapp_caixas_departamento to authenticated, service_role;

comment on view public.vw_whatsapp_caixas_departamento is
...
RETURNS TABLE(aluno_id integer, unidade_id uuid, nome text, unidade_nome text, curso_nome text, professor_nome text, data_primeira_aula date, data_matricula date, whatsapp_jid text, status text, nota integer)
LANGUAGE sql
STABLE
AS $function$
  ...
  candidatos AS (
    ...
    LEFT JOIN LATERAL (
      ...
      JOIN vw_whatsapp_caixas_departamento wc ON wc.id = ac2.caixa_id
