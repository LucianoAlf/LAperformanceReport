...
-- 2026-08-11 — Correcao: retificacao nao exige mais motivo obrigatorio.
--
-- A regra anterior bloqueava mudar de falta->presente (ou qualquer
-- retificacao de resposta humana) sem motivo. Mas a propria secretaria
-- marcando falta sem querer ficava presa: o respondido_por virou
-- 'agenda_secretaria' (humano), e corrigir exigia motivo.
--
-- Agora: a retificacao ainda e REGISTRADA no historico (para auditoria),
-- mas o motivo e opcional. O motivo continua obrigatorio so para
-- falta_justificada (regra de negocio separada, mantida).
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_itens) loop
    ...
    if found then
      ...
      -- presente->falta, etc). O motivo e opcional — se vier, fica no
      -- historico; se nao, a retificacao e registrada sem motivo.
...
-- Grants preservados (create or replace nao derruba grants, mas garantimos)
