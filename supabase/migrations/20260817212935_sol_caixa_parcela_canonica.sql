...
-- Casador de parcela do CAIXA na fonte canônica (contrato v4, 17/08/2026).
-- Não altera sol_caixa_casar_parcela (congelada pelo contrato): esta é nova e lê SOMENTE
-- get_faturas_alunos_financeiro_v1. Corrige os 2 erros do preview de 17/08:
--   (a) mostrava valor_sem_desconto_condicional (447) como "valor da parcela" — o certo é
--       valor_com_desconto (417), e valor_hoje quando vencida;
--   (b) pulava para a competência seguinte porque só olhava fatura ABERTA — a parcela do mês
--       já constava PAGA. Agora ela reconhece "essa parcela já consta paga".
create or replace function public.sol_caixa_parcela_canonica(
  p_unidade_id uuid, p_aluno text, p_valor numeric default null, p_as_of date default null)
...
declare
  ...
  v_env jsonb; v_itens jsonb; v_it jsonb; v_esc jsonb; v_motivo text;
  v_status_env text;
begin
  ...
  -- 1) aberta VENCIDA (atrasada) — a mais antiga
  ...
  -- 2) aberta do mês vigente (a vencer)
  ...
  -- 3) já PAGA e compatível com este pagamento (mesmo valor, baixa recente)
  ...
  -- 4) qualquer aberta restante (mais antiga)
  if v_esc is null then
    select x into v_esc from jsonb_array_elements(v_itens) x
  ...
  return jsonb_build_object(
    ...
    'motivo_escolha', v_motivo,
    'fonte_status', v_status_env,
    'total_faturas_janela', jsonb_array_length(v_itens),
    'fatura', jsonb_build_object(
      'canonical_fatura_id', v_esc->>'canonical_fatura_id',
      'emusys_fatura_id', v_esc->>'emusys_fatura_id',
      'tipo_fatura', v_esc->>'tipo_fatura',
      'descricao', v_esc->>'descricao',
      'numero_parcela', v_esc->'numero_parcela',
      'total_parcelas_contrato', v_esc->'total_parcelas_contrato',
      'competencia', v_esc->>'competencia',
      'data_vencimento', v_esc->>'data_vencimento',
      'status', v_esc->>'status',
      'data_pagamento', v_esc->>'data_pagamento',
      'vencida', coalesce((v_esc->'cobranca'->>'d0')::boolean,false),
      'dias_atraso', case when coalesce((v_esc->'cobranca'->>'d0')::boolean,false)
                          then greatest(0, v_as_of - (v_esc->>'data_vencimento')::date) else 0 end,
...
revoke all on function public.sol_caixa_parcela_canonica(uuid, text, numeric, date) from public, anon, authenticated;
grant execute on function public.sol_caixa_parcela_canonica(uuid, text, numeric, date) to service_role;
