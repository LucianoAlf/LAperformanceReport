...
-- Corrigir ticket médio por professor para excluir alunos com valor_parcela = 0
-- Regra: Ticket Médio = SUM(valor_parcela de TODOS) / COUNT(alunos com valor_parcela > 0)
-- Alunos de banda, coral, bolsistas (valor = 0) não entram no denominador
...
carteira AS (
  SELECT 
    ...
    -- CORREÇÃO: Ticket médio = SUM(parcelas de todos) / COUNT(alunos PAGANTES, ou seja, valor_parcela > 0)
    -- Alunos com valor_parcela = 0 (banda, coral, bolsistas) NÃO entram no denominador
    CASE 
      WHEN COUNT(*) FILTER (WHERE a.valor_parcela > 0) > 0 
