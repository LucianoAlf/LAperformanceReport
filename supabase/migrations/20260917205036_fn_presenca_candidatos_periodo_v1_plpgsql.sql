-- Convertida de "language sql" para plpgsql + return query, mesmo padrao de
-- fn_presenca_ocorrencias_escopo_interno_v2. Funcao SQL SECURITY DEFINER nao e
-- inlineavel e estava executando com plano ruim: 2.855ms contra 26ms da mesma
-- logica como query solta.
drop function if exists public.fn_presenca_candidatos_periodo_v1(uuid, date, date);

create function public.fn_presenca_candidatos_periodo_v1(
  p_unidade_id uuid,
  p_data_inicio date,
  p_data_fim date
)
...
begin
  ...
  with alvo as (
    ...
    select
      ...
      un.aluno_id as c_aluno_id,
      a.id as c_aula_emusys_id,
      a.unidade_id as c_unidade_id,
      a.professor_id as c_professor_id,
      a.data_aula as c_data_aula,
      a.data_hora_inicio as c_data_hora_inicio,
      a.data_hora_fim as c_data_hora_fim,
      a.curso_nome as c_curso_nome,
      a.turma_nome as c_turma_nome,
      coalesce(re.estado, 'sem_fotografia') as c_roster_estado,
