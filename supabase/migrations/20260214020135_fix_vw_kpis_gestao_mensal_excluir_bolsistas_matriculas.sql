...
-- Regra de negócio: matrícula = quem pagou passaporte
-- Bolsista integral NÃO conta como matrícula
-- Adicionar filtro para excluir bolsistas integrais da contagem de matrículas

CREATE OR REPLACE VIEW vw_kpis_gestao_mensal AS
WITH matriculas_mes AS (
    -- Fonte: tabela leads (consistente com página Comercial)
    -- Exclui bolsistas integrais: matrícula = quem pagou passaporte
    SELECT 
        l.unidade_id,
