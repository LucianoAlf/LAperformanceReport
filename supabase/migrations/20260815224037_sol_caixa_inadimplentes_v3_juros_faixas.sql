...
-- Inadimplentes por unidade (read-only, service_role). Fonte: emusys_faturas vencidas.
-- Valor = valor_original (cheio, sem desconto de pontualidade) + multa + mora (por parcela).
-- Juros PROVISORIO (multa 2% + mora 1%/mes pro rata) ate confirmar o contrato. So ATIVOS.
-- Faixas: normal (<=grave), atencao (>grave), critico (>critico).
...
as $$
  with cob as (
    select f.emusys_student_id,
           ...
           max(current_date - f.data_vencimento) dias,
           sum(f.valor_original) valor_original,
           sum(f.valor_original * (1 + p_multa_pct
                 + p_mora_pct_mes * ((current_date - f.data_vencimento)::numeric / 30.0))) valor_atualizado
...
-- limpa a assinatura antiga (3 args) pra nao ficar duas versoes
drop function if exists public.sol_caixa_inadimplentes(uuid,int,int);
