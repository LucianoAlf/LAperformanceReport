-- Acrescenta o enriquecimento do Meta Ads à saúde das automações.
--
-- O hook useSaudeCrons já mapeava `enriquecer-meta-ads-diario` -> 'meta_ads', mas a RPC
-- NUNCA devolveu esse tipo: mapeamento órfão, o cron seguia como "não medido".
--
-- Evidência: `meta_ads_cache.enriquecido_em`. O cron roda 10 8 * * * UTC (05:10 BRT), então
-- 26h de tolerância cobre um dia perdido sem alarme falso.
--
-- ⚠️ NÃO foram instrumentados os três alertas de projeto (`alertas-diarios`,
-- `alertas-tarefas-atrasadas`, `resumo-semanal`), embora também tivessem mapeamento órfão:
-- eles são CONDICIONAIS e `projeto_tarefas` está vazia (0 tarefas, 7 projetos). Medir por
-- "tempo desde o último envio" deixaria os três permanentemente vermelhos por silêncio
-- legítimo — falso positivo que ensina a ignorar a tela. Os mapeamentos foram removidos do
-- hook; se o módulo de projetos passar a ser usado, medir por execução-sem-erro, não por
...
as $function$
  ...
  -- Meta Ads: o cron enriquece nome de anúncio/campanha via Graph API. Sem ele, o lead de
  -- anúncio fica com source_id cru e o Tráfego Pago perde o nome da campanha.
  meta_ads as (
    select 'meta_ads'::text as sync_tipo, null::uuid as unidade_id,
           max(enriquecido_em) as ultima, null::text as forcar_status
