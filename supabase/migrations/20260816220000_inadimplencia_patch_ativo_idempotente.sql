-- Reaplica a regra de escopo financeiro de forma idempotente e tolerante à
-- formatação produzida por pg_get_functiondef durante um replay.
...
begin
  ...
  if strpos(v_definition, 'active_or_locked') > 0 then
    ...
    if v_definition = v_before
      ...
      raise exception 'patch idempotente nao encontrou todos os trechos da regra ativa';
    end if;
...
set
  ...
  updated_at = now(),
  updated_by = 'migration:20260816220000'
