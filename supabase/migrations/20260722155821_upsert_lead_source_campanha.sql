...
BEGIN
  ...
  ELSIF p_source_type = 'campanha' THEN
    -- Lead vindo do agente de campanha (WhatsApp): so existe match por telefone+unidade
  ...
  IF v_lead_id IS NOT NULL THEN
    ...
    ELSIF p_source_type = 'nocodb' THEN
      ...
      WHERE id = v_lead_id;
    ELSIF p_source_type = 'campanha' THEN
      -- Lead ja existe (Emusys/NocoDB/outra campanha ja o criou): o agente de campanha
      -- NAO sobrescreve dados de sistema (nome/canal/curso/status/etapa). So marca contato.
  ...
  ELSIF p_source_type = 'nocodb' THEN
    ...
    RETURNING id INTO v_lead_id;
  ELSIF p_source_type = 'campanha' THEN
    -- Lead novo, vindo do agente de campanha (WhatsApp). Sem emusys_lead_id/nocodb_lead_id:
    -- o match futuro com o Emusys se da por telefone+unidade (mesmo ON CONFLICT abaixo).
    INSERT INTO leads (nome, telefone, email, unidade_id, curso_interesse_id, canal_origem_id, etapa_pipeline_id, status, data_contato, created_at, updated_at)
    VALUES (p_nome, p_telefone, p_email, p_unidade_id, v_curso_id, v_canal_id, 1, 'novo', COALESCE(p_data_contato, (NOW() AT TIME ZONE 'America/Sao_Paulo')::date), NOW(), NOW())
    ON CONFLICT (telefone, unidade_id) WHERE telefone IS NOT NULL AND arquivado = false
    DO UPDATE SET
      nome = COALESCE(NULLIF(EXCLUDED.nome, ''), leads.nome),
      email = COALESCE(EXCLUDED.email, leads.email),
