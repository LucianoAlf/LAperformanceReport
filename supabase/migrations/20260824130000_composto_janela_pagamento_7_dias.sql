-- Composto: candidata paga passa a valer por 7 DIAS, não só no próprio as_of.
-- Corrige o fix de 22/08 (20260822240000) na primeira vez que ele encontrou o mundo real.
--
-- O CASO (24/08/2026, Recreio): a Valentina Mendes Rodrigues Aleixo faz DOIS cursos
-- (Canto R$ 418,91 + Teclado R$ 395,90 = R$ 814,81). Ela pagou os dois em **21/08 às
-- 22:57** (sexta à noite) e a Fernanda só mandou o comprovante no grupo na **segunda,
-- 24/08 09:05**. Com a regra "paga só no as_of", nenhuma das duas era candidata →
-- `composicao_exige_duas_faturas` → o runtime caiu no casamento simples, pegou uma
-- parcela só e avisou "o comprovante difere do valor da parcela".
--
-- A PREMISSA ERRADA era minha: "o comprovante que chega no grupo é de um pagamento de
-- hoje". Não é — pagamento de sexta à noite, sábado ou feriado chega no próximo dia útil,
-- e é justamente quando mais chega comprovante acumulado. O nome do arquivo enviado
-- (`Comprovante_20260821_225734.pdf`) carrega a data do pagamento, não a do envio.
--
-- A REGRA NOVA: candidata = fatura **aberta** OU **paga nos últimos 7 dias corridos**.
--   • cobre sexta→segunda, feriado prolongado e o acúmulo de fim de semana;
--   • continua matando o problema que motivou o fix (parcelas de jun/jul criando
--     combinações falsas — a Valentina tinha 6 matches antes);
--   • quem paga hoje uma parcela atrasada de junho continua entrando (o corte é pela
--     DATA DE PAGAMENTO, não pela competência).
--
-- ⚠️ Lição registrada: as duas correções minhas que quebraram na segunda-feira
-- (a trava de abertura em 20260824110000 e esta) partiram de uma janela temporal
-- apertada demais, validada contra o caso do dia e não contra o calendário real da
-- operação — que tem fim de semana, feriado e comprovante atrasado.
...
begin
  ...
  -- Candidata: aberta OU paga nos ultimos 7 dias. O comprovante chega no grupo com
  -- atraso (fim de semana/feriado): a Valentina pagou sexta 22:57 e o comprovante veio
  -- na segunda. Pagamentos antigos seguem fora, para nao criar combinacao artificial.
  with candidatos as (
    select x
    from jsonb_array_elements(coalesce(v_env->'items','[]'::jsonb)) x
