...
raw_por_professor as (
  ...
  group by r.professor_id, r.unidade_id, ua.unidade_nome
),
eventos as (
  select
    le.id,
...
classificados_por_professor as (
  select
    ...
    c.unidade_id,
    c.unidade_nome,
    count(distinct c.aluno_taxa_id) filter (
      where c.incluir_taxa_exp_mat
        and c.contar_taxa_exp_mat
        and c.aluno_taxa_id is not null
...
base as (
  select
    ...
    greatest(
      coalesce(r.raw_matriculas_pos_exp, 0),
      coalesce(c.conciliacao_matriculas_pos_exp, 0)
