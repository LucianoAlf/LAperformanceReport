...
-- Competencias passadas podem possuir uma fotografia materializada, mas ainda
-- nao ter passado pelo fechamento oficial do ciclo de vida. A leitura nao deve
-- apagar esse retrato: entrega o estado explicito (parcial/sem_base) e deixa
-- ranking e classificacao oficial bloqueados pelos campos do snapshot.
--
-- O materializador continua append-only e este patch nao promove nenhum
-- snapshot. Ele apenas permite que o leitor veja a ultima revisao provisoria
-- historica ja materializada.
...
comment on function public.get_health_score_professor_v3_performance_snapshot_v1(
  date, uuid, text
) is 'Leitor direto da ultima fotografia V3; competencias historicas provisoriamente materializadas sao visiveis como parciais, sem promover ranking ou fechamento oficial.';
