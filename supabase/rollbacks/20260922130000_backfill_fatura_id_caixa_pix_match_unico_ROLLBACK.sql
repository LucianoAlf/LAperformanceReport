-- ROLLBACK de 20260922130000_backfill_fatura_id_caixa_pix_match_unico.sql
--
-- Desfaz o vinculo somente onde a linha continua EXATAMENTE como o reparo
-- deixou (fatura_id e aluno_id iguais ao par congelado). Linha alterada depois
-- por outro caminho e preservada: a contagem divergente aborta com exception
-- em vez de apagar decisao alheia.
-- Nenhuma das 146 tinha aluno_id antes do reparo (verificado em 22/09/2026),
-- entao restaurar null e fiel ao estado anterior.
...
begin
  ...
  if v_count <> v_expected then
    raise exception 'rollback_backfill_fatura: lista congelada divergente (% de %)', v_count, v_expected;
  end if;

  -- So desfaz par que segue EXATAMENTE como o reparo deixou.
  ...
  if v_count = 0 then
    raise notice 'rollback_backfill_fatura: nada aplicado — nada a desfazer';
    return;
  end if;
  if v_count <> v_expected then
    raise exception 'rollback_backfill_fatura: % de % pares divergem do estado do reparo — revisar antes de desfazer', v_count, v_expected;
  end if;
  ...
  if v_changed <> v_expected then
    raise exception 'rollback_backfill_fatura: esperava desfazer %, desfez %', v_expected, v_changed;
  end if;

  raise notice 'rollback_backfill_fatura: % vinculos desfeitos', v_changed;
end
$$;
