...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: mensagem, error: msgError } = await supabase
      .from('crm_mensagens')
      .select('id, whatsapp_message_id, direcao, tipo, conteudo')
      .eq('id', mensagem_id)
      .single();
    ...
    // 2. Chamar UAZAPI /message/edit
    let baseUrl = UAZAPI_URL;
    if (!baseUrl.startsWith('http://') && !baseUrl.startsWith('https://')) {
      baseUrl = 'https://' + baseUrl;
    }

    console.log(`[editar-mensagem-lead] Editando mensagem ${mensagem.whatsapp_message_id}...`);

    const uazapiResponse = await fetch(`${baseUrl}/message/edit`, {
      method: 'POST',
      headers: {
