...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: mensagem, error: msgError } = await supabase
      .from('crm_mensagens')
      .select('id, whatsapp_message_id, tipo, transcricao, conversa_id')
      .eq('id', mensagem_id)
      .single();
    ...
    console.log(`[transcrever-audio] Transcrevendo áudio ${mensagem.whatsapp_message_id} (caixa: ${creds.caixaNome})...`);

    const uazapiResponse = await fetch(`${creds.baseUrl}/message/download`, {
      method: 'POST',
      headers: {
