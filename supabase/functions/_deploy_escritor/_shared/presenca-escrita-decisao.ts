...
export function decidirEscritaProfessor(input: {
  aulaCancelada: boolean;
  identidadeOk: boolean;
  // professor_presenca/_origem de aulas_emusys — estado vigente da Agenda.
  // A ficha do LA Teacher nao pode transformar em presenca uma falta que a
  // secretaria declarou: mexe com ponto e folha.
  professorPresencaVigente?: string | null;
  professorPresencaOrigem?: string | null;
  marca: MarcaEmusys;
  ultimaEscrita: { presente: boolean } | null;
}): DecisaoEscrita {
  ...
  const vigente = input.professorPresencaVigente ?? null;
  const origemHumana = input.professorPresencaOrigem != null
    && input.professorPresencaOrigem !== 'emusys';
  // Sinal 1 de falta humana: o banco diz 'ausente' com origem declarada
  // (agenda_secretaria; origem null e o inconclusivo do sync e nao protege).
  if (vigente === 'ausente' && origemHumana) {
    return { acao: 'pular', decisao: 'conflito_marca_humana', motivo: 'falta_vigente_secretaria' };
  }
  if (input.marca === 'presente') {
    return { acao: 'pular', decisao: 'ja_coerente', motivo: 'professor_ja_presente' };
  }
  // Sinal 2: o ultimo 'escrito' nosso foi presente:false — a falta no Emusys
  // foi escrita por nos a pedido da secretaria (ausente nao carimba: rele
  // 'sem_resposta'). Se o vigente ja virou 'presente', a secretaria mudou de
  // ideia e a ficha e coerente com a decisao nova — segue o fluxo normal.
  if (input.ultimaEscrita?.presente === false && vigente !== 'presente') {
    return { acao: 'pular', decisao: 'conflito_marca_humana', motivo: 'falta_escrita_nossa' };
  }
  if (input.marca === 'sem_resposta') {
