...
async function processarFichasProfessor(
  ...
  // professor_presenca/_origem entram junto: a falta dada pela secretaria na
  // Agenda e' decisao humana e a ficha nao pode troca-la por presenca.
  const { data: aulasRows } = await supabase
    .from('aulas_emusys')
    .select('id,emusys_id,professor_presenca,professor_presenca_origem')
    .in('id', aulaIds);
  const emusysPorAula = new Map<number, number>(
    (aulasRows ?? []).filter((a: { emusys_id: number | null }) => a.emusys_id != null)
      .map((a: { id: number; emusys_id: number }) => [a.id, a.emusys_id]),
  );
  const vigentePorAula = new Map<number, { presenca: string | null; origem: string | null }>(
    (aulasRows ?? []).map((a: { id: number; professor_presenca: string | null; professor_presenca_origem: string | null }) =>
      [a.id, { presenca: a.professor_presenca, origem: a.professor_presenca_origem }]),
  );
  ...
  for (const ficha of pendentes) {
    ...
    const vigenteAula = vigentePorAula.get(ficha.aula_id) ?? null;
    const estadoAntes = {
      aula_get_id: aulaApiId,
      aula_cancelada: Boolean(aula.cancelada),
      professor: professorNaAula,
      professor_presenca_vigente: vigenteAula?.presenca ?? null,
      professor_presenca_origem: vigenteAula?.origem ?? null,
    };
    ...
    const decisao: DecisaoEscrita = decidirEscritaProfessor({
      aulaCancelada: Boolean(aula.cancelada),
      identidadeOk,
      professorPresencaVigente: vigenteAula?.presenca ?? null,
      professorPresencaOrigem: vigenteAula?.origem ?? null,
      marca,
      ultimaEscrita: ultima,
    });
