...
- **Atendimento por agente (Chatwoot — desde 2026-07-11):** sub-aba **"Atendimento"** em Analytics → Comercial (`TabComercialNew.tsx`, ao lado de Leads/Experimentais/Matrículas). Mostra performance de atendimento por agente (conversas atribuídas, resolvidas, tempo médio de 1ª resposta/resposta/resolução) via edge `chatwoot-atendimento-insights` (proxy read-only 100% ao vivo, sem cache — mesmo racional do `meta-ads-insights`). Edge chama `GET /api/v2/accounts/5/summary_reports/agent` + `/api/v1/accounts/5/agents` em paralelo; hook `useChatwootAtendimentoInsights` (fetch lazy, só quando a sub-aba está ativa) + `src/lib/chatwootAtendimento.ts` (tipos + `formatarDuracaoSegundos`). Usa o filtro de competência do topo da página (Mês/Trim/Semestre); a edge converte ano/mês em janela epoch BRT. ⚠️ **O endpoint `summary_reports/agent` IGNORA filtro de inbox** — sempre agrega a conta inteira, então a sub-aba NÃO recorta por unidade (aviso visível na UI). ⚠️ **Chatwoot REST bloqueia tokens de BOT** (`401 "not authorized for bots"`) — os secrets `CHATWOOT_URL`/`CHATWOOT_ACCOUNT_ID`/`CHATWOOT_API_TOKEN` usam token de **usuário admin**, não o token de bot do `.mcp.json`. URL real: `crmchat.agenticflowio.com.br` (o antigo `chatmusic.poliredeomnichat.com.br` da doc dá 404). Sem restrição de acesso extra (só `verify_jwt`). Detalhes em `.claude/memory/chatwoot.md`.
