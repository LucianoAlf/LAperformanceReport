...
# Emusys limita 120 req/60s (medido 26/08/2026) e a varredura do relatorio faz ~107
# paginas do GET /aulas. Sem espacamento as 3 unidades estouram a cota e a edge
# devolve FALHA_UPSTREAM_EMUSYS. A pausa distribui; o retry cobre a colisao com o
# cron sync-metadados-aulas-15m, que chama a mesma edge no mesmo minuto.
PAUSA_ENTRE_UNIDADES_S = 40
# O lease da admissao do snapshot e de 150s: antes disso o retry recebe
# SNAPSHOT_EXPERIMENTAIS_RETRY_BLOQUEADO em vez de tentar de verdade.
RETRY_ESPERA_S = 170
RETRY_MAX = 1
...
def gerar_com_retry(base, key, unidade_id, data_referencia, indice):
    if indice > 0:
        time.sleep(PAUSA_ENTRE_UNIDADES_S)
    tentativa = 0
    ultimo = None
    while True:
        try:
            ultimo = edge_dry_run_comercial(base, key, unidade_id, data_referencia)
        except Exception as exc:
            ultimo = {'success': False, 'error': str(exc)[:500]}
        if isinstance(ultimo, dict) and ultimo.get('success') and ultimo.get('texto'):
            return ultimo
        if tentativa >= RETRY_MAX:
            return ultimo
        tentativa += 1
        time.sleep(RETRY_ESPERA_S)
...
def main():
    ...
    for index, unit in enumerate(units):
        generated = gerar_com_retry(base, key, unit['id'], today, index)
        if not generated or not generated.get('success') or not generated.get('texto'):
            ...
            # 'error' e sempre a string generica da edge; a causa real vem em 'detalhe'.
            error_detalhe = generated.get('detalhe') if isinstance(generated, dict) else None
            summary['results'].append({
                'unidade': unit['nome'],
                'status': 'erro_geracao',
                'error': error_message,
                'detalhe': error_detalhe,
            })
            continue
