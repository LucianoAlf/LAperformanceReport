...
const groupEngagementPolicy = groupEngagement.createGroupEngagementPolicy({
  gruposQueRespondem: GRUPOS_QUE_RESPONDEM,
  janelaMs: GRUPO_JANELA_ATIVA_MS,
});

function abrirJanelaGrupo(chatId, senderId, motivo = 'unknown') {
  const rec = groupEngagementPolicy.abrirJanela({ chatId, senderId, motivo });
  if (rec) {
    try {
      console.log(JSON.stringify({ event: 'group_continuation_window', chatId, senderId, motivo, until: new Date(rec.until).toISOString(), ttlMs: GRUPO_JANELA_ATIVA_MS }));
    } catch {}
  }
  return rec;
}
          ...
          try {
            ...
            if (_abf) {
              const _sf = async (cid, txt) => {
                ...
                // pode conversar sem ser interceptado pela Sol.
            ...
            if (_fh) {
              ...
              // ⚠️ Elogio/agradecimento NAO leva guarda: ela respondeu "nao entendi" a
              // "Certinho" (Jhon/CG 25/08). Guarda que fala em cima de conversa vira ruido,
              // e ruido ensina a equipe a ignorar o aviso quando ele importa. Segue calada.
              const _souConversa = !!(_fh.ehConversaSemComando && _fh.ehConversaSemComando(body));
              if (!_tratouCaixa && _souConversa && _fh.temPendencia && _fh.temPendencia(chatId)) {
                _caixaLog({ step: 'guarda_pendencia_conversa_ignorada', chatId: chatId });
                _tratouCaixa = true;   // nao responde, mas tambem nao manda pro LLM
              }
              // Com pendencia aberta, so intercepta quando a mensagem parece ser PRA
