...
- Edge **`sync-matriculas-emusys`** (v5, verify_jwt=false, **dry-run** via env `SYNC_MATRICULAS_DRYRUN` default true): varredura `GET /matriculas` (status=todas, throttle 1.1s) por unidade (`?u=cg|recreio|barra`). Motor `reconciliar()` puro → `{upd(AUTO), divergencias[]}`. AUTO (só aplica se NÃO dry-run): status, data_fim, curso_id, professor_atual_id, **valor LIMPO** (`liquido=cheio−fixo−cond>=0`). FILA (`matriculas_divergencias`, gravada SEMPRE mesmo em dry-run): `ambiguo`, `ausente_api`, `disciplina_nao_mapeada`, `valor_divergente`, `classificacao_divergente`.
- **Régua de valor**: `liquido<0`/`cheio<=0` = `desconto_fixo` embutido no `valor_mensalidade` (fonte ambígua) → enfileira `valor_divergente` sugerindo `efetivo=cheio−cond` (só se >0). NUNCA grava parcela negativa.
...
- RPC **`get_conciliacao_matriculas(p_unidade_id)`** lê da fila (resolvido=false E sem decisão) → `{resumo, items}`.
- UI: aba **Conciliação Emusys** (`ConciliacaoMatriculas.tsx`) na página Alunos — filtros (nome/tipo/unidade), paginação 20/pág, ações por linha (Aplicar API / Editar valor / Reclassificar / Manter / Ignorar) + lote. Ação grava no aluno → fixa campo em `matriculas_campos_fixados` (UNIQUE aluno_id,campo, bloqueia sobrescrita da varredura) → decisão → resolvido.
...
- **Cron (decisão Hugo 2026-06-23 = opção A "só fila")**: 3 jobs pg_cron `sync-matriculas-{cg,recreio,barra}` (jobids 24/25/26), 1×/dia defasados 20min (02:00/02:20/02:40 UTC = 23:00/23:20/23:40 BRT), `net.http_post` com `?u=<unidade>` + anon key. **Roda em dry-run** (env não setada → default true) → NUNCA toca em `alunos`, só popula a fila. v6: em dry-run a edge NÃO grava `automacao_log` de AUTO (evita ~250 logs/rodada). Ligar AUTO no futuro: setar `SYNC_MATRICULAS_DRYRUN=false` (validar amostra antes).
