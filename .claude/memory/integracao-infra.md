...
### WhatsApp / CRM
- `webhook-whatsapp-inbox` — router UAZAPI (messages + messages_update). Captura nome do contato (pushName) de `message.senderName`/`chat.name`/`chat.wa_name` e atualiza `nome_externo` a cada msg (reflete troca de nome, sem duplicar — elo da conversa é o NÚMERO). Conversa externa em caixa consolidada é casada por telefone+departamento+`aluno_id NULL`+`unidade_id NULL`. Respostas `buttonOrListid` da pesquisa pós-1ª aula são encaminhadas antes da evasão; nunca viram resposta de evasão do mesmo telefone e continuam sendo gravadas na Caixa mesmo se o processador de nota falhar. O código local também contém o motor append-only da evasão, opt-in por `resposta_ingestao_versao`: resolução por mensagem citada ou telefone+caixa, deduplicação por provider ID e compatibilidade integral para `legado_v1`. **Em 02/08/2026 esse motor e suas migrations estavam apenas preparados localmente, sem deploy/aplicação em produção.** Os logs de execução passam exclusivamente por `diagnostics.ts`, que copia apenas correlation ID, caixa, tipo, rota, resultado, status/código fechado e duração; payload, telefone, nomes, texto, mídia, transcrição, URL, token e corpo de erro externo não entram no logger. Deploy via Supabase CLI com os arquivos locais, NÃO via MCP inline.
...
- `sync-presenca-emusys` (v58, 2026-06-09) — sync aulas/presenca do Emusys (pg_cron diario **19:50-19:54 BRT**, antecipado de 22h em 2026-06-23). **Só registra presença; NÃO calcula mais `dia_aula`/`horario_aula`** (movido para a função SQL `sincronizar_grade_horaria_alunos`).
- **DEDUP de experimental por CHAVE DE NEGÓCIO (2026-07-14) — bug do webhook reenviado.** ⚠️ O Emusys **REENVIA** o webhook `aula_experimental_criada` **2-4× por experimental** (rajada <1s). No payload, `body.id` é o **id do EVENTO**, NÃO da aula — o id real da aula (~250k-800k) **não vem no webhook** (dentro de `aula` só há `lead_id`/`sala_id`; confirmado via `GET /aulas`: 3 webhooks → 1 aula real). A coluna `lead_experimentais.emusys_aula_id` guardava esse id de evento → o `ON CONFLICT (emusys_aula_id)` **nunca colapsava** → 1 experimental virava N linhas (inflava "agendadas" ~2×; CG jul mostrava 10 real 5). Pior: a **sync** inseria +1 linha (buscava pelo id real da aula, que a linha do webhook não tinha) → **realizadas** também duplicavam (69/168 em 30d). **Fix (forward-only, histórico NÃO colapsado):** dedup por `lead+data+horário+curso`. (1) RPC `registrar_experimental` → `pg_advisory_xact_lock` na chave + SELECT/UPDATE/INSERT (não mais `ON CONFLICT emusys_aula_id`); guard: não rebaixa realizada→agendada. (2) Edge **v58** ganhou "match 2b" em `reconciliarExperimentaisOrfas`: casa a linha do webhook pela chave de negócio antes de inserir. Chave usa `nome_aluno` (homônimo real = 0; os "colisores" eram mesma pessoa c/ lead interno duplicado). **PENDENTE:** gate no n8n `Fucq0bQwF4oeuWnv` p/ as **mensagens** duplicadas (consultor/professor/lead recebem 3×) — banco só corrige contagem, não as msgs. Detalhe timeout `/aulas` (falta de filtro de data = varredura, 9s) na skill `emusys-api`.
...
- `enviar-pesquisa-evasao` — pesquisa churn via WhatsApp. **Contrato V2 preparado localmente em 2026-07-30, ainda sem deploy:** `verify_jwt=true`; POST aceita somente `previsualizar` (movimentação canônica válida, permissão estrita por unidade, assinatura/template ativos únicos, produção só com `telefone_snapshot`, snapshot/hash por 10 min) ou `confirmar` (somente `preview_id`, reautorização pelo escopo persistido e claim service-only). O claim separa slots produção/teste, preserva replay histórico, bloqueia clique/concorrência e transforma `enviando` stale em `incerto` sem redispatch. UAZAPI/WAHA não oferecem idempotência de transporte comprovada neste contrato; 5xx/timeout/resposta sem message ID ficam `incerto` para reconciliação humana. Teste nunca abre fluxo de conversa real nem altera `alunos`/`movimentacoes_admin`.
...
## Observador emusys (edge `debug-webhook-emusys-observador`) — em sombra, pronto para virar (2026-07-30)
- **O que é**: edge que captura em paralelo os webhooks `lead_criado`/`lead_editado`/`aula_experimental_*` do Emusys e chama as **mesmas RPCs canônicas que o n8n chama** (`upsert_lead`, `registrar_experimental`) — não reimplementa a lógica em TS (v15 fazia isso e gerava regressões; v16, 2026-07-29, corrigiu chamando a RPC direto). `lead_arquivado` fica **fora de escopo por desenho** (n8n trata via UPDATE direto e deve continuar).
- **`OBSERVADOR_DRY_RUN=true`** (default, não alterado) → só lê e loga em `automacao_log` (`workflow_id='observador-worker'`), nunca escreve. O n8n (workflows `EB0LibpOJCLhKp7M` lead, `j41tPbyjGXUQUxrN` experimental) segue sendo o único que grava em produção.
- **O que o observador adiciona sobre a RPC pura**: resolução de professor por `telefone_whatsapp`+ativo (fallback `professores_unidades.emusys_nome` da unidade, só ativos — mais rígido que o `ILIKE` do n8n) e aplica `agente_comercial`/`motivo_arquivamento`/`etapa_pipeline_id` como delta pós-upsert.
- **Status em 2026-07-30 (1º dia limpo pós-fix v16)**: 0 erros, cobertura 100% (120/120 eventos de lead batendo com `leads_automacao_log` do n8n), `lead_not_found` zerado (era 76% em 28-29/07), curso resolvido em 100% das experimentais (inclusive Campo Grande, que antes zerava), sem colisão de upsert em caso real de multi-instrumento mesmo lead/mesma data (Alice/Helena Iribarne, `emusys_aula_id` distinto). Detalhe da investigação nas daily-notes 2026-07-29 e 2026-07-30.
- ⚠️ **Correção (2026-07-30, lido o código-fonte real, não suposição)**: o match `(unidade_id, emusys_lead_id)` do observador só existe na função `previewLeadExperimental` — que roda **só em `DRY_RUN=true`**, é puramente diagnóstico (o próprio comentário no código diz "quem escreve é a RPC; se divergirem, a RPC vale") e NUNCA é usada quando `DRY_RUN=false`. No modo real, o observador chama `sb.rpc('registrar_experimental', args)` direto, com o mesmo `p_emusys_lead_id` que o n8n manda. O bug da camada 1 (`WHERE emusys_lead_id = p_emusys_lead_id` **sem** `AND unidade_id=...`, confirmado lendo `pg_get_functiondef`) está **dentro da RPC**, não no chamador — então **virar a chave NÃO corrige esse bug**; ele persiste idêntico com qualquer um dos dois chamadores. Corrigir exige alterar a RPC em si (fora do escopo do observador).
- **Confirmado na fonte (não suposição)**: `upsert_lead` tem um `CASE` fixo que já reconhece o texto cru do Emusys (`'ALUNO DA ESCOLA'→'Indicação'`, `'PLACA DA FACHADA'→'Visita/Placa'`, `'EX ALUNO'→'Ex-aluno'`, `'INTERNET'→'Google'`, etc.) — o observador manda o texto cru de propósito (de-para caseiro removido em v16) e a RPC resolve sozinha.
- **O que de fato mudaria ao trocar (pra melhor, por desenho "pode adicionar, remover não")**: professor resolvido com mais rigor (telefone+ativo, não `ILIKE` solto — evita o bug Erick 54×52); 3 campos que o n8n hoje não grava (`agente_comercial`, `motivo_arquivamento`, `etapa_pipeline_id`, aplicados como delta pós-RPC). `lead_arquivado` **não está na lista de eventos que a edge processa** (`EVENTOS_LEAD = [lead_criado, lead_editado]`) — tem que continuar no n8n sempre, não é so uma recomendação operacional, é ausência de código. Comportamento em falha real de RPC (timeout/erro de rede) não foi testado — 0 erros até 30/07, sem amostra de falha pra comparar com o tratamento do n8n.
- **Antes de `DRY_RUN=false`**: desligar só os branches de lead/experimental dos 2 workflows n8n acima, no mesmo instante — manter o branch de `lead_arquivado` ativo (fora de escopo do observador).
- **Mapeamento de canal confirmado correto (2026-07-30)**: o observador manda o texto **cru** do Emusys pra `upsert_lead` (`ALUNO DA ESCOLA`, `PLACA DA FACHADA`, `EX ALUNO`, `INTERNET`...), o n8n manda a versão **já traduzida** (`Indicação`, `Visita/Placa`, `Ex-aluno`, `Google`) — textos diferentes por desenho (de-para caseiro removido do observador em v16). Verificado que o `canal_origem_id` final gravado é o mesmo nos dois caminhos: 71 leads editados hoje com `p_canal` preenchido pelo observador, 0 ficaram sem canal. A RPC resolve o texto cru sozinha. Curso passa igual nos dois lados (mesma string), sem de-para prévio de nenhum caminho.
...
## GRANTs de tabela x RLS — regressão da auditoria de segurança (2026-07-18)
- **Incidente**: aba **Automação** (Alunos) e página **AutomacoesPage** (Feed/Jornadas) mostravam "0 registros". Console: `42501 permission denied for table automacao_log` (não 403/empty). Causa: a auditoria de segurança de 2026-06-30 revogou o **GRANT SELECT** de `authenticated` em `automacao_log`, mas essa tabela é lida DIRETO pelo front (`TabAutomacao.tsx` + hook `useAutomacoesData`). Fix: migration `restore_grant_select_automacao_log_authenticated` (`GRANT SELECT ON public.automacao_log TO authenticated`).
- **Regra**: no Postgres o **GRANT de tabela é checado ANTES da policy RLS**. Policy `SELECT ... authenticated using(true)` presente + GRANT ausente = `42501` (não volta array vazio; a policy nem é avaliada). RLS negado ≠ GRANT negado. `anon` segue sem grant (correto).
- **Varredura feita** (só `automacao_log` estava quebrado ativo): outras tabelas sem grant p/ authenticated (`aluno_presenca_*`, `aula_alunos_emusys`, `presenca_politicas_confiabilidade`, `professor_ponto_confirmacoes`, `health_score_*`, `vw_*`) NÃO são bug — são lidas via RPC/view SECURITY DEFINER (grant ausente é intencional). Só quebra tabela que o front lê via `.from().select()`. `automacao_invariantes` (join embedded do hook) já tinha o grant.
- **Diagnóstico rápido de "tela vazia sem erro"**: 1) ver console (42501 vs 403 vs 200-vazio); 2) `role_table_grants` (grant existe?) ANTES de olhar `pg_policies` (RLS); 3) cruzar com `.from('<tabela>')` no front (bug ativo só se lida direto).

## Módulo Campanhas — agentes IA: notificação de consultor via Chatwoot (não mais Quepasa) (2026-07-14)
- **Quepasa foi descontinuado.** As caixas WhatsApp por unidade hoje rodam em **WAHA** por trás do Chatwoot — mas a `agente-webhook` (tool `transfer`) **não precisa falar com WAHA diretamente**: a notificação do consultor passa a usar a própria **API REST do Chatwoot** (mesma credencial `chatwoot_api_token`/`chatwoot_api_url`/`chatwoot_account_id` já configurada no agente, sem token novo).
- **Fluxo**: `executarTransfer` em `supabase/functions/agente-webhook/index.ts` — ao invés de `enviarMensagemQuepasa(quepasa_url, bot_token, consultant_phone, msg)`, agora faz `buscarContato`→`criarContato` (se não existir)→`criarConversa` (na `inbox_id` da unidade)→`enviarMensagem` (todas em `_shared/chatwoot-api.ts`), mandando uma mensagem normal (não nota privada) pro contato do consultor nessa inbox.
- **UI (`ModalEditarAgente.tsx`, `TransferConfig`)**: removidos os campos de Quepasa (`quepasa_bot_token`/`quepasa_url`); texto explicativo "a notificação do consultor é enviada pela própria API do Chatwoot (na inbox da unidade) — sem credencial adicional".
- ⚠️ **Gotcha de nome de unidade**: `executarTransfer` tem um `unitNorm` que normaliza por substring case-insensitive — "campo grande" vira o literal `'CG'`; "barra"/"recreio" viram `'Barra'`/`'Recreio'`. O campo `config.units[].name` no cadastro do agente **precisa ser digitado exatamente `"CG"`** (não "Campo Grande"), senão a comparação de unidade falha silenciosamente. Placeholder do input já ajustado pra "Nome (CG, Recreio ou Barra)".
- **Tipos**: `_shared/tool-types.ts` — removidos `quepasa_bot_token` (de `TransferUnit`) e `quepasa_url` (de `TransferToolConfig`). `enviarMensagemQuepasa` removida de `_shared/chatwoot-api.ts` (só era referenciada ali).
- ⚠️ **Gotcha de deploy da edge `agente-webhook` via MCP** (`deploy_edge_function`): quando os `files` passados misturam `index.ts` + módulos em subpasta (`_shared/*.ts`), a resolução de import relativo do tool é instável — ele envolve os arquivos num prefixo `source/` de forma inconsistente e quebra com `Module not found`. **Workaround usado**: bundlar TODO o código compartilhado (`chatwoot-api.ts`, `ai-client.ts`, `whatsapp-meta-api.ts`, `supabase-client.ts`, `tool-types.ts`) dentro de um único `index.ts` autocontido (zero imports relativos) só pro artefato de deploy — o código-fonte versionado no repo continua modular (`_shared/`) normalmente. **Cuidado**: uma tentativa de sondar o path com um arquivo placeholder (`entrypoint_path: "source/index.ts"`, conteúdo `"PLACEHOLDER"`) foi ao ar por engano como versão 30 e **quebrou a função em produção** por alguns instantes antes do fix (v31, bundle autocontido, validado via curl). Sempre testar deploy de edge crítica com curl logo após, e nunca usar conteúdo placeholder num deploy real — só em ambiente descartável.
- Deployado como **versão 31**, verificado funcionando. Commit: `996b4c8`.
